...
LIBSBML_EXTERN int FbcExtension_convertToV1IfNecessary(SBMLDocument_t* sbmlDocument)
{
  const FbcSBMLDocumentPlugin* plug = dynamic_cast<const FbcSBMLDocumentPlugin*>(sbmlDocument->getPlugin("fbc"));

  if (plug != NULL && plug->getPackageVersion() == 2)
  {
    ...
    return sbmlDocument->convert(props);
  }
