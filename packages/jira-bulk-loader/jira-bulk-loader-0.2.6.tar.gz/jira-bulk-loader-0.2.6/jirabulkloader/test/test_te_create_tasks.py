...
def existing_h5_task():
    return [
        {'markup': '...', 'issue_key': 'JIRA-1234',
            'description': 'line1\nline2', 'line_number': 3},
        {'assignee': 'assignee', 'markup': '#',
            'summary': 'sub-task', 'line_number': 4},
    ]


def test_subtask_for_existing_h5_task(te, existing_h5_task, dry_run_key):
    ...
    assert te.create_tasks(existing_h5_task) == \
        "... JIRA-1234\nline1\nline2\n# sub-task ({0})".format(dry_run_key)
    assert te.create_issue.call_args_list == [call(
        {'issuetype': 'Sub-task', 'assignee': 'assignee', 'markup': '#',
            'parent': 'JIRA-1234', 'summary': 'sub-task', 'line_number': 4})]
    assert te.update_issue_desc.call_args_list == \
        [call('JIRA-1234', u'line1\nline2\n# sub-task (DRYRUN-1234)')]
...
def test_subtask_for_existing_h4_task(te, existing_h4_task, dry_run_key):
    te.create_issue = MagicMock()
    te.create_issue.return_value = dry_run_key
    te.update_issue_desc = MagicMock()
    te.update_issue_desc.return_value = dry_run_key

    assert te.create_tasks(existing_h4_task) == \
        ".. JIRA-1234\n# sub-task ({0})".format(dry_run_key)
    assert te.create_issue.call_args_list == [call(
        {'issuetype': 'Sub-task', 'assignee': 'assignee', 'markup': '#',
            'parent': 'JIRA-1234', 'summary': 'sub-task', 'line_number': 4})]
