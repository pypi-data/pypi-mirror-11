...
import json
from autotag_app.models import Classifier, Tag
...
#             print(data_count)
#             data_encoded = [(w,c) for w,c in data_count]
            data_encoded = [(unicode(w, encoding = 'utf-8'),c) for w,c in data_count]
        ...
        if not data:
            ...
            for fname in os.listdir(self.res_dir):
                ...
                if not m:
                    continue
                try:
                    tag = get_object_or_404(Tag, name=fileName)
                except Http404:
                    tag = None
                if tag:
                    tags.append(tag)
...
#         features = [unicode(str(feature), encoding = 'utf-8', errors = 'replace') for feature in classifier.most_informative_features(n)]
    ...
    def dump_classifier(self, classifier, tag):
        '''
...
#             with open(os.path.join(self.res_dir, fname),'w') as classifier_file:
        with open('tmp','w') as classifier_file:
            pickle.dump(classifier,classifier_file)
        with open('tmp') as classifier_file:
            c = Classifier(name = fname)
            c.classifier_file = File(classifier_file)
            c.save()
...
#             raise Exception(error_msg)
    ...
    def load_classifier(self, tag):
        ...
        fname = unicode(tag) + '.clsfr'
        try:
            c = get_object_or_404(Classifier, name=fname)
        except Http404:
            pass
...
#             classifier = pickle.load(classifier_file)
        classifier = pickle.load(c.classifier_file)
        return classifier
...
#         data = [entry for entry in data if entry['tags']]
        for entry in data:
            entry['features'] = self.get_document_features(entry['text'])
        for tag in tags:
            print('training tag: ' + unicode(tag))
...
#             featuresets = [(self.get_document_features(entry['text']), tag in entry['tags']) for entry in test_set]
            ...
            print(unicode(tag)+': '+unicode(accuracy[tag]))
        return accuracy 
