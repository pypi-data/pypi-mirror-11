...
class NotFoundPublished(AppError):
  pass
...
def _remove_old_target(target_path, force=False, make_backup=False):
...
def _install_from_cache(cache_path, target_path, copy_type, force=False, make_backup=False):
  ...
  if copy_type == configs.CopyType.hardlink and os.path.isdir(cache_path):
    raise AppError("can't hardlink a directory: %s" % cache_path)

  _remove_old_target(target_path, force=force, make_backup=make_backup)
