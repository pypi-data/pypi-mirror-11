...
tickets_Deployment = resolve_model("tickets.Deployment")
...
def create_accounts_account(id, ref, seqno, name, sales_allowed, purchases_allowed, wages_allowed, clearings_allowed, clearable, chart, group_id, type):
    kw = dict()
    kw.update(id=id)
    kw.update(ref=ref)
...
def create_contacts_partner(id, modified, created, country_id, city_id, zip_code, region_id, addr1, street_prefix, street, street_no, street_box, addr2, name, language, email, url, phone, gsm, fax, remarks, isikukood, vat_regime, payment_term_id, invoice_recipient_id):
    kw = dict()
    kw.update(id=id)
...
def create_users_user(id, modified, created, username, password, profile, initials, first_name, last_name, email, remarks, language, partner_id, open_session_on_new_ticket, access_class, event_type_id):
    kw = dict()
    kw.update(id=id)
    kw.update(modified=modified)
    kw.update(created=created)
...
def create_ledger_movement(id, voucher_id, partner_id, seqno, account_id, amount, dc, match_id, satisfied):
    kw = dict()
    kw.update(id=id)
...
def create_finan_bankstatementitem(id, seqno, match_id, amount, dc, remark, account_id, partner_id, voucher_id, date):
    kw = dict()
    kw.update(id=id)
    kw.update(seqno=seqno)
    kw.update(match_id=match_id)
...
def create_finan_grouperitem(id, seqno, match_id, amount, dc, remark, account_id, partner_id, voucher_id):
    kw = dict()
    kw.update(id=id)
    kw.update(seqno=seqno)
    kw.update(match_id=match_id)
...
def create_finan_journalentryitem(id, seqno, match_id, amount, dc, remark, account_id, partner_id, voucher_id, date):
    kw = dict()
    kw.update(id=id)
    kw.update(seqno=seqno)
    kw.update(match_id=match_id)
...
def create_finan_paymentorderitem(id, seqno, match_id, amount, dc, remark, account_id, partner_id, voucher_id, bank_account_id):
    kw = dict()
    kw.update(id=id)
    kw.update(seqno=seqno)
    kw.update(match_id=match_id)
...
def create_tickets_project(id, ref, parent_id, company_id, contact_person_id, contact_role_id, closed, private, planned_time, name, assign_to_id, type_id, description, srcref_url_template, changeset_url_template):
    kw = dict()
    kw.update(id=id)
    kw.update(ref=ref)
    kw.update(parent_id=parent_id)
...
def create_clocking_servicereport(id, start_date, end_date, printed_by_id, interesting_for_id, ticket_state):
    kw = dict()
    kw.update(id=id)
    kw.update(start_date=start_date)
    kw.update(end_date=end_date)
    ...
    kw.update(ticket_state=ticket_state)
...
def create_sales_vatproductinvoice(voucher_ptr_id, partner_id, payment_term_id, match_id, total_incl, total_base, total_vat, vat_regime, your_ref, due_date, title, bank_account_id, printed_by_id, language, shipping_mode_id, subject, intro, discount):
    return create_mti_child(ledger_Voucher,voucher_ptr_id,sales_VatProductInvoice,voucher_ptr_id=voucher_ptr_id,partner_id=partner_id,payment_term_id=payment_term_id,match_id=match_id,total_incl=total_incl,total_base=total_base,total_vat=total_vat,vat_regime=vat_regime,your_ref=your_ref,due_date=due_date,title=title,bank_account_id=bank_account_id,printed_by_id=printed_by_id,language=language,shipping_mode_id=shipping_mode_id,subject=subject,intro=intro,discount=discount)
...
def create_tickets_milestone(id, printed_by_id, site_id, label, expected, reached, description, changes_since):
    kw = dict()
    kw.update(id=id)
    kw.update(printed_by_id=printed_by_id)
    kw.update(site_id=site_id)
    ...
    kw.update(description=description)
    kw.update(changes_since=changes_since)
    return tickets_Milestone(**kw)

def create_tickets_ticket(id, modified, created, closed, private, planned_time, project_id, site_id, product_id, nickname, summary, description, ticket_type_id, duplicate_of_id, reported_for_id, fixed_for_id, assigned_to_id, reporter_id, state, waiting_for, deadline, priority, feedback, standby):
    kw = dict()
    kw.update(id=id)
    kw.update(modified=modified)
    kw.update(created=created)
...
def create_tickets_deployment(id, ticket_id, milestone_id, remark):
    kw = dict()
    kw.update(id=id)
    kw.update(ticket_id=ticket_id)
    kw.update(milestone_id=milestone_id)
    kw.update(remark=remark)
    return tickets_Deployment(**kw)
...
def create_vat_vataccountinvoice(voucher_ptr_id, partner_id, payment_term_id, match_id, total_incl, total_base, total_vat, vat_regime, your_ref, due_date, title, bank_account_id):
    return create_mti_child(ledger_Voucher,voucher_ptr_id,vat_VatAccountInvoice,partner_id=partner_id,payment_term_id=payment_term_id,match_id=match_id,total_incl=total_incl,total_base=total_base,total_vat=total_vat,vat_regime=vat_regime,your_ref=your_ref,due_date=due_date,title=title,bank_account_id=bank_account_id)
...
def main():
    ...
    execfile("tickets_deployment.py")
