...
class App:
    ...
    def get_tpl(self, input):
        return {
            ...
            'dir' : os.path.join(get_script_dir(), 'development-environment', self.dir),
            'dev' : input == 'dev'
        }
...
def get_config(config_filename):
    try:
        with open(config_filename, 'r') as config_file:
            ...
            return data['output'], [ App(name, values) for name, values in data['apps'].iteritems() ]
    except IOError:
...
def main():    
    ...
    output, apps = get_config(args.config)
    ...
    output_dir = os.path.join(tempfile.mkdtemp(), output['dir'])
    os.mkdir(output_dir)
    
    output_yml = pystache.render(template, tpl)
    with open(os.path.join(output_dir, output['file']), 'w') as output_file:
        output_file.write(output_yml)

    print 
    print "Run this:"
    print
    print "export %s=\"%s\"" % (output['env'], output_file.name)
    print
