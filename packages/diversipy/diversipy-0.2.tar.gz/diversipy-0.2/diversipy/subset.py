...
# use as much Python 3 as possible
...
class MinBoundingBox:
    ...
    def closest_point_index(self, sought_point, candidate_indices,
    ...
    def obtain_representative(self, selection_target="centroid_of_hypercube",
                              tournament_size=0, dist_matrix_function=None):
        ...
        """
        representative_index = self.obtain_representative_index(selection_target, tournament_size, dist_matrix_function)
        representative = self.all_points[representative_index]
    ...
    def obtain_representative_index(self,
        ...
        if selection_target == "max_dist_from_boundary":
            ...
            representative_index = candidate_point_indices[max_dist_index]
        else:
            representative_index = self.closest_point_index(target_point, candidate_point_indices, dist_matrix_function)
...
def psa_partition(points, num_clusters, available_points_indices=None):
    ...
    while len(clusters) + 1 < num_clusters:
        ...
        indices1 = []
        indices2 = []
        indices1_append = indices1.append
        indices2_append = indices2.append
        for member_index in most_dissimilar_cluster.member_indices:
            if points[member_index][split_index] < split_position:
                indices1_append(member_index)
            else:
                indices2_append(member_index)
...
def psa_select(points, num_selected_points, available_points_indices=None,
               selection_target="centroid_of_hypercube", tournament_size=0,
               dist_matrix_function=None):
    ...
    representatives = [cluster.obtain_representative(selection_target, tournament_size, dist_matrix_function) for cluster in clusters]
...
def select_greedy_maximin(points, num_selected_points, existing_points=None, dist_matrix_function=None):
    ...
    if len(existing_points) > 0:
        ...
        previous_index = random.randrange(num_points)
    selected_indices = [previous_index]
    while len(selected_indices) < num_selected_points:
        distances = dist_matrix_function(np.atleast_2d(points_array[previous_index]), points_array)
...
def select_greedy_maxisum(points, num_selected_points, existing_points=None, dist_matrix_function=None):
    ...
    if len(existing_points) > 0:
        ...
        # choose an extremal point of the sample
        previous_index = np.argmin(points.sum(axis=1))
    selected_indices = [previous_index]
    while len(selected_indices) < num_selected_points:
        distances = dist_matrix_function(np.atleast_2d(points[previous_index]), points_array)
