...
def run(hexFile1 = None, hexFile2 = None, hexFile3 = None):
	...
	api = API.API(device_family)            # Initializing API with NRF51 family type (will be checked later to see if our guess was correct).
	api.open()                              # Open the dll with the set family type.
	api.connect_to_emu_without_snr()        # Connect to emulator, if multiple are connected - pop up will appear.

	# Check if family used was correct, otherwise change.
	...
	for hex_file_path in hexFiles: 
		...
		except Exception, e: # If hex_file_path not a valid file, print an error and raise an exception.
			api.close()
