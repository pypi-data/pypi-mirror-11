...
u"""Cross-platform which command."""
...
def which(program):
    u"""Identify the location of an executable file."""
...
def _get_path_list():
    return os.environ[u'PATH'].split(os.pathsep)
...
if os.name == u'nt':
    def find_exe(program):
        ...
        if ext:
            ...
            for ext in os.environ[u'PATHEXT'].split(os.pathsep):
    ...
    def get_path_list():
        ...
        if WIN_ALLOW_CROSS_ARCH:
            alt_sys_path = os.path.expandvars(ur"$WINDIR\Sysnative")
            if os.path.isdir(alt_sys_path):
                paths.insert(0, alt_sys_path)
            else:
                alt_sys_path = os.path.expandvars(ur"$WINDIR\SysWOW64")
                if os.path.isdir(alt_sys_path):
