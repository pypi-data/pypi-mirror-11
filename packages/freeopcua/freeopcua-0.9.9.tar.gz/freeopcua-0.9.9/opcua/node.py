...
def instanciate_node(parent, node_type, typedefinition, idx):
    """
    Instanciate a new node under 'parent' using a type
    """

    ra = _ReadAdder(parent.server, node_type.nodeid)
    ra.add(ua.AttributeIds.NodeClass)
    ra.add(ua.AttributeIds.BrowseName)
    ra.add(ua.AttributeIds.DisplayName)
    nclass, bname, dname = ra.read()
    ...
    rdesc.ReferencetypeId = ua.ObjectIds.HasComponent
    rdesc.TypeDefinition = typedefinition
...
def _instanciate_node(server, parentid, rdesc, idx):
    ...
    node.RequestedNewNodeId = generate_nodeid(idx)
    node.BrowseName = rdesc.BrowseName
    node.NodeClass = rdesc.NodeClass
    node.ParentNodeId = parentid
    node.ReferenceTypeId = ua.NodeId.from_string(ua.ObjectIds.HasComponent)
    node.TypeDefinition = rdesc.TypeDefinition

    if rdesc.NodeClass in (ua.NodeClass.Object, ua.NodeClass.ObjectType):
        ra = _ReadAdder(server, rdesc.NodeId)
        ra.add(ua.AttributeIds.Description)
        ra.add(ua.AttributeIds.WriteMask)
        ra.add(ua.AttributeIds.UserWriteMask)
        ra.add(ua.AttributeIds.EventNotifier)

        vals = ra.read()

        attrs = ua.ObjectAttributes()
        attrs.DisplayName = rdesc.BrowseName
        attrs.Description = vals[0]
        attrs.WriteMask = vals[1]
        attrs.UserWriteMask = vals[2]
        attrs.EventNotifier = vals[3]
        ...
        ra.add(ua.AttributeIds.WriteMask)
        ra.add(ua.AttributeIds.UserWriteMask)
        ra.add(ua.AttributeIds.Value)
        ra.add(ua.AttributeIds.DataType)
        ra.add(ua.AttributeIds.ValueRank)
        ra.add(ua.AttributeIds.ArrayDimensions)
        ra.add(ua.AttributeIds.AccessLevel)
        ra.add(ua.AttributeIds.UserAccessLevel)
        ra.add(ua.AttributeIds.MinimumSamplingInterval)
        ra.add(ua.AttributeIds.Historizing)

        vals = ra.read()

        attrs = ua.ObjectAttributes()
        attrs.DisplayName = vals[0]
        attrs.Description = vals[1]
        attrs.WriteMask = vals[2]
        attrs.UserWriteMask = vals[3]
        attrs.Value = vals[4]
        attrs.DataType = vals[5]
        attrs.ValueRank = vals[6]
        attrs.ArrayDimensions = vals[7]
        attrs.AccessLevel = vals[8]
        attrs.UserAccessLevel = vals[9]
        attrs.MinimumSamplingInterval = vals[10]
        attrs.Historizing = vals[11]

        node.NodeAttributes = attrs
        server.add_nodes([node])

    else:
        print("Node class not supported: ", rdesc)

    refs = []
    ref = ua.AddReferencesItem()
    ref.IsForward = True
    ref.ReferenceTypeId = rdesc.ReferenceTypeId
    ref.SourceNodeId = parentid
    ref.TargetNodeClass = rdesc.NodeClass
    ref.TargetNodeId = rdesc.NodeId

    refs.append(ref)
    server.add_references(refs)

    node = Node(server, rdesc.NodeId)
    descs = node.get_children_descriptions(includesubtypes=False)
    for rdesc in descs:
        _instanciate_node(server, node.nodeid, rdesc, idx)
    return node
