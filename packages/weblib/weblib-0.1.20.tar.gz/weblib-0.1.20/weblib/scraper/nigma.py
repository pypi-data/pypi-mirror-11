...
def build_search_url(query):
    url_tpl = 'http://nigma.ru/?t=web&s=%s'
...
def check_integrity(grab):
    ...
    elif grab.doc('//input[@name="captcha"]').exists():
        raise RequestBanned('Found captcha')


def check_search_result_integrity(grab):
    check_integrity(grab)
    if not grab.doc('//input[@name="s"]').exists():
        raise DataNotValid('Search query input not found')


def parse_search_result(grab):
    res = []
    for elem in grab.doc('//ol[@id="reslist"]'
                         '/li[div[@class="snippet_title"]/a]'):
        link = elem.select('div[@class="snippet_title"]/a')
        res.append({
            'url': link.attr('href'),
            'anchor': link.text(),
        })
