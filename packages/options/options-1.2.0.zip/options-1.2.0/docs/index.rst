...
``Transient``
  Option is not set initally or on a per-instance basis, but may be
 ...
 * This is a work in progress. The underlying techniques have been
   successfully used in multiple projects, but it remains in active
   development. The API may evolve over time.
