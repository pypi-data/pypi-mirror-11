...
"""
    ...
    Implements i18n/l10n support for MediaTUM (+jinja2) based on Babel.
    Forked from flask-babel, originally by Armin Ronacher and Daniel Neuhäuser.

    :copyright: (c) 2013 by Armin Ronacher, Daniel Neuhäuser, Tobias Stenzel
    :license: BSD, see LICENSE for more details.

"""
from __future__ import absolute_import
import os
from core.transition.globals import _request_ctx_stack, current_app
...
class Babel(object):
    ...
    def __init__(self, app=None, default_locale='en', default_timezone='UTC', translations_dirs=None,
                 date_formats=None, configure_jinja=True):
        self._default_locale = default_locale
        self._default_timezone = default_timezone
        self.translation_dirs = translations_dirs or ["i18n"]
    ...
    def init_app(self, app):
        ...
        if self._configure_jinja:
            ...
            app.jinja_env.globals.update(dict(
                list_locales=self.list_translations,
                get_current_locale=get_locale))

    def add_translations_dir(self, dirname):
        if dirname not in self.translation_dirs:
            self.translation_dirs.append(dirname)
    ...
    def list_translations(self):
        ...
        result = []
        for dirname in self.translation_dirs:
            if not os.path.isabs(dirname):
                dirpath = os.path.join(self.app.root_path, dirname)
            else:
                dirpath = dirname
            if not os.path.isdir(dirpath):
                return []
            for folder in os.listdir(dirpath):
                locale_dir = os.path.join(dirpath, folder, 'LC_MESSAGES')
    ...
    def _load_translation_from_dir(self, dirname, locale):
        if not os.path.isabs(dirname):
            dirpath = os.path.join(self.app.root_path, dirname)
        else:
            dirpath = dirname
        translations = self.translations.get(locale)
        new_translations = support.Translations.load(dirpath, locale)
        if translations is None or isinstance(translations, NullTranslations):
            self.translations[locale] = new_translations
        else:
            self.translations[locale] = translations.merge(new_translations)

    def _load_translations_for_locale(self, locale):
        for dirname in self.translation_dirs:
            self._load_translation_from_dir(dirname, locale)
        return self.translations[locale]

    def translations_for_locale(self, locale):
        translations = self.translations.get(locale)
        if translations is None:
            translations = self._load_translations_for_locale(locale)
        return translations
...
def get_translations():
    ...
    this request. This returns `None` if used outside of
    a request. Language changes during a request are not supported at the moment.
    """
    ...
    if translations is None:
        translations = current_app.extensions["babel"].translations_for_locale(get_locale())
