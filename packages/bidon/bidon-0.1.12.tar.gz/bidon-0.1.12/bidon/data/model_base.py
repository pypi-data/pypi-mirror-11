...
class ModelBase(object):
  ...
  def all_errors(self, joiner="; "):
    ...
    # return joiner.join("{0}: {1}".format(prop, errlist) for prop, errlist in self.errors.items())
    parts = []
    for pname in self.errors:
      for err in self.errors[pname]:
  ...
  def to_dict(self, *, include_keys=None, exclude_keys=None, use_default_excludes=True):
    ...
    if include_keys:
      ...
      # Exclude errors from the dict if there are None
      if not self.errors:
        if skeys:
          skeys.add("errors")
        else:
