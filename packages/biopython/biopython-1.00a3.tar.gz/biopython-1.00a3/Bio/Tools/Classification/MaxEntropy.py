...
def _train_iis(xs, classes, features, f_sharp, alphas, e_empirical):
    ...
    #e = _calc_model_expects(xs, classes, features, alphas)
    #print "EMPIRICAL:", e_empirical
    #print "MODEL:", e
    return newalphas
...
##import profile
##training_set = map(lambda x: x[0], samples)
##results = map(lambda x: x[1], samples)
##profile.run("me = train(training_set, results, features)")
##me = train(training_set, results, features)
##print me.alphas
