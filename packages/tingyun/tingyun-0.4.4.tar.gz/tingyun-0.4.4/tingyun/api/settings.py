    ...
    def _flatten(settings_item, name, setting_object):
        for key, value in setting_object.__dict__.items():
            if key in filter_pro:
                _logger.debug("skip flatten property %s", key)
                continue
...
def apply_config_setting(settings_object, name, value):
    ...
    if fields[0] == "ignored_status_codes" and value:
        try:
            ...
            _logger.warning("got invalid ignore status code %s, errors %s", value, err)
    ...
    if fields[0] == "urls_captured" and value:
        try:
            ...
            _logger.warning("compile re url failed, %s, %s", value, err)
...
def merge_settings(server_side_config=None):
    ...
    global_conf_name = ["applicationId", "enabled", "appSessionKey", "dataSentInterval", "apdex_t"]

    if not server_side_config:
        _logger.warn("update server config failed %s, local settings will be used.", server_side_config)
    ...
    for name in global_conf_name:
        if name not in server_side_config:
            _logger.warning("Lost server configure %s", name)
            continue
    ...
    _logger.info("return merged settings %s", settings_snapshot)
    return settings_snapshot
