...
from tingyun.api.tracert.attribution import TimeMetric, ApdexMetric, TracedError
from tingyun.api.tracert.attribution import node_start_time, node_end_time


_logger = logging.getLogger(__name__)
_TransactionNode = namedtuple('_TransactionNode', ['type', 'group', 'name', 'start_time', 'end_time', 'request_uri',
...
class TransactionNode(_TransactionNode):
    """hold the transaction trace data for collect
    ...
    def action_metrics(self):
        """
        :return: the full transaction metric of the top
        """
    ...
    def apdex_metrics(self):
        ...
        """
        if self.type != "WebAction":
            _logger.debug("get apdex with none webaction %s", self.type)
            return
    ...
    def traced_error(self):
        ...
        for error in self.errors:
            ...
            yield TracedError(error_filter_key=error_filter_key, transaction_type=error.transaction_type,
    ...
    def slow_action_trace(self, limit):
