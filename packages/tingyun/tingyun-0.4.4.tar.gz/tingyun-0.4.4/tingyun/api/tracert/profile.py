...
class Profile(object):
    ...
    def profile_data(self):
        ...
        for category in self.final_data:
            ...
            if (category not in self.stacktraces) or (not self.stacktraces[category].has_child()):
                _logger.info("Get empty data with category: %s", category)
    ...
    def update_thread_trace(self, category, new_node):
        ...
        if not new_node.has_child:
            _logger.info("New profile node is empty, ignore it now.")
            return
        ...
        if key not in self.stacktraces:
            ...
            # _logger.info('new tree with category: %s, data: %s', category, new_node)
            return
...
class ProfileManager(object):
    ...
    def start_profile(self, cid, app_name, profile_id, duration, interval):
        ...
        if not self.enable:
            _logger.info("profile disabled in local settings.")
            return False

        if self.current_profile:
            _logger.warning("There's maybe running a profile, this is a unnormal status, if this continues,"
                            "please report us for further investigation.")
            return False

        with self.lock:
            if self.profile_thread_running:
                _logger.warning("There are running a thread profile, this is a unnormal status, if this continues,"
                                "please report us for further investigation.")
                return False
    ...
    def _do_profile(self):
        ...
        :return:
        """
        _logger.info("Staring thread for doing profile.")
        while True:
            if not self.current_profile:
                _logger.info("Stopping profile now.")
                return
            ...
            for thread_id, category, stack_node in self.trace_thread_stack():
                if not stack_node.has_child():
                    _logger.debug("Do not get the thread stacktrace.")
                    continue
    ...
    def update_profile_status(self):
        ...
        if self.current_profile:
            if time.time() > self.cmd_info['end_time']:
                self.stop_profile()
                _logger.info("Finish the profile thread.")

    def trace_thread_stack(self):
        """
        :return:
        """
        for transaction, thread_id, category, frame in transaction_cache().get_active_threads():
    ...
    def generate_profile_data(self):
        ...
        if 0 == len(self.finished_profiles):
            _logger.warning("We got empty profile data, this maybe some issues of the python code, if this continues, "
                            "please report to us ofr further investigation. Thank U.")
            _logger.warning("Current command information is %s", self.cmd_info)
            return None

        try:
            final_data = {"id": self.cmd_info['cid'], "result": self.finished_profiles[0].profile_data()}
        except Exception as err:
            _logger.warning("Errors, when generate profile data. %s, %s", self.cmd_info, err)
            self.finished_profiles = []
            return None
    ...
    def shutdown(self, stop_cmd=False):
        ...
        :return:
        """
        _logger.info("Force shutdown the profile, because receive the shutdown signals.")
        self.stop_profile()
        self.finished_profiles = []
        if stop_cmd:
