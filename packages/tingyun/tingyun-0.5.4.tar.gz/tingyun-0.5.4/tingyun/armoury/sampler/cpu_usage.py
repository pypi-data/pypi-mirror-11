...
class CpuUsage(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        now = os.times()
        cpu_use_time = int((now[0] + now[1]) - (self.__times[0] + self.__times[1]))
        self.__times = now

        cpu_use_metric = "GENERAL/CPU/NULL/UserTime"
        yield [TimeMetric(name=cpu_use_metric, scope=cpu_use_metric, duration=cpu_use_time, exclusive=cpu_use_time), ]
