...
def calculate_metrics(dt, stop_event, calcs):
    pre = datetime.now()
    end_date = datetime(dt.year, dt.month, dt.day)
    end_date = end_date.replace(hour=23, minute=59, second=59, microsecond=999999)
    t_end = calendar.timegm(end_date.timetuple())
    t_begin = calendar.timegm(dt.timetuple())
    ...
    log.info('Updated {} calculations ({}) for day {} in {}ms'.format(len(calc_names), calc_names, dt, took))
