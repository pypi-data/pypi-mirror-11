...
def __build_time_chunk(store, key, begin, end, fill):
    ...
    while _next < end:
        _end = _next + 86400
        stored_values = [eval(res)['v'] for res in store.db.zrangebyscore(key, '({}'.format(_next - 1), _end)]
        if stored_values:
...
def aggregate(store, key, begin, end, max_n, aggr=sum, fill=0, extend=False):
    ...
    while step_begin - end + step <= 1:
        ...
        if not extend:
            ...
            if pre_fill:
                chunk += [fill] * pre_fill
            chunk += list(__build_time_chunk(store, key, math.floor(step_begin), math.ceil(step_end), fill))
