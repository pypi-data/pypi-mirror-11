...
def __build_time_chunk(store, key, begin, end, fill):
    ...
    while _next < end:
        ...
        stored_values = [eval(res)['v'] for res in store.db.zrangebyscore(key, _next, min(end, _end - 1))]
        if stored_values:
...
def aggregate(store, key, begin, end, max_n, aggr=sum, fill=0, extend=False):
    ...
    begin_dt = datetime.utcfromtimestamp(begin)
    begin = date(begin_dt.year, begin_dt.month, begin_dt.day)
    begin = calendar.timegm(begin.timetuple())

    step_begin = begin
    values = []
    step = get_step()

    extend = begin < data_begin or end > data_end or max_n != 1

    offset = -1
    while step_begin - end + step <= 1:
        step_end = step_begin + step
        if end - step_end <= 1:
                offset = 0
        if not extend:
            chunk = [eval(res)['v'] for res in store.db.zrangebyscore(key, step_begin, step_end + offset)]
        else:
            chunk = []
            pre_fill = int(math.ceil((data_begin - begin) / 86400))
            if pre_fill:
                chunk += [fill] * pre_fill
            chunk += list(__build_time_chunk(store, key, math.floor(step_begin), math.ceil(step_end + offset), fill))
