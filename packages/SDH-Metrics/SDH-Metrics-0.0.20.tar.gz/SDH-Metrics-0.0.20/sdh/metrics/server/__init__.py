...
class MetricsApp(AgoraApp):
    @staticmethod
    def __get_metric_definition_graph(md, info):
        g = MetricsGraph()
        ...
        if desc is not None:
            g.add((me, PLATFORM.description, Literal(desc)))
        return g
    ...
    def __get_definition(self, md):
        ...
        g = self.__get_metric_definition_graph(md, self.metric_info[md])
    ...
    def orgmetric(self, path, aggr, mid, title=None, description=None):
        def context(request):
            return [], self._get_metric_context(request)

        return lambda f: self.metric(path, context, '{}-org-{}'.format(aggr, mid), title, description)(f)

    def repometric(self, path, aggr, mid, title=None, description=None):
        def context(request):
            return [self._get_repo_context(request)], self._get_metric_context(request)

        return lambda f: self.metric(path, context, '{}-repo-{}'.format(aggr, mid), title, description)(f)

    def usermetric(self, path, aggr, mid, title=None, description=None):
        def context(request):
            return [self._get_user_context(request)], self._get_metric_context(request)

        return lambda f: self.metric(path, context, '{}-user-{}'.format(aggr, mid), title, description)(f)

    def repousermetric(self, path, aggr, mid, title=None, description=None):
        def context(request):
            return [self._get_repo_context(request), self._get_user_context(request)], self._get_metric_context(request)

        return lambda f: self.metric(path, context, '{}-repo-user-{}'.format(aggr, mid), title, description)(f)

    def orgtbd(self, path, mid, title=None, description=None):
        def context(request):
            return [], self._get_tbd_context(request)

        return lambda f: self.metric(path, context, 'tbd-org-' + mid, title, description)(f)

    def repotbd(self, path, mid, title=None, description=None):
        def context(request):
            return [self._get_repo_context(request)], self._get_tbd_context(request)

        return lambda f: self.metric(path, context, 'tbd-repo-' + mid, title, description)(f)

    def usertbd(self, path, mid, title=None, description=None):
        def context(request):
            return [self._get_user_context(request)], self._get_tbd_context(request)

        return lambda f: self.metric(path, context, 'tbd-user-' + mid, title, description)(f)

    def userrepotbd(self, path, mid, title=None, description=None):
        def context(request):
            return [self._get_repo_context(request), self._get_user_context(request)], self._get_tbd_context(request)

        return lambda f: self.metric(path, context, 'tbd-repo-user-' + mid, title, description)(f)
