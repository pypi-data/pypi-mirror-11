...
class Model(object):
    ...
    def get_by_map(clz, params={}, start=0, limit=50, and_or="and", select_cols=None, order_by=None, group_by=None, force_index=None):
        ...
        try:
            if not params:
                if select_cols:
                    sql = "select %s from %s %s %s %s limit %s,%s" % ("`" + "`,`".join(select_cols) + "`", clz.tablename, force_index, group_by, order_by, start, limit)
                    logger.debug(sql)
                    c.execute(sql)
                else:
                ...
                if select_cols:
                    sql = "select %s from  %s %s where %s %s %s limit %s,%s" % (
                        "`" + "`,`".join(select_cols) + "`", clz.tablename, force_index, cond, group_by, order_by, start, limit)
