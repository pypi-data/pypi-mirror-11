...
class Model(object):
    ...
    @classmethod
    def get_by_map(clz, params={}, start=0, limit=50, and_or="and", select_cols=None, order_by=None, group_by=None):
        c = clz.cursor()
        order_by = order_by and " order by " + ",".join(order_by) or ""
        group_by = group_by and ",".join("`%s`" % g for g in group_by) or None
        group_by = group_by and " group by %s" % group_by or ""
        try:
            if not params:
                if select_cols:
                    sql = "select %s from %s %s %s limit %s,%s" % ("`" + "`,`".join(select_cols) + "`", clz.tablename, group_by, order_by, start, limit)
                    logger.debug(sql)
                    c.execute(sql)
                else:
                    sql = "select * from %s %s %s limit %s,%s" % (clz.tablename, group_by, order_by, start, limit)
                    logger.debug(sql)
                    c.execute(sql)
            else:
                ...
                if select_cols:
                    sql = "select %s from  %s where %s %s %s limit %s,%s" % (
                        "`" + "`,`".join(select_cols) + "`", clz.tablename, cond, group_by, order_by, start, limit)
                    logger.debug(sql + ", %s" % vals)
                    c.execute(sql, vals)
                else:
                    sql = "select * from %s where %s %s %s limit %s,%s" % (clz.tablename, cond, group_by, order_by, start, limit)
                    logger.debug(sql + ", %s" % vals)
                    c.execute(sql, vals)
            return c.fetchall()
        finally:
