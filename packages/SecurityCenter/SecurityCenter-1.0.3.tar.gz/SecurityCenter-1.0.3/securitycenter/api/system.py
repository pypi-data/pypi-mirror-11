...
class System(Module):
    ...
    @extract_value('ip')
    def resolve_host(self, hostname, dns_type=None):
        return self._request('resolveHost', {
            'hostname': hostname,
            'dnsType': dns_type
        })
...
class Heartbeat(Module):
    ...
    def beat(self, module=None, module_params=None, messages_viewed=None, messages_deleted=None, id=None):
        ...
        return self._request('beat', {
            'messagesViewed': [{'id': int(m_id)} for m_id in messages_viewed] if messages_viewed else [],
            'messagesDeleted': [{'id': int(m_id)} for m_id in messages_deleted] if messages_deleted else [],
            'module': module,
