...
def get_debug_level():
    debug_level = os.environ.get('DEBUG') or 0
    try:
...
class DynoCli(click.MultiCommand):
    ...
    def get_command(self, ctx, name):
        ...
        try:
            module = importlib.import_module("{module}.cli".format(
                module=module_def['module']))
        except ImportError, e:
            if get_debug_level() > 0:
                message = ("WARNING: Could not import {module_name}\n" "Erorr message: {error}").format(module_name=name, error=e)
                return click.echo(message, err=True)
            return
        if module and not hasattr(module, 'cli'):
            if get_debug_level() > 0:
                return click.echo(
                    "WARNING: {module}.cli does not have `cli` object".format(
                        module_def['module']),
                    err=True)
            return
