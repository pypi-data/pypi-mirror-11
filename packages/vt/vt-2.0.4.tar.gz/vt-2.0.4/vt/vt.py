...
class vtAPI():
    ...
    def getReport(self, *args, **kwargs):
        ...
        for jdata in jdatas:
            ...
            if kwargs.get('search_intelligence'):
                if kwargs.get('return_json') and (kwargs.get('hashes') or 'hashes' in args):
                    self.return_json.update({'hashes': jdata['hashes']})
                else:
    ...
    def getDomain(self, *args,  **kwargs):
        ...
        for domain, jdata in jdatas:
            ...
            if jdata.get('response_code') and jdata['response_code'] == 1:
                ...
                if jdata.get('Websense ThreatSeeker category') and ((kwargs.get('websense_threatseeker'), 'websense_threatseeker' in args) or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
...
def main():
    ...
    if api_type:
        allinfo_opt = opt.add_argument_group(
            'All information related')

        allinfo_opt.add_argument('--report-all-info', action='store_true',
