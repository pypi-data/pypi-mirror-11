...
try:
     ...
     sys.exit('[-] Missed requests library')
...
class vtAPI():
        ...
        if result:
            ...
            for hashes_report in kwargs.get('value'):
                ...
                if kwargs.get('allinfo'):
                    self.params.setdefault('allinfo', kwargs.get('allinfo'))
            ...
            if jdata.get('response_code') == 0 or jdata.get('response_code') == -1:
                if kwargs.get('not_exit'):
                    ...
                    if jdata.get('verbose_msg'):
                        print '\n[!] Status : {verb_msg}\n'.format(verb_msg=jdata.get('verbose_msg'))
                    sys.exit()
    ...
    def rescan(self, **kwargs):
        ...
        for hash_part in kwargs.get('value'):
            ...
            if kwargs.get('delete'):
                ...
                if kwargs.get('date'):
                    self.params.setdefault('date', kwargs.get('date'))

                if kwargs.get('period'):
                    self.params.setdefault('period', kwargs.get('period'))

                    if kwargs.get('repeat'):
                        self.params.setdefault('repeat', kwargs.get('repeat'))

                if kwargs.get('notify_url'):
                    self.params.setdefault('notify_url', kwargs.get('notify_url'))

                    if kwargs('notify_changes_only'):
                        self.params.setdefault('notify_changes_only', kwargs.get('notify_changes_only'))
    ...
    def url_scan_and_report(self, **kwargs):
        """
        Url scan:
        ...
        for url_upload in url_uploads:
            ...
            if kwargs.get('key') == 'scan':
                ...
                self.params.setdefault('scan', kwargs.get('action'))
    ...
    def getIP(self, **kwargs):
        ...
        for ip, jdata in jdatas:
            if jdata['response_code'] == 0 or jdata['response_code'] == -1:
                ...
                if jdata.get('asn') and (kwargs.get('asn') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'asn':jdata['asn']})
                    else:
                        print '\n[+] ASN: {0}'.format(jdata['asn'])

                if jdata.get('as_owner') and (kwargs.get('as_owner') or kwargs.get('as_owner')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'as_owner':jdata['as_owner']})
                    else:
                        print '\n[+] AS owner: {0}'.format(jdata['as_owner'])

                if jdata.get('country') and (kwargs.get('country')  or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'country':jdata['country']})
                    else:
                        print '\n[+] Country: {0}'.format(jdata['country'])

                self.print_results(jdata, **kwargs)

                if jdata.get('resolutions') and (kwargs.get('passive_dns') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'passive_dns':jdata['resolutions']})
                    else:
    ...
    def getDomain(self, **kwargs):
        ...
        for domain, jdata in jdatas:
            ...
            if jdata.get('response_code') and jdata['response_code'] == 1:
                if jdata.get('categories') and (kwargs.get('categories') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'categories': jdata['return_json']})
                    else:
                        print '\n[+] Categories'
                        print '\t{0}'.format('\n\t'.join(jdata['categories']))
                if jdata.get('TrendMicro category') and (kwargs.get('trendmicro') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'trendmicro': jdata['TrendMicro category']})
                    else:
                        print '\n[+] TrendMicro category'
                        print '\t', jdata['TrendMicro category']
                if jdata.get('Websense ThreatSeeker category') and (kwargs.get('websense_threatseeker') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'websense_threatseeker': jdata['Websense ThreatSeeker category']})
                    else:
                        print '\n[+] Websense ThreatSeeker category'
                        print '\t', jdata['Websense ThreatSeeker category']
                if jdata.get('BitDefender category') and (kwargs.get('bitdefender') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'bitdefender': jdata['BitDefender category']})
                    else:
                        print '\n[+] BitDefender category'
                        print '\t', jdata['BitDefender category']
                if jdata.get('Dr.Web category') and (kwargs.get('drweb_cat') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'drweb_cat': jdata['Dr.Web category']})
                    else:
                        print '\n[+] Dr.Web category'
                        print '\t', jdata['Dr.Web category']
                if jdata.get('Alexa domain info') and (kwargs.get('alexa_domain_info') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'alexa_domain_info': jdata['Alexa domain info']})
                    else:
                        print '\n[+] Alexa domain info'
                        print '\t', jdata['Alexa domain info']
                if jdata.get('Alexa category')  and (kwargs.get('alexa_cat') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'alexa_cat': jdata['Alexa category']})
                    else:
                        print '\n[+] Alexa category'
                        print '\t', jdata['Alexa category']
                if jdata.get('Alexa rank') and (kwargs.get('alexa_rank') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'alexa_rank': jdata['Alexa rank']})
                    else:
                        print '\n[+] Alexa rank:'
                        print '\t', jdata['Alexa rank']
                if jdata.get('Opera domain info') and (kwargs.get('opera_info') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'opera_info': jdata['Opera domain info']})
                    else:
                        print '\n[+] Opera domain info'
                        print '\t', jdata['Opera domain info']
                if jdata.get('WOT domain info') and (kwargs.get('wot_domain_info') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'wot_domain_info': jdata['WOT domain info']})
                    else:
                        print '\n[+] WOT domain info'
                        plist = [[]]
                        ...
                        del plist
                if jdata.get('Webutation domain info') and (kwargs.get('webutation_domain') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'webutation_domain': jdata['Webutation domain info']})
                    else:
                        print "\n[+] Webutation"
                        plist = [[]]
                        ...
                        pretty_print_special(
                            plist, ['Name', 'Value'], [25, 20], ['c', 'c'])
                        del plist
                if jdata.get('whois') and (kwargs.get('whois') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'whois': jdata['whois']})
                    else:
                        print '\n[+] Whois data:'
                        print '\t{0}'.format(jdata['whois'].replace('\n', '\n\t'))
                if  jdata.get('whois_timestamp') and (kwargs.get('whois_timestamp') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'whois_timestamp': jdata['whois_timestamp']})
                    else:
                        print '\n[+] Whois timestamp:'
                ...
                if jdata.get('pcaps') and (kwargs.get('pcaps') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'pcaps': jdata['pcaps']})
                    else:
                        print '\n'
                ...
                if kwargs.get('walk'):
                    ...
                    for ip in sorted(jdata['resolutions'], key=methodcaller('get', 'last_resolved'), reverse=True):
                        if ip['ip_address'] not in filter_ip:
                            ...
                            self.getIP(**kwargs)
                if jdata.get('subdomains') and (kwargs.get('subdomains') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'subdomains': jdata['subdomains']})
                    else:
                        print '\n[+] Subdomains:'
                        print '\t{0}'.format('\n\t'.join(jdata['subdomains']))
                if jdata.get('domain_siblings') and (kwargs.get('domain_siblings') or kwargs.get('verbose')):
                    if kwargs.get('return_json'):
                        self.return_json.update({'domain_siblings': jdata['domain_siblings']})
                    else:
                        print '\n[+] Domain siblings:'
    ...
    def clusters(self, **kwargs):
        ...
        if result:
            ...
            if by_id:
                self.params.setdefault('query', 'cluster:{0}'.format(kwargs.get('value')[0]))
            else:
    ...
    def comment(self, **kwargs):
        ...
        if result:
            ...
            self.params.setdefault('resource', kwargs.get('value'))
            if kwargs.get('action') == 'add':
                url = self.base.format('comments/put')
                self.params.setdefault('comment', kwargs.get('value')[1])
                jdata, response = get_response(url, params=self.params, method="post")
                ...
                if kwargs.get('value')[0]:
                    self.params.setdefault('before', kwargs.get('date'))
                jdata, response = get_response(url, params=self.params)
                if kwargs.get('return_raw'):
                    return jdata
            else:
    ...
    def download(self, **kwargs):
        ...
        result, name = is_file(kwargs.get('value'))
        if result:
            hashes = open(kwargs.get('value'), 'rb').readlines()
        if isinstance(kwargs.get('value'), list) and len(kwargs.get('value')) == 1:
            pass
            ...
            if f_hash != '':
                ...
                if kwargs.get('api_type'):
                    if file_type not in ('file', 'pcap'):
                        ...
                        return
                    if file_type == 'pcap':
                        _, response = get_response(self.base.format('file/network-traffic'), params=self.params)
                        name = '{hash}.pcap'.format(hash=f_hash)
                    elif file_type == 'file':
                        _, response = get_response(self.base.format('file/download'), params=self.params)
                ...
                if len(response.content) > 0 and "VirusTotal - Free Online Virus, Malware and URL Scanner" not in response.content and '{"response_code": 0, "hash":' not in response.content:
                    if kwargs.get('return_raw'):
                        return response
                    else:
                        f = open(name, "wb")
    ...
    def distribution(self, **kwargs):
        ...
        if kwargs.get('value'):
            ...
            if kwargs.get('before'):
                self.params.setdefault('before', kwargs.get('before'))
            if kwargs.get('after'):
                self.params.setdefault('after', kwargs.get('after'))
            if kwargs.get('limit'):
                self.params.setdefault('limit', kwargs.get('limit'))

            if kwargs.get('action') == 'file':
                if kwargs.get('reports'):
                    self.params.setdefault('reports', str(kwargs.get('reports')).lower())
    ...
    def behaviour(self, **kwargs):
        ...
        if result:
            ...
            self.params.setdefault('hash', kwargs.get('value')[0])
        ...
        if jdata['info'] and kwargs.get('verbose'):
            if kwargs.get('return_json'):
                self.return_json.update({'info': jdata['info']})
            else:
                print '\nInfo\n'
        ...
        if kwargs.get('behavior_network') or kwargs.get('verbose'):

            print '\nHTTP requests\n'

            if 'behavior-network' in jdata and 'http' in jdata.get('network'):
                if kwargs.get('return_json'):
                    self.return_json.update({'http':jdata['network']['http']})
                else:
            ...
            if jdata['network']['hosts']:
                if kwargs.get('return_json'):
                    self.return_json.update({'hosts': jdata['network']['hosts']})
                else:
                    pretty_print(jdata['network']['hosts'], ['hosts'])

            if jdata['network']['dns']:
                if kwargs.get('return_json'):
                    self.return_json.update({'dns': jdata['network']['dns']})
                else:
                    print '\nDNS requests\n'
            ...
            if jdata['network']['tcp']:
                if kwargs.get('return_json'):
                    self.return_json.update({'tcp': jdata['network']['tcp']})
                else:
                    print '\nTCP Connections'
            ...
            if jdata['network']['udp']:
                if kwargs.get('return_json'):
                    self.return_json.update({'udp': jdata['network']['udp']})
                else:
                    print '\nUDP Connections'
                    unique = []
        ...
        if kwargs.get('behavior_process') or kwargs.get('verbose'):
            print '\nBehavior\n'
            print '\nProcesses\n'
            if kwargs.get('return_json'):
                    self.return_json.update({'processes': jdata['behavior']['processes']})
            else:
        ...
        if kwargs.get('behavior_summary') or kwargs.get('verbose'):
            if kwargs.get('return_json'):
                    self.return_json.update({'files':  jdata['behavior']['summary']['files']})
            else:
            ...
            if jdata['behavior']['summary']['keys']:
                if kwargs.get('return_json'):
                    self.return_json.update({'keys':  jdata['behavior']['summary']['keys']})
                else:
                    print '\nSet keys\n'
                    pretty_print(
                        sorted(jdata['behavior']['summary']['keys']), ['keys'], [100])

            if jdata['behavior']['summary'].get('mutexes') is not None and jdata['behavior']['summary']['mutexes'] != [u'(null)']:
                if kwargs.get('return_json'):
                    self.return_json.update({'mutex':  jdata['behavior']['summary']['mutex']})
                else:
                    print '\nCreated mutexes\n'
                    pretty_print(
    ...
    def print_results(self, jdata, **kwargs):

        if jdata.get('detected_downloaded_samples') and (kwargs.get('detected_downloaded_samples') or kwargs.get('verbose')):
            if kwargs.get('return_json'):
                self.return_json.update({'detected_downloaded_samples': jdata['detected_downloaded_samples']})
            else:
        ...
        if jdata.get('undetected_downloaded_samples') and (kwargs.get('undetected_downloaded_samples') or kwargs.get('verbose')):
            if kwargs.get('return_json'):
                self.return_json.update({'undetected_downloaded_samples': jdata['undetected_downloaded_samples']})
            else:
        ...
        if jdata.get('detected_communicating_samples') and (kwargs.get('detected_communicated') or kwargs.get('verbose')):
            if kwargs.get('return_json'):
                self.return_json.update({'detected_communicating_samples': jdata['detected_communicating_samples']})
            else:
        ...
        if jdata.get('undetected_communicating_samples') and (kwargs.get('undetected_communicating_samples') or kwargs.get('verbose')):
            if kwargs.get('return_json'):
                self.return_json.update({'undetected_communicating_samples': jdata['undetected_communicating_samples']})
            else:
        ...
        if jdata.get('detected_referrer_samples') and (kwargs.get('detected_referrer_samples') or kwargs.get('verbose')):
            if kwargs.get('return_json'):
                self.return_json.update({'detected_referrer_samples': jdata['detected_referrer_samples']})
            else:
                print '\n[+] Latest detected referrer files\n'
                pretty_print(sorted(jdata['detected_referrer_samples']), [
                             'positives', 'total',  'sha256'], [15, 10, 70], ['c', 'c', 'c'])

        if jdata.get('undetected_referrer_samples') and (kwargs.get('undetected_referrer_samples') or kwargs.get('verbose')):
            if kwargs.get('return_json'):
                self.return_json.update({'undetected_referrer_samples': jdata['undetected_referrer_samples']})
            else:
                print '\n[+] Latest undetected referrer files\n'
                pretty_print(sorted(jdata['undetected_referrer_samples']), [
                             'positives', 'total',  'sha256'], [15, 10, 70], ['c', 'c', 'c'])

        if jdata.get('detected_urls') and (kwargs.get('detected_urls') or kwargs.get('verbose')):
            if kwargs.get('return_json'):
                self.return_json.update({'detected_urls':  jdata['detected_urls']})
            else:
                url_size = max(
...
def main():
    ...
    try:
        ...
        sys.exit('''
        No API key provided or cannot read ~ /.vtapi. Specify an API key in ~ /.vtapi.
        Format:
            [vt]
    ...
    if api_type:
        ...
        more_private.add_argument('-dl', '--download',  dest='download', action='store_const', const='file', default=False,
