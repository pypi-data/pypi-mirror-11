...
class SlidingWindow(object):
    ...
    def run_target_async(self,token):
        def wrapper(*args):
            self.target(*args)
            self.cb_wrapper(args, token)

    def cb_wrapper(self, task, token):
        self.callback()
    ...
    def start(self, running_tasks=None, pending=None):
        self.run_async= False
        ...
        for task in self.tasks:
            ...
            try:
                process = Process(target= self.run_target_async(token), args=task)
    ...
    def start_async(self):
        self.run_async= True
        self.process = Process(target=self.start,args = (self.running_tasks, self.pending))
        print self.name, self.process.pid
        sys.stdout.flush()
        self.process.start()
