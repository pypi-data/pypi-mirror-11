...
$("#cargar_pagos").on("click",function(){
			...
			success:function(data){
				if (!$row.hasClass("diot_xml"))
				{
					if (data.msg == '')
					{
						if (data.total != null)
						{
							// SE AXSIGNA EL VALOR DE IVA ACREDITABLE CORRESPONDIENTE AL PAGO CARGADO SIN TOMAR EN
					...
					{
						folios_errores.push(folio+" - "+ data.msg);
					};
				};
	...
	if (folios_errores.length > 0)
	{
		alert("se encontraron inconsistencias en las siguientes relaciones XML-Poliza:\n"+folios_errores.join("\n"))
	};
