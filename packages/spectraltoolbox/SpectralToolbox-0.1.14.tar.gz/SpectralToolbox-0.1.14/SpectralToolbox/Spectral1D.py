...
Available polynomials:
    ...
    * Fourier or ``Spectral1D.FOURIER`` (despite not being polynomials..)
...
LAGUERREF = 'LaguerreF'
ORTHPOL = 'ORTHPOL'
AVAIL_POLY = [FOURIER, JACOBI, HERMITEP, HERMITEF, HERMITEP_PROB, LAGUERREP, LAGUERREF, ORTHPOL]
...
class Poly1D(object):
    ...
    def __init__(self,poly,params,sdout=sys.stderr):
        ...
        if self.poly == JACOBI:
            self.base_span = [-1.,1.]
        elif self.poly == HERMITEP or self.poly == HERMITEF or self.poly == HERMITEP_PROB:
    ...
    def __JacobiP(self,x,N,alpha=None,beta=None, old1=None, old2=None):
    ...
    def __LegendreP(self,r, N, old1=None, old2=None):
        """
        P = LegendreP(r,N)
    ...
    def __ChebyshevP(self,r, N, old1=None, old2=None):
        """
        T = ChebyshevP(r, N)
    ...
    def __HermiteP(self,r, N, old1=None, old2=None):
        """
        Returns the N-th Hermite Physicist polynomial using the recurrence relation
        """
        if (self.poly != HERMITEP and self.poly != HERMITEF):
    ...
    def __HermiteF(self,r, N, old1=None, old2=None):
        """
        ...
        if (self.poly != HERMITEF):
            ...
            new = (2.**N * factorial(N) * np.sqrt(np.pi))**(-.5) * np.exp(-(r**2.)/2.) * self.__HermiteP(r,N)
            return new
    
    def __HermiteP_Prob(self,r, N, old1=None, old2=None):
        """
        Returns the N-th Hermite Probabilistic polynomial using the recurrence relation
        Use the Probabilistic Hermite Polynomial
        """
        if (self.poly != HERMITEP_PROB and self.poly != HERMITEF):
    ...
    def __LaguerreP(self,r,N,alpha=None, old1=None, old2=None):
        """
    ...
    def __LaguerreF(self,r,N,alpha=None, old1=None, old2=None):
        """
    ...
    def __GradLaguerreP(self,r,N,k,alpha=None, old1=None, old2=None):
        """
    ...
    def __GradLaguerreF(self,r,N,k,alpha=None, old1=None, old2=None):
        """
    ...
    def __GradHermiteP(self,r,N,k, old1=None, old2=None):
        """
        ...
        if (self.poly != HERMITEP):
            ...
            if (N-k < 0):
                ...
                dP = 2.**k * fact * self.__HermiteP(r,N-k)
            return dP
        
    def __GradHermiteF(self,r,N,k, old1=None, old2=None):
        """
    ...
    def __GradHermiteP_Prob(self,r,N,k, old1=None, old2=None):
        """
        ...
        if (self.poly != HERMITEP_PROB):
            ...
            if (N-k < 0):
                ...
                fact = np.exp( gammalnF(N+1) - gammalnF(N-k+1) )
                dP = fact * self.__HermiteP_Prob(r,N-k)
            return dP
    
    def __GradJacobiP(self,r, N, k, old1=None, old2=None):
        """
    ...
    def __GradChebyshevP(self,r, N, k, method=0, old1=None, old2=None):
        """
    ...
    def GradEvaluate(self, r, N, k, norm=True, old1=None, old2=None):
        """
        ...
        :type norm: bool
        :param old1: previous polynomial in the recursion if available
        :type old1: 1d-ndarray
        :param old2: second to last polynomial in the recursion if available
        :type old2: 1d-ndarray
        ...
        if (self.poly == JACOBI):
            ...
            p = self.__GradHermiteP(r,N,k,old1,old2)
        elif (self.poly == HERMITEF):
    ...
    def GradVandermonde1D(self,r,N,k,norm=True):
        ...
        for i in range(0,N+1):
            if i == 0:
                ...
                DVr[:,i] = self.GradEvaluate(r,i,k,norm,DVr[:,i-1])
            else:
                DVr[:,i] = self.GradEvaluate(r, i, k, norm, DVr[:,i-1], DVr[:,i-2])
    ...
    def interpolate(self, x, f, xi, order):
        ...
        ..note:: this is the same of calling :py:method:`PolyInterp`
        .. seealso:: PolyInterp
        """
    ...
    def DiscretePolynomialTransform(self,r, f, N):
        ...
        if (self.poly == JACOBI) and ( AlmostEqual(alpha,-0.5) and AlmostEqual(beta,-0.5) ):
            ...
            if equal:
                ...
                fhat = self.FastChebyshevTransform(f)
        elif (self.poly == FOURIER):
            N = len(f)
