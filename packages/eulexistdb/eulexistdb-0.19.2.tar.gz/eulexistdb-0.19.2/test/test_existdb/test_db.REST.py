...
class ExistDBTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # don't create collection, just put resources & collection
        # will be created automatically
        # self.db_admin.createCollection(self.COLLECTION, True)

        self.db_admin.load('<hello>World</hello>', self.COLLECTION + '/hello.xml', True)

        xml = '<root><element name="one">One</element><element name="two">Two</element><element name="two">Three</element></root>'
        self.db_admin.load(xml, self.COLLECTION + '/xqry_test.xml', True)

        xml = '<root><field name="one">One</field><field name="two">Two</field><field name="three">Three</field><field name="four">Four</field></root>'
        self.db_admin.load(xml, self.COLLECTION + '/xqry_test2.xml', True)

    def tearDown(self):
        self.db_admin.removeCollection(self.COLLECTION)
    ...
    def test_failed_authentication_from_settings(self):
        ...
        try:
            ...
            test_db = db.ExistDB()
            self.assertRaises(exc.ExistDBException,
    ...
    def test_removeDocument(self):
        ...
        # FIXME: previously was db and not db_admin ??
        result = self.db_admin.removeDocument(self.COLLECTION + "/hello.xml")
    ...
    def test_getCollectionDescription(self):
        ...
        # FIXME: was testing as guest instead of admin user...
        self.assertEqual("/db" + self.COLLECTION, info.name,
            "collection name returned (expected '/db/%s', got '%s'" % (self.COLLECTION, info.name))
        self.assertEqual("eulexistdbtest", info.owner,
            "collection owner returned (expected 'eulexistdb', got %s" % info.owner)
        ...
        self.assertRaises(exc.ExistDBException, self.db.getCollectionDescription,
            "bogus_collection")

    def test_createCollection(self):
        """Test creating new collections in eXist"""
        # FIXME: no longer relevant since putting files creates collections?
    ...
    def test_query_with_no_result(self):
        ...
        # in exist 2.2, start is still 1 even if no results were returned
        self.assertEquals(qres.start, 1)
    ...
    # TODO: figure out how we want to handle execute query
    # and query caching.  No need to duplicate the xmlrpc api...
    ...
    def test_executeQuery(self):
        ...
        print 'summary - ', summary.serialize(pretty=True)
        self.assertEqual(2, summary.hits,
            "querySummary should returns hit count 2, got %s" % summary.hits)
        # no query time
    ...
    def test_executeQuery_noresults(self):
        ...
        self.assertRaises(exc.ExistDBException, self.db.retrieve, result_id, 0)
    ...
    def test_executeQuery_bad_xquery(self):
        ...
        self.assertRaises(exc.ExistDBException, self.db.executeQuery, xqry)
    ...
    def test_failed_authentication(self):
        ...
        test_db = db.ExistDB(server_url=EXISTDB_SERVER_URL, username='bad_user',
    ...
    def test_loadCollectionIndex(self):
        ...
        # FIXME: was previously testing as guest, not db_admin
        self.assertTrue(self.db_admin.loadCollectionIndex(self.COLLECTION, "<collection/>"))
        ...
        # overwrite now assumed
        # reload with overwrite disabled - should cause an exception
        # self.assertRaises(exc.ExistDBException, self.db.loadCollectionIndex,
    ...
    def test_removeCollectionIndex(self):
        ...
        self.db_admin.loadCollectionIndex(self.COLLECTION, "<collection/>")

        self.assertTrue(self.db_admin.removeCollectionIndex(self.COLLECTION))
        # collection config file should be gone
        self.assertRaises(exc.ExistDBException, self.db.getDocument,
        ...
        self.db_admin.loadCollectionIndex(self.COLLECTION, "<collection/>")
        # FIXME: the logic for removing the collection may need to change slightly

        # create something in a sub-collection
        # FIXME: something is adding an extra /db to the path...
        # self.db.createCollection(self.db._configCollectionName(self.COLLECTION) + "/subcollection")
        print self.db._configCollectionName(self.COLLECTION) + "/subcollection/test.xml"
        print 'loaded? ', self.db_admin.load(self.db._configCollectionName(self.COLLECTION) + "/test.xml", "<test/>")
        self.assertTrue(self.db_admin.removeCollectionIndex(self.COLLECTION))
    ...
    def test_reindexCollection(self):
        ...
        self.assertRaises(exc.ExistDBException, self.db.reindexCollection, "notacollection")
