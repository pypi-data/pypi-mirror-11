...
class Bible():
	def __init__(self, book="Genesis", chapter=1, verse_begin="", verse_end=""):
		...
		if len(str(verse_end)) == 0:
			homepage = "http://labs.bible.org/api/?passage={}%20{}:{}".format (book, chapter, verse_begin)
		else: homepage = "http://labs.bible.org/api/?passage={}%20{}:{}-{}".format(book, chapter, verse_begin, verse_end)
	...
	def read(self):
		...
		print("%s\n%s\n" % (self.header.upper(), len(self.header) * "="))
		print (results)
		return ""
	...
	def display(self, colour = (0, 0, 0), background = (255, 250, 250), size=28):
		...
		while running:
			...
			self._settings(screen, self.header, colour, font="verdanabold", size=25, position = (60, 72) )
			x = 0
			while x < len(self.content):
				self._settings(screen, self.content[x], colour, size=15, position = (30, 32*x+110))
				x +=1
