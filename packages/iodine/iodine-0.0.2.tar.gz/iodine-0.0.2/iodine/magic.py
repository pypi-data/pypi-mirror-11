...
class SaltMagics(Magics):
    ...
    def _print(self, result, end='\n'):
        result = yaml.safe_dump(result, default_flow_style=False)
        print(highlight(
            result,
            YamlLexer(),
            Terminal256Formatter()), end=end)
    ...
    def _run(self, args):
        try:
            ...
            #self._print(result)
            print("result in %s" % total_time)
            print("No return from: %s" % (set(minions) - set(final_result.keys())))
            return result
