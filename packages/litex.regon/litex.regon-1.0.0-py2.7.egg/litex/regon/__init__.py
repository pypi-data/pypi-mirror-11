...
detailed_report_re = re.compile('[^"]*"(?P<regon>[^"]*)","(?P<report_name>[^"]*)"')
detailed_report_names_map = {
    'DaneRaportPrawnaPubl': 'PublDaneRaportPrawna',
    'DaneRaportFizycznaPubl': 'PublDaneRaportFizycznaOsoba'
}
    ...
    def search(self, nip=None, regon=None, krs=None, nips=None, regons=None, krss=None, detailed=False):
        ...
        if not detailed:
            ...
            for rs in search_results:
                full_regon, report_name = detailed_report_re.search(str(rs.RegonLink)).groups()
                correct_report_name = detailed_report_names_map.get(report_name, report_name)
                rs.detailed = self.full_report(full_regon, correct_report_name)
