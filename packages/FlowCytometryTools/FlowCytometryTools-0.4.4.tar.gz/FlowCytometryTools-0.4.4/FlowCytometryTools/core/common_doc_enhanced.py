...
filename_to_ID_mapper : ['name' | 'number' | 'read' | mapping | callable]
...
ID_to_position_mapper : [None, callable, mapping, 'name', 'enumeration_row_first', 'enumeration_col_first']

    Returns the coordinates (row, col) which correspond to the key.
    (The key is the key extracted from the filename by the filename_to_ID_mapper.)

    For example, the key 'A1' corresponds to the matrix coordinates (0, 0).

        * None     : if None, then uses the same value as the filename_to_ID_mapper
        * callable : gets key and returns position
        * mapping  : key:pos
        * 'name'   : parses things like 'A1', 'G12'
        * 'enumeration_row_first' : converts number to positions, going over rows first
        * 'enumeration_col_first' : converts number to positions, going over columns first.""",
