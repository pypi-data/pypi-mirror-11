...
@doc_replacer
def _assign_IDS_to_datafiles(datafiles, filename_to_ID_mapper, measurement_class=None, **kwargs):
    """
    ...
    Dict of ID:datafile
    """
    if isinstance(filename_to_ID_mapper, collections.Mapping):
        fparse = lambda x: filename_to_ID_mapper[x]
    elif hasattr(filename_to_ID_mapper, '__call__'):
        fparse = lambda x: filename_to_ID_mapper(x, **kwargs)
    elif filename_to_ID_mapper == 'name':
    ...
    elif filename_to_ID_mapper == 'number':
        fparse = lambda x: int(x.split('.')[-2])
    elif filename_to_ID_mapper == 'read':
        fparse = lambda x: measurement_class(ID='temporary', datafile=x).ID_from_data(**kwargs)
    else:
        raise ValueError,  'Encountered unsupported value "%s" for parser parameter.' %filename_to_ID_mapper
...
class MeasurementCollection(collections.MutableMapping, BaseObject):
    ...
    @classmethod
    @doc_replacer
    def from_files(cls, ID, datafiles, filename_to_ID_mapper, readdata_kwargs={}, readmeta_kwargs={}, **ID_kwargs):
        """
        ...
        {_bases_ID_kwargs}
        """
        if ('parser' in kwargs.keys()) or ('position_mapper' in kwargs.keys()):
            raise ValueError('"parser" and "position_mapper" have been deprecated in favor '
                             'of filename_to_ID_mapper and ID_to_position_mapper')

        d = _assign_IDS_to_datafiles(datafiles, filename_to_ID_mapper, cls._measurement_class, **ID_kwargs)
        measurements = []
    ...
    @classmethod
    @doc_replacer
    def from_dir(cls, ID, datadir, filename_to_ID_mapper, pattern='*.fcs', recursive=False,
        ...
        return cls.from_files(ID, datafiles, filename_to_ID_mapper,
...
class OrderedCollection(MeasurementCollection):
    ...
    """
    @doc_replacer
    def __init__(self, ID, measurements, ID_to_position_mapper, shape=(8,12),
        ...
        ##set positions
        self._positions = {}
        self.set_positions(positions, ID_to_position_mapper=ID_to_position_mapper)
    ...
    @classmethod
    @doc_replacer
    def from_files(cls, ID, datafiles, filename_to_ID_mapper='name',
                   ID_to_position_mapper=None,
        ...
        if ('parser' in kwargs.keys()) or ('position_mapper' in kwargs.keys()):
            raise ValueError('"parser" and "position_mapper" have been deprecated in favor '
                             'of filename_to_ID_mapper and ID_to_position_mapper')
        if ID_to_position_mapper is None:
            if isinstance(filename_to_ID_mapper, basestring):
                ID_to_position_mapper = filename_to_ID_mapper
            else:
        ...
        for sID, dfile in d.iteritems():
            try:
                ...
                msg = 'Error occured while trying to parse file: %s' %dfile
                raise IOError, msg 
        return cls(ID, measurements, ID_to_position_mapper, **kwargs)

    @classmethod
    @doc_replacer
    def from_dir(cls, ID, path,
                 filename_to_ID_mapper='name',
                 ID_to_position_mapper=None, pattern='*.fcs', recursive=False,
                 readdata_kwargs={}, readmeta_kwargs={}, ID_kwargs={}, **kwargs):
        """
        Create a Collection of measurements from data files contained in a directory.
        ...
        return cls.from_files(ID, datafiles, filename_to_ID_mapper=filename_to_ID_mapper,
                              ID_to_position_mapper=ID_to_position_mapper,
                              readdata_kwargs=readdata_kwargs, readmeta_kwargs=readmeta_kwargs,
...
#             raise TypeError, 'Unsupported axis value %s' %axis
    ...
    @doc_replacer
    def _get_ID2position_mapper(self, ID_to_position_mapper):
        '''
        ...
        if hasattr(ID_to_position_mapper, '__call__'):
            mapper = ID_to_position_mapper
        elif isinstance(ID_to_position_mapper, collections.Mapping):
            mapper = lambda x: ID_to_position_mapper[x]
        elif ID_to_position_mapper == 'name':
            mapper = lambda x: (x[0], int(x[1:]))
        elif ID_to_position_mapper in ('enumeration_row_first', 'number'):
            mapper = lambda x : num_parser(x, 'F')
        elif ID_to_position_mapper == 'enumeration_col_first':
            mapper = lambda x : num_parser(x, 'C')
        else:
            msg = '"{}" is not a known key_to_position_parser.'.format(ID_to_position_mapper)
            raise ValueError(msg)
        return mapper

    def set_positions(self, positions=None, ID_to_position_mapper='name', ids=None):
        '''
        ...
        if positions is None:
            if ids is None:
                ...
                ids = to_list(ids)
            mapper = self._get_ID2position_mapper(ID_to_position_mapper)
