...
from okapi.tests.utils import TestCase, TestResponse
...
class RunnerTest(TestCase):
    ...
    def test_save_output(self):
        ...
        runner.render()
        runner.save(output)
        self.assertIn('<em>OK</em>', open(output + '/index.html').read())

    # region Test creating requests
    @patch('requests.request')
    def test_request_ok(self, request):
        request.return_value = TestResponse(status_code=200)
    ...
    def test_request_fail(self, request):
        request.return_value = TestResponse(status_code=404)
    ...
    def test_request_with_headers(self, request):
        request.return_value = TestResponse(status_code=201)
    ...
    def test_request_with_json_payload(self, request):
        ...
        self.assertIn(u'{&#34;foo&#34;:\xa0&#34;bar&#34;}', rendered)
    ...
    def test_request_post(self, request):
        request.return_value = TestResponse(status_code=204)
    ...
    def test_response_json(self, request):
        request.return_value = TestResponse(content='{"foo": "bar"}')
        ...
        self.assertIn(expected_output, rendered.encode('utf-8'))

    @patch('requests.request')
    def test_response_text(self, request):
        request.return_value = TestResponse(content='FOOBAR')
    ...
    def test_escape_html_in_response(self, request):
        request.return_value = TestResponse(content='<strong>FOO</strong>')
    ...
    def test_hidden_headers_are_not_in_html(self, request):
        request.return_value = TestResponse()
