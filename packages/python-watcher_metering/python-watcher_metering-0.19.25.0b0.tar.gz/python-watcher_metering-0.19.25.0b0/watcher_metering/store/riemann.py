...
class RiemannClient(MetricsStoreClientBase):
    ...
    def get_config_opts(cls):
        return super(RiemannClient, cls).get_config_opts() + [
            cfg.StrOpt(
                'store_endpoint', help="Riemann endpoint",
                sample_default="tcp://store-host:5555",
                default="tcp://127.0.0.1:5555", required=True),
            cfg.StrOpt(
                'default_metric_host',
                default='openstack',
    ...
    def create_event(self, metric):
        ...
        try:
            return RiemannClient.event_factory(event_dict)
        except TypeError as exc:
            LOG.exception(exc)
            raise MetricsStoreError(exc)
