...
* Enter your API key and choice of browser in CONFIG.txt (cygwin users should enter `cygwin` as their browser). An example .rc file is available in cliquery/CONFIG.txt. To find the install directory use cliquery -c. The API key field may be left blank if you did not sign up for a key.
