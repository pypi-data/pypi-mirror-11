...
class Mock(object):
    ...
    def verify(self, matcher):
        ...
        ...
        Exception: Found one matching call invoked from theMock :
          theMock('1stCall')
        > theMock('2ndCall')
          theMock('3rdCall')
        ...
        Traceback (most recent call last):
        ...
        Exception: Found one matching call invoked from theMock.theNewMock, but not in order :
    ...
    def verify_no_more_call_invoked(self):
        ...
        Traceback (most recent call last):
        ...
        Exception: Found one call invoked from theMock that was not verified :
        X theMock('1stCall')
          theMock('2ndCall')
        X theMock('3rdCall')
...
def do_verification(mock, matcher, expectation):
    ...
    if expectation(call_invocation_history.matching_call_invocations):
        ...
        if len(call_invocation_history.call_invocations) == 0:
            raise Exception('No call invoked from {mock_name}.'.format(
                mock_name=mock.name,
            ))

        if len(call_invocation_history.matching_indices) == 0:
            raise Exception('Found no matching call invoked from {mock_name} :'.format(
                mock_name=mock.name,

            ) + '\n' + print_call_invocation_history(call_invocation_history))

        if len(call_invocation_history.matching_indices) == 1:
            raise Exception('Found one matching call invoked from {mock_name} :'.format(
                mock_name=mock.name,

            ) + '\n' + print_call_invocation_history(call_invocation_history))

        if len(call_invocation_history.matching_indices) > 1:
            raise Exception('Found {nb_calls} matching calls invoked from {mock_name} :'.format(
...
def do_in_order_verification(mock, matcher, considered_mocks):
    ...
    if call_invocation_history.in_order_matching_indices:
        ...
        if len(call_invocation_history.matching_indices) == 1:
            raise Exception('Found one matching call invoked from {mock_name}, but not in order :'.format(
                mock_name=mock.name,

            ) + '\n' + print_in_order_call_invocation_history(call_invocation_history))

        if len(call_invocation_history.matching_indices) > 1:
            raise Exception('Found {nb_calls} matching calls invoked from {mock_name}, but not in order :'.format(
                nb_calls=len(call_invocation_history.matching_indices), mock_name=mock.name,
...
def do_no_more_verification(mock):
    ...
    if len(call_invocation_history.non_verified_indices) == 1:
        raise Exception('Found one call invoked from {mock_name} that was not verified :'.format(
            mock_name=mock.name,

        ) + '\n' + print_no_more_call_invocation_history(call_invocation_history))

    if len(call_invocation_history.non_verified_indices) > 1:
        raise Exception('Found {nb_calls} calls invoked from {mock_name} that was not verified :'.format(
