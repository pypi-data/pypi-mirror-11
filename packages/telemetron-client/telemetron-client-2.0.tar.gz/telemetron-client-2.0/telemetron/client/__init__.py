...
class Client(object):
    ...
    @param timestamp is the timestamp associated with the metric. If not set, will use current time (utc)
    """
    def time(self, name, value, tags={}, agg=['avg', 'p90', 'count', 'count_ps'],
    aggFreq=10, namespace=defaultNS, timestamp=None):
        tags.update({"unit": 'ms'})
        self.put(
            'timer.%s' % (name,),
            tags,
...
class AutoFlushClient(Client):
    ...
    @param flush_size is the buffer size at which a flush should be triggered.
    """
    ...
    def putRaw(self, line):
        super(AutoFlushClient, self).putRaw(line)
