...
class installation:
	...
	def set_json_value(self, nom, value):
	...
	def comptage_general_rec_function(self, date_time, energy, power):
		...
		self.set_json_value(self.nom_compteur_general, power)
		if self.compteur_general.type_horaire() == 'HC':
			self.set_json_value(self.nom_compteur_general + "_hc", self.compteur_general.counter_hc)
		else:
			self.set_json_value(self.nom_compteur_general + "_hp", self.compteur_general.counter_hp)
	...
	def enregistre_circuits_secondaires(self):
		...
		if not self.th_enregistre_circuits.terminated:			
			...
			for circuit in self.compteurs_secondaires:
				...
				self.db.add(circuit.nom, date_debut, date_fin, energie, moyenne,type_horaire)
				self.set_json_value(circuit.nom, moyenne)
				...
				for (Id, date_debut, date_fin, energie, puissance, type_horaire) in self.db.get_not_eclate_consos(circuit_mes.nom):
					...
					for circuit_enfant in possibilite[idx_solution]:
						...
						self.db.add(circuit_enfant.nom, \
									...
									puissance_enfant, \
									type_horaire)
						self.set_json_value(circuit_enfant.nom, puissance_enfant)
					...
					for circuit_enfant in circuit_mes.enfants:
						if circuit_enfant not in possibilite[idx_solution]:
							...
							self.db.add(circuit_enfant.nom, \
									...
									0, \
									type_horaire)
							self.set_json_value(circuit_enfant.nom, 0)
