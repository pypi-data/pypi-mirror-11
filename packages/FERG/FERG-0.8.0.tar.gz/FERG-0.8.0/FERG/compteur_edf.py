	...
	def record(self, rec_function):
		...
		if not self.terminated:
			...
			if self.type_horaire() == 'HC':
				self.counter_hc += _power/self.nb_tours_mesure
				logging.debug('Compteur HC = %s.' % self.counter_hc)
			else:
				self.counter_hp += _power/self.nb_tours_mesure
				logging.debug('Compteur HP = %s.' % self.counter_hp)
			# Enregistrement
			rec_function( \
