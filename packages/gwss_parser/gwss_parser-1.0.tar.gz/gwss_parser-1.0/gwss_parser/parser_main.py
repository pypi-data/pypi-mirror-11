		...
		for i in range(0, n):
			if f[i]:
				...
				if os.path.exists(filePath):
					...
					logger.info("File %s doesn't exist !" % filePath)
					continue
...
def check_for_update(ListConfigFile):	
	try:
		...
		if stdout.find("fail") == -1:
			...
			if need_restart:
				logger.debug("Parser needs restart soon")
				RUN_PARSER = False
			else:
			...
			logger.debug("Nginx conf fail, skip check update")
	except Exception, e:
		logger.error(e, exc_info=True)
...
def run(server_ip, LogAccess, LogPattern, ListConfigFile):
	...
	try:
		...
		# short summary
		logger.debug("======= ALL ACCESS FILES =======")
		logger.debug(LogAccess)
		logger.debug("Total access files: %d" % len(LogAccess))
		logger.debug("================================")
		logger.debug("Server IP: %s" % server_ip)
		logger.debug("Tailing access files started...")
		...
		logger.debug("PARSER runnning")
		while RUN_PARSER:
			# get next row and process, can be refactored more
			logger.debug("before get line")
			row = source.next()

			# parse row
			logger.debug("get line already")
			...
			try:
				...
				print "ROW: ", formatted_row
			except Exception, e:
				logger.error("Invalid row|Row %s|Parsed row %s|" % (row, parsed_row))
				continue
			...
			if formatted_row is not None:
				...
				if (current_time - last_call).seconds > 0:
					try:
						...
						logger.error(e, exc_info=True)
						logger.error("Possible Redis connection problem")
