...
def main():
	...
	logger = logging.getLogger("parser.start")
	...
	try:
		while 1: 
			...
			machine_id, LogAccess, LogPattern, ListConfigFile = parser_init.run()
			parser_main.run(machine_id, LogAccess, LogPattern, ListConfigFile)
			logger.info("There is a change in nginx config. Parser will restart")
			time.sleep(1)
