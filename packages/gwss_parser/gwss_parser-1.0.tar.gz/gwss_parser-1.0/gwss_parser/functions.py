...
logger = logging.getLogger("parser.functions")
...
def timer(msg):
	...
	global startTime
	logger.debug("%s%.5f", msg, currentTime - startTime)
...
def get_all_log_formats(ListConfigFile):
	...
	LogFormat = {}
	logger.debug(  "------- Attempt to extract log_format information -------" )
	...
	for each_file in ListConfigFile:
		...
		if new_log_format is not None:
			logger.debug("------- Log format in %s -------" % each_file ) 
			logger.debug("\t %s" % new_log_format)
	
	logger.debug("------- All Log Formats -------")
	for k in LogFormat:
		logger.debug("%s %s" % (k, LogFormat[k]))
...
def detect_all_config_file():
	...
	logger.debug( "------- Find main nginx conf path : -------")
...
def get_all_access_logs(ListConfigFile):
	...
	for each_file in ListConfigFile:
		with open(each_file) as f:
			config_str = f.read()
		logger.debug("------- Attempt to get server_name and access_log directives -------")
		logger.debug("Location %s: " % each_file)
		list_access_log = []
	...
	logger.debug("------- ALL LOG FILES INFO -------")
	logger.debug(all_access_log)	
...
def map_server_name_alias(all_access_logs):
	...
	logger.debug("Server name alias")
	logger.debug(server_name_alias)
