...
class TransfererClass(BaseClass):
	...
	def transferNetwork(self):
		...
		if self.StationarizingRateVariable!=None:
	...
	def viewNetwork(self):
		...
		#Check 
		if self.ViewingColorStrsList==None or len(self.ViewingColorStrsList)!=self.StationarizingUnitsInt:
		...
		#Check
		if self.ViewingLabelStrsList==None or len(self.ViewingLabelStrsList)!=self.StationarizingUnitsInt:
		...
		self.debug(
			[
				"len is "+str(len(self.TeamDict['Populations'].ManagementDict))
			]
		)
