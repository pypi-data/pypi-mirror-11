...
def run_unsafe_python(example: Example, src: str, *,
    ...
    try:
        ...
        partial_response = Example(interaction, validate=False)

        # Execution exceeded timeout
        if isinstance(ex, TimeoutError):
            ex = errors.TimeExceededError()
            out = Example(interaction, error=ex)

        # Program asked for a non-existing input
        elif isinstance(ex, errors.MissingInputError):
            ex = errors.MissingInputError(example, partial_response)
            out = Example(interaction, error=ex, validate=False)

        # Bad syntax in code
        elif isinstance(ex, SyntaxError):
            tb = ex.__traceback__
            ex = errors.BuildError(example, src, ex.with_traceback(tb))
            out = Example(interaction, error=ex, validate=False)

        # Some generic runtime error happened
        else:
            tb = ex.__traceback__
            ex = errors.UnexpectedError(example, ex.with_traceback(tb))
            out = Example(interaction, error=ex, validate=False)
    ...
    # Execution terminated
        ...
        if inputs:
            partial_response = Example(interaction, validate=False)
            ex = errors.UnusedInputError(example, partial_response, inputs)
            out = Example(interaction, error=ex, validate=False)
        else:
