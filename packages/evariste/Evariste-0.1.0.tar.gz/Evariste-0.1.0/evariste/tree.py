...
from evariste.action import DirectoryAction
...
class Tree:
    ...
    @methodhooks(name="buildtreeitem", getter=get_tree_plugins)
    ...
    def pprint(self, prefix="", *, last=True, print_function=print, report=False, color=False):
        ...
        if report:
            if self.report.success:
                ...
                if color and self.is_dir():
                    reportstr = "\033[93mfail\033[0m"
                elif color and not self.is_dir():
        ...
        line.append(str(self.relativename))
...
class File(Tree):
    ...
    def compile(self, *, to_prune):
        ...
        with self.shared.builder.lock.thread_unsafe():
            ...
            try:
                if not self.shared.builder.plugins.get_plugin('misc.changed').compile(self):
            ...
            with self.shared.builder.plugins.contexthook("compilefile", args=[self]):
    ...
    def make_archive(self, destdir):
        ...
        if (
                ...
                and not self.shared.builder.plugins.get_plugin('misc.changed').compile(self)
                # Archive has already been generated
                and 'archivepath' in self.shared.tree[self.from_source]['misc.changed']
            ):
            return self.shared.tree[self.from_source]['misc.changed']['archivepath']
