...
class Setup:
    ...
    def pprint(self, *, print_function=print):
        """Pretty print of the object."""
        for section in sorted(self.keys()):
            print_function("'{}':".format(section))
            for option in sorted(self[section].keys()):
                print_function("  '{}': {}".format(option, self[section][option]))
