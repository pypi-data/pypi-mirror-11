...
from evariste.action.command import Command, CommandCompiler
...
class LatexCompiler(CommandCompiler):
    ...
    def _read_config(self):
        ...
        postcommands = []
        commands = []
        if self.plugin_id in self.path.config:
            for option in self.path.config[self.plugin_id]:
                if option.startswith("pre"):
                    precommands.append("!{}".format(self.path.config[self.plugin_id][option]))
                elif option.startswith("cmd"):
                    commands.append(self.path.config[self.plugin_id][option])
                elif option.startswith("post"):
                    postcommands.append("!{}".format(self.path.config[self.plugin_id][option]))
        if not commands:
        ...
        # Reading binaries
        for key in self.bin:
            if self.path.config[self.plugin_id][key] is not None:
                if self.path.config[self.plugin_id][key] is not None:
                    self.bin[key] = self.path.config[self.plugin_id][key]

        # Setting target
        if not self.path.config[self.plugin_id]['target'] is not None:
            basetarget = self.path.format(self.TARGET[self.bin['latex']])
        else:
            basetarget = self.path.format(self.path.config[self.plugin_id]["target"])
        self._report.target = (
            self.path.parent.from_source
            /
            pathlib.Path(basetarget)
            )
    ...
    def _conditionnal_command_bibtex(self):
        ...
        """
        if self.path.config[self.plugin_id]['bibtex'] is not None:
            self.latex_todo += 1
    ...
    def _conditionnal_command_makeindex(self):
        ...
        It might be detected, some day, by looking at the LaTeX log.
        """
        if self.path.config[self.plugin_id]['makeindex'] is not None:
            self.latex_todo += 1
