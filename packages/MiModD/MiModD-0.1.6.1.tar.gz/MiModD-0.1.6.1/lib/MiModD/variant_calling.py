...
def get_coverage_from_vcf (inputfile, ofile = None):
    ...
    try:
        with pyvcf.open(inputfile, 'rb') as raw_vcf:
            # copy comment and header lines over to post-processed vcf file
            # determine the number of samples and their names
            sample_names = raw_vcf.info.sample_names
            ...
            # no chromosome dealt with so far
            last_chr_covered = ''

            for cov_info in raw_vcf.field_iter('DP'):
                if last_chr_covered != cov_info[0]:
                    if last_chr_covered != '':
                        cov_mean = [str(c // contig_lengths[last_chr_covered]) for c in cov_sum]
                        out_stats.write('{0}\t{1}\n'.format(last_chr_covered, '\t'.join(cov_mean)))
                    last_chr_covered = cov_info[0]
                    cov_sum = [0 for sample in sample_names]
                for i, coverage in enumerate(cov_info[2]):
                    cov_sum[i] += int(coverage)
    finally:
