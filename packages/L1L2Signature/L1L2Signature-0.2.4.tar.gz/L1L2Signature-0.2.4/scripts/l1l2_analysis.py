...
def main(result_dir, show):
    ...
    if len(np.unique(labels)) == 2: # Classification
        ...
        print #--------------------------------------------------------------------
        print('Extracting (by signatures) and ordering (by frequencies) submatrices...')
        ...
        ### Assume it's a regression task, write r2 score
        print #--------------------------------------------------------------------
        print('R2 score computation...')
        print('  * Reading pickled inputs...')
        ...
        print('  * Calculating predicted target values over splits...')
        pred_labels = np.zeros((len(mu_range), len(labels))) # for each mu
        real_labels = np.empty((len(labels),))
        
        r2_scores = np.zeros((len(mu_range), len(outputs)))
        
        k = 0
        for inp, out in zip(inputs, outputs):
            ts_idxs = inp[6] # External split indexes (ts)
            tr_idxs = inp[5] # External split indexes (tr)
            
            ### Read beta for each value of mu and compute predictions on the test set
            betas = list()
            predictions = list()
            for m in xrange(len(mu_range)):
                idx = out['selected_list'][m].nonzero()[0] ### indexes of selected features
            
                ### Select only the test set, and only those columns corresponding to selected features
                X_ts = br.data[ts_idxs, :]
                X_ts = X_ts[:, idx]
            
                ### Extract beta, compute predictions and save R2 score
                beta = out['beta_list'][m]
                Y_lr = X_ts.dot(beta)
                sc = r2_score(br.labels[ts_idxs], Y_lr)
                
                betas.append(beta)
                predictions.append(Y_lr)
                r2_scores[m,k] = sc
            
            k += 1

        print('  * Saving regression performances measures...')
        
        for m in xrange(len(mu_range)):
            
            row = "R2 score summmary for mu%d:" % m
            print(row)
            sw._writeline(row)
            
            row = "R2 scores from outer iterations:"
            print(row)
            sw._writeline(row)
            
            print(r2_scores[m,:])
            sw._writeline(r2_scores[m,:])
            
            row = "Average R2 score: %f" % r2_scores[m,:].mean()
            print(row)
            sw._writeline(row)
