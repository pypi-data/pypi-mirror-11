...
class DeviceConnection(object):
    ...
    def _send_command_and_return_response(self, command, suppress=False, timeout=None, end_condition_regex=None):
        """
        ...
        if suppress:
            ...
            logger.debug('Received output {0}'.format(response))

        return response

    def _send_command_and_error_on_unexpected_output(self, command, expected_output_regex=None, timeout=None, suppress=False):
        response = self._send_command_and_return_response(command, end_condition_regex=expected_output_regex, timeout=timeout, suppress=suppress)
        if expected_output_regex:
            ...
            if not re.search(sanitized_regex, sanitized_response):
                if suppress:
                    ...
                    raise RuntimeError("Did not receive expected output '{0}'. Actual output: {1}".format(expected_output_regex, response))
        return response
