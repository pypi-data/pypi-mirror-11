...
from luna_manipulator_utils import *
import re
...
class LunaManipulatingAdapter(LunaAdapter):
    ...
    def add_network_interface(self, network_interface, hsm_ip_address, netmask, gateway_ip=''):
        ...
        return_value = self.send_command_and_check_output(command, generic_success_output, timeout=120)
    ...
    def ping(self, ip_address):
        ...
        return_value = self.send_command_and_check_output(command, luna_shell_prompt, timeout=30)
    ...
    def create_partition(self, partition_label, partition_password, cloning_domain):
        ...
        self.send_command_and_check_output(command, expected_create_partition_output, suppress=True)

    def delete_partition(self, partition_label):
        command = "partition delete -partition {0} -f".format(partition_label)
        self.send_command_and_check_output(command, expected_delete_partition_output, suppress=True)
    ...
    def remove_ntp_host(self, host):
        ...
        self.send_command_and_check_output(command,expected_ntp_host_removed_output, timeout=30, suppress=False)
    ...
    def remove_syslog_host(self, host):
        ...
        self.send_command_and_check_output(command, generic_success_regex, timeout=30, suppress=False)
