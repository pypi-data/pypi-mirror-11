...
from luna_manipulator.luna_manipulator_utils import generic_success_regex
import re
import logging
...
class LunaReadingAdapter(LunaAdapter):
    ...
    def get_interface_info(self):
        ...
        raw_output = self.send_command("status interface")
        tokenized_output = self._tokenize_string(raw_output)
    ...
    def get_syslog_servers(self):
        ...
        raw_output = self.send_command("syslog remotehost list", timeout=30)
        tokenized_output = self._tokenize_string(raw_output)
    ...
    def get_ntp_servers(self):
        ...
        #Get the remote server list
        raw_output = self.send_command("sysconf ntp listservers", timeout=30)
        tokenized_output = self._tokenize_string(raw_output)
    ...
    def get_ntp_configuration_and_status(self):
        ...
        raw_output = self.send_command("sysconf ntp status", timeout=30)
    ...
    def get_snmp_configuration_and_status(self):
        ...
        #Grab the running/enabled status
        raw_output = self.send_command("sysconf snmp show")
        if "not running" in raw_output:
        ...
        #Grab the user list
        raw_output = self.send_command("sysconf snmp user list")
        tokenized_output = self._tokenize_string(raw_output)
        ...
        raw_output = self.send_command("sysconf snmp notification list")
        tokenized_output = self._tokenize_string(raw_output)
    ...
    def get_syslog_configuration_and_status(self):
        ...
        raw_output = self.send_command("service status syslog")
        if "not running" in raw_output:
            syslog_info['running'] = False

        #Grab the list of remote hosts.
        raw_output = self.send_command("syslog remotehost list")
        tokenized_output = self._tokenize_string(raw_output)
        #The list of remote hosts has one tokens per line, so we can dump all lines with more tokens than that.
        sanitized_output = self._remove_rows_without_correct_number_of_tokens(tokenized_output, 1)
        for line in sanitized_output:
    ...
    def get_users(self):
        ...
        raw_output = self.send_command("user list")
        tokenized_output = self._tokenize_string(raw_output)
    ...
    def is_package_installed(self, package_name):
        ...
        raw_output = self.send_command("package list")
    ...
    def get_clients(self):
        ...
        raw_output = self.send_command("client list")
        tokenized_output = self._tokenize_string(raw_output)
    ...
    def get_partitions(self):
        ...
        raw_output = self.send_command("partition list")
    ...
    def get_client_partitions(self, client_label):
        ...
        raw_output = self.send_command("client show -c {0}".format(client_label))
        tokenized_output = self._tokenize_string(raw_output)
        for line in tokenized_output:
    ...
    def get_client_fingerprint(self, client_label):
        ...
        raw_output = self.send_command("client fingerprint -c {0}".format(client_label))
        tokenized_output = self._tokenize_string(raw_output)
        for line in tokenized_output:
