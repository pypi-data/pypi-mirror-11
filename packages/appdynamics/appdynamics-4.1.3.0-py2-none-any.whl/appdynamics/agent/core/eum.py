...
def has_entry_point_errors_key(use_long_form):
    return 'hasEntryPointError' if use_long_form else 'h'
...
def generate_eum_metadata(bt):
    ...
    metadata[guid_key(use_long_form)] = str(uuid4())
...
def inject_eum_metadata(bt, response_headers):
    ...
    """
    if eum_metadata_injection_enabled(bt.request.user_agent):
        metadata = generate_eum_metadata(bt)
