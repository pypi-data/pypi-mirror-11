...
def make_virtual_bindeps_package():
    ...
    if not os.path.exists(tombstone):
        # This gets really weird because we could be racing other processes trying to build the virtual package.
        # Even if we know we are losing the race, it's better to keep racing to ensure that at the end, the virtual
        # package is completely built.
        try:
            try:
                os.makedirs(dest)
            except:
