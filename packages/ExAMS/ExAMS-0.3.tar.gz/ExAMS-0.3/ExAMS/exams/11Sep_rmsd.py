...
#def rmsd_avstr(mycofasu, uref, output='rmsd_avstr_output.dat', select='all'):	
#
...
#          for i in range(reps):
#                #if(k<universesArray[i].trajectory.numframes): 
...
#                      MDAnalysis.analysis.align.alignto(mycofasu.fasulist[i].u, uref, select=select)
#                      currentAtoms = mycofasu.fasulist[i].u.selectAtoms(select)
#                      structureAvg += currentAtoms.coordinates()
#                      # Here, at this point we should advance all universes by one timestep
#                      if(k<mycofasu.fasulist[i].numframes()-1):
#                            #mycofasu.fasulist[i].u.trajectory.next()
#                            mycofasu.fasulist[i].u.trajectory[k]
    ...
    for k in range(num_snapshots):
          ...
          for i in range(reps):
                if(k<mycofasu.fasulist[i].numframes()): 
                      ...
                      structureAvg += mycofasu.fasulist[i].coords(k)
          structureAvg=structureAvg/numReplicas
          RMSDAvg[k]=MDAnalysis.analysis.rms.rmsd(ref0,structureAvg)
