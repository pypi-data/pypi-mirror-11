...
class WishlistApi(object):
    ...
    def show_wishlist(self, id):
        url = self.base_url + "wishlists/%s" % id
        contents = self.get(url)
        return contents

    def create_wishlist_item(self, id, item):
        #@todo: rename method create_item_wishlist
        url = self.base_url + "wishlists/%s/items" % id
        contents = self.post(url, data_j=item)
        return contents

    def update_wishlist_item(self, id, item_id, item_update):
        #@todo rename method update_item_wishlist
        url = self.base_url + "wishlists/%s/items/%s" % (id, item_id)
        contents = self.patch(url, data_j=item_update)
        return contents

    def delete_wishlist_item(self, id, item_id):
        #@todo: rename method delete_item_wishlist
        url = self.base_url + "wishlists/%s/items/%s" % (id, item_id)
