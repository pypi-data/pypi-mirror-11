...
class PaymentApi(object):
    ...
    def list_payment_methods(self):
        #@todo change this method name: list methods payment
        url = self.base_url + "payments/methods"
        contents = self.get(url)
        return contents

    def create_payment(self, id, payment):
        url = self.base_url + "payments/%s" % id
        contents = self.put(url, data_j = payment)
        return contents

    def confirm_payment(self, id, confirmation):
        url = self.base_url + "payments/%s" % id
        contents = self.patch(url, data_j = confirmation)
        return contents
