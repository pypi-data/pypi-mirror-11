...
```
''' % dict(
            method=smart_str(self.method),
            request_uri=smart_str(request_uri),
            post_params=smart_str(pprint.pformat(json.loads(self.post_json), width=1)),
            cookie_params=smart_str(pprint.pformat(json.loads(self.cookies_json), width=1)),
            meta_params=smart_str(pprint.pformat(json.loads(self.meta_json), width=1))
        )
...
Logged at: %(logged_at)s''' % dict(
            filename=smart_str(self.signature.file_name),
            line_no=self.signature.line_number,
            line=self.signature.line,
            stack_trace=smart_str(self.stack_trace),
            logged_at=self.logged_at,
            local_variables=smart_str(self.local_variables),
            request=smart_str(self.request.to_markdown())
        )
