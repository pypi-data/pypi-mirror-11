    ...
    def run(self):
        ...
        while 1:
            try:
                ...
                logging.error("Connection", self.name, " did not start with", u)
            else:
                break
        ...
        while self.length > 0:
            ...
            try:
                ...
                if len(data_block) == 0:
                    logging.info("Connection %s: [TESTING]: 0 sized block fetched." % self.name)
                if len(data_block) != fetch_size:
                    logging.info("Connection %s: len(data_block) != fetch_size, but continuing anyway." % self.name)
                    self.run()
                    return

            except socket.timeout as s:
                logging.error("Connection {} timed out with {}".format(self.name, s))
                self.run()
                return
...
def download(url, options):
    ...
    try:
        ...
        logging.info("Need to fetch {}".format(report_bytes(conn_state.filesize - sum(conn_state.progress))))
