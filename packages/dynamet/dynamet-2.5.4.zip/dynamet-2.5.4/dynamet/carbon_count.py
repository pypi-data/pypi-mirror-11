...
def estimate_num_c_nl(table, integration=False, lin_abs_error=0.03):
    ...
    num_c=dict()
#    def max_num_c(m0, m1, lb_error=lin_abs_error*0.3, ub_error=lin_abs_error):
...
def _ident_common_features(ref_m0, t_idms, isol_width=3*emzed.MMU, rttol=10):
    ...
    """
    rttol=helper.avoid_zero_value(rttol)
    isol_width=helper.avoid_zero_value(isol_width)
...
#        & t_idms.rt.inRange(rtmin, rtmax) & ((t_idms.z==ref_m0.z) | (t_idms.z==0)))
    common=t_idms.join(ref_m0, t_idms.mz.equals(ref_m0.mz, abs_tol=isol_width)\
        & t_idms.rt.equals(ref_m0.rt, abs_tol=rttol) &\
        ((t_idms.z.equals(ref_m0.z, abs_tol=None)) | (t_idms.z==0)))
    ...
    common.addColumn('fid', common.feature_id__0, type_=int)
    replace=['z'] # since z might be 0  in t_idms
    postfix=common.supportedPostfixes(replace)[-1]
    for rep in replace:
        common.replaceColumn(rep, common.getColumn(rep+postfix), type_=int)
...
def potential_idms_pairs(t_m0, t_ul, rttol=5, mztol=0.8*MMU):   
    ...
    z_crit = ((t_m0.z.equals(t_ul.z, abs_tol=None)) | (t_ul.z==0))
    rt_crit = (t_m0.rt.equals(t_ul.rt, abs_tol=rttol))
    m0mul=t_m0.join(t_ul, rt_crit &z_crit)
    m0mul.addColumn('temp',(m0mul.mz__0-m0mul.mz)*m0mul.z*1.0, type_=float, format_='%.5f')
    m0mul=m0mul.filter(m0mul.temp>=(delta_c-0.5*emzed.MMU))# at least 1 carbon atom
    m0mul.addColumn('temp1',m0mul.temp.apply(lambda v: abs(v-round(v)*delta_c)), type_=float,
                    format_='%.5f')
    ...
    m0mul.replaceColumn('feature_id', m0mul.fid, type_=int)
    ...
    for f in features:
        ...
        def _fun(v, pairs=values):
            for pair in pairs:
                if v==pair[0]:
                    return pair[1]
        f.addColumn('grouped',f.c_distance.apply(_fun), type_=int)
        subs=f.splitBy('grouped')
        for sub in subs:
            sub.addColumn('mi_frac', sub.area__0/sub.area__0.sum(), type_=float)
            expected=_expected_pattern(sub.c_distance.values, p=p_c_source)
            sub.addColumn('mi_expected', expected, type_=float)
...
def _score_n_c_values(t):
    ...
    t.updateColumn('_is_score', (same_value & not_none).thenElse(t._is_score+1, t._is_score), 
                   type_=int)
    t.updateColumn('_is_score', (t.score_is==3).thenElse(t._is_score+2, t._is_score), type_=int)
