...
def  enhanced_integrate(t, step=1, fwhm=None):
    if not fwhm:
        ...
        if not t.hasColumn('fwhm'):        
            t.addColumn('fwhm', fwhm, type_=float)
    ...
    emg1.addColumn('_diff', diff, type_=float)
    emg2.addColumn('_diff', diff, type_=float)
...
def special_integrate(t, a=1):
    t1=t.copy()
    t1.replaceColumn('rtmin', t1.rt-a*t1.fwhm, format_="'%.2fm' %(o/60.0)", type_=float)
    t1.replaceColumn('rtmax', t1.rt+a*t1.fwhm, format_="'%.2fm' %(o/60.0)", type_=float) 
    n_cpus=get_n_cpus(t)
