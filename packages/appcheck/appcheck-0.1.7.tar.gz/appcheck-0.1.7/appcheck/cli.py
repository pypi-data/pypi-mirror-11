...
def _print_result(appcheck, id_or_sha1, json_output, wait=True):
    ...
    while True:
        try:
            ...
            if wait and res['status'] == Appcheck.STATUS_BUSY:
                if not wait_printed:
                    wait_printed = True
    ...
    if not res['status'] == Appcheck.STATUS_READY:
...
def scan(appcheck, file, group, background):
    ...
    for i, f in enumerate(file):
        ...
        if res['results']['status'] == Appcheck.STATUS_READY:
...
def login():
    ...
    config = AppcheckClientConfig()
    if click.confirm("Use AppCheck managed service https://appcheck.codenomicon.com/?"):
        config.set_appcheck_host(DEFAULT_APPCHECK_HOST)
    else:
        host = click.prompt("Enter AppCheck URI (https://YOUR-APPLIANCE/)")
