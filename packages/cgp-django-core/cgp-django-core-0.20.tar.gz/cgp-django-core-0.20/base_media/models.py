...
def _media_file_name( instance, filename, file_attribute_name, folder, model_name ):
    ...
    if exists:
        ...
        #If anohter object in this model is using that path, then don't allow this instance to overrride it
        kwargs = {
        ...
        if exists_in_separate_object==False and instance.allow_file_to_override==True:
...
def secure_document_file_name( instance, filename ):
    
    folder = '/'.join( [ 'media', 'document' ] )
    full_path = _media_file_name(instance, filename, 'media_file', folder, settings.SECURE_DOCUMENT_MODEL)
    return full_path    


def get_secure_document_storage():
...
class BaseMedia( models.Model ):
    ...
    help = {
        'title': 'If empty, the filename will be used to determine the title.',
        'is_searchable': 'Allow this file to be indexed and searchable by the public.',
        'clean_filename_on_upload': "This removes spaces, special characters, and capitalization from the file name for more consistent naming.",
        'allow_file_to_override': "This will only replace a file of the same name if it is apart of this same image. If another image uses the same filename (e.g. logo.png), then this image will not replace that file.",
        'admin_description': "Add administrative description to help others find and use this element.",
        'size':"File size in bytes."
    }

       
    title = models.CharField(_("title"), max_length=255, help_text=help['title'], blank=True, null=True)

    try:
        is_searchable = models.BooleanField( _("Is Searchable"), 
            default = settings.MEDIA_SEARCHABLE_BY_DEFAULT, help_text=help['is_searchable'])
    except:
        is_searchable = models.BooleanField( _("Is Searchable"), 
            default = False, help_text=help['is_searchable'])

    clean_filename_on_upload = models.BooleanField( _("Clean filename on upload"), 
        default = True, help_text=help['clean_filename_on_upload'])

    allow_file_to_override = models.BooleanField( _("Allow file to override existing file with the same name"), 
        default = True, help_text=help['allow_file_to_override'])

    legacy_url = models.CharField(_("Legacy URL"), max_length=255, 
        blank=True, db_index=True)    

    # -- Meta Data
    admin_description = models.TextField(_("Administrative Description"), 
        blank = True, help_text=help['admin_description'] )
    
    size = models.BigIntegerField(null=True, blank=True, 
        help_text=help['size'])
    display_size = models.CharField(_("Display Size"), max_length=255, 
        blank=True, null=True)   
    ...
    users = models.ManyToManyField(settings.AUTH_USER_MODEL, 
        verbose_name=_(u"Users"), blank=True)
    ...
    tags = models.ManyToManyField('media.MediaTag', blank=True, 
...
class BaseImage( BaseMedia ):
    ...
    help = {
        'credit':"This field is helpful if a license requires that an author attribution be displayed.",
        'caption':"",
        ...
        'alt': "This field is very important for SEO and accessibility. If not specified, the image caption or title will be used.",
        'use_png': "PNG files are much larger but better in some cases, such as for vector images."
    }

    image = models.ImageField(upload_to=image_title_file_name, help_text=help['image'])
...
class BaseSecureDocument( BaseMedia ):
    ...
    image = models.ForeignKey(settings.IMAGE_MODEL, null=True, blank=True, 
        related_name="%(app_label)s_%(class)s_related", on_delete=models.SET_NULL) 
    try:
        media_file = models.FileField(upload_to=secure_document_file_name, blank=True, 
            help_text="Documents, i.e. PDFs or Word docs",storage=get_secure_document_storage())
    except:
        media_file = models.FileField(upload_to=secure_document_file_name, blank=True, 
            help_text="Documents, i.e. PDFs or Word docs")
...
def remove_document_file_from_s3(sender, instance, using, **kwargs):
    ...
    model = get_model(settings.DOCUMENT_MODEL.split('.')[0], settings.DOCUMENT_MODEL.split('.')[1])
