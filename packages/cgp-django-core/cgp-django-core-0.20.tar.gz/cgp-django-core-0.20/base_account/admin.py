...
class BaseGroupMemberInline(admin.TabularInline):
    verbose_name = "Group Membership"
    verbose_name_plural = "Group Memberships"

    def edit_group_link(self, obj):
        if obj.group:
            object_type = type(obj.group).__name__            
            url = reverse('admin:%s_%s_change' %(obj.group._meta.app_label,  obj.group._meta.model_name),  args=[obj.group.id] )
            return u"<a href='%s'>Edit Group ></a>"%(url)
        return ''
    edit_group_link.allow_tags = True

    autocomplete_lookup_fields = {
        'fk': ['group',],
    }
    raw_id_fields = ('group', )
    readonly_fields = ('edit_group_link',)
