...
def user_passes_test(test_func, cache_duration=settings.CACHE_DURATION):
    """
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if len(messages) > 0 or test_func(request.user):
                #if user logged in, don't return cached view
                #print "not cached"
                return view_func(request, *args, **kwargs)

            else:
                #if user is not logged in, return cached veiw
                #print "return cached page"
