...
from hpelm.modules import make_hdf5, ireader
...
class HPELM(SLFN):
    ...
    def train(self, X, T, *args, **kwargs):
        ...
        self.async = False  # asyncronous I/O with HDF5
    ...
    def predict(self, fX, fY, async_io=False):
        ...
        # start async reader for HDF5 files
        if async_io:
            q_in = mp.Queue()
            q_out = mp.Queue(1)
            reader = mp.Process(target=ireader, args=(fX, q_in, q_out))
            reader.daemon = True
            reader.start()

        Y = self._makeh5(fY, N, self.targets)
        ...
        for b in xrange(1, nb+1):
            ...
            if async_io:
                Xb = q_out.get().astype(np.float64)
