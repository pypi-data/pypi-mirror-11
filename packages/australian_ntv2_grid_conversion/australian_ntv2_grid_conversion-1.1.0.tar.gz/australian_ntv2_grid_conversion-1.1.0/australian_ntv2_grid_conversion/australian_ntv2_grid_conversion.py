...
class Ntv2:
    ...
    def _read_subgrid_header(self, f):
        ...
        return {
            ...
            'LONG_INC': long_inc[1],
            'GS_COUNT': gs_count[1],
            'data_offset': f.seek(0, 1)  # add value for the offset to the start of this subgrid's values for easier access later
        }
    ...
    def convert_csv_file(self, from_system, to_system, input_file_path, output_file_path=None, output_format='', suppress_accuracy=False):
        ...
        row_number = 1
        try:
            with open(input_file_path, newline='') as csv_file:
                ...
                for row in reader:
                    ...
                    try:
                        if data_type == 'latlon':
                                data.append({'latitude': float(row['latitude']),
                                             'longitude': float(row['longitude'])})
                        elif data_type == 'grid':
                                data.append({'easting': float(row['easting']),
        ...
        try:
            with open(output_file_path, 'w', newline='') as output_file:
                if data_type == 'latlon':
                    if suppress_accuracy:
                        fieldnames = ['latitude', 'longitude']
                    else:
                        fieldnames = ['latitude', 'longitude', 'latitude transformation accuracy (m)', 'longitude transformation accuracy (m)']
                elif data_type == 'grid':
                    if suppress_accuracy:
                        fieldnames = ['easting', 'northing', 'zone']
                    else:
                        fieldnames = ['easting', 'northing', 'zone', 'latitude transformation accuracy (m)', 'longitude transformation accuracy (m)']
                else:
                    print('Invalid data_type')
                    sys.exit(1)
                ...
                for pt in data:
                    ...
                    if output_format == 'grid':
                        if suppress_accuracy:
                            writer.writerow({'easting': new_pt['easting'],
                            ...
                            writer.writerow({'easting': new_pt['easting'],
                                             'northing': new_pt['northing'],
                                             'zone': new_pt['zone'],
                                             'latitude transformation accuracy (m)': new_pt['lat_trans_acc'],
                                             'longitude transformation accuracy (m)': new_pt['lon_trans_acc']})
                    if output_format == 'latlon':
                        if suppress_accuracy:
                            writer.writerow({'latitude': new_pt['latitude'],
