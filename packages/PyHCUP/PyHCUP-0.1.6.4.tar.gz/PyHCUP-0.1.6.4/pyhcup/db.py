...
import parser
from .config import MISSING_PATTERNS, LONG_TABLE_COLUMNS
...
def gen_table(meta, table, schema=None, append_state=True,
    ...
    table = Table(table, MetaData(), *column_clauses)
    return table
...
def gen_long_table(table_name, category):
    """
    Generates a SQLAlchemy Table object for a long table of the
    specified category.
    
    Does not actually create the table in the database.
    ...
    elif category not in LONG_TABLE_COLUMNS.keys():
        raise Exception("At present only long {lt} are supported. (Got {c})"\
            .format(lt=', '.join([x for x in LONG_TABLE_COLUMNS.keys()]),
                    c=category))

    columns = [Column(*c) for c in LONG_TABLE_COLUMNS[category]]
...
def load_raw(eng, handle, dummy_separator='\v', table_name=None):
    ...
    if table_name is None:
        ...
        table_name = 'raw_%s_%s' % (base_filename, timestamp)
...
def process_raw_table(eng, table, meta_df, state, year,
    ...
    if table_name is None:
        ...
        if 'raw' in raw_table_name:
            table_name = raw_table_name.replace('raw', 'loaded')
        else:
            table_name = 'loaded_' + raw_table_name
...
def shovel(eng, tbl_source, tbl_destination, fields_in, fields_out=None,
              where_not_null=[], preserve_source=True, scalars=None):
    """
    ...
    #print "fields_in: %s" % fields_in
    #print "fields_out: %s" % fields_out
    
    source = Table(tbl_source.lower(), MetaData(), autoload_with=eng)
    #print "source: %s" % source
    destination = Table(tbl_destination.lower(), MetaData(), autoload_with=eng)
    ...
    if scalars is not None:
        ...
        try:
            ...
            insert_fields.extend([str(v) for k, v in scalars.iteritems()])
        except:
    ...
    sel = select(select_fields).select_from(source)
    #print "sel: %s" % sel
    ...
    #print "tbl_destination: %s" % tbl_destination
    stmt = destination.insert().from_select(insert_fields, sel)
    #print "stmt: %s" % stmt
    r = eng.execute(stmt)
    #print r
...
def match_col_iexact(name, table):
    '''
    Seeks a case-insensitive match for column name
    in table.
    '''
    m = [c for c in table.c if c.name.lower() == name.lower()]
    if len(m) == 1:
        return m[0]
    else:
        return None
...
def shovel_sqla(source, destination, fields_in,
                fields_out=None, where_not_null=[],
                preserve_source=True, scalars=None):
    """
    Generates a SQLAlchemy object for moving fields_in
    from source to destination. Expects source and
    destination to be SQLAlchemy Table objects.

    fields_out must either be None or a list with length
    len(fields_in). These will be used as "SELECT {field_in}
    AS {field_out}" to map values to differently named columns.

    If scalars is not None, must be a dictionary with
    key->value pairs to pass as constants for everything
    shoveled by this function.

    TODO: shall we drop scalars?
    """

    if fields_out is not None:
        assert len(fields_out) == len(fields_in), \
            "If provided, fields_out must have the same length as fields_in."
        select_fields = [source.c[i[0]].label(i[1])
                        for i in zip(fields_in, fields_out)]
        insert_fields = [destination.c[i[1]]
                        for i in zip(fields_in, fields_out)]
    else:
        select_fields = [source.c[i[0]] for i in fields_in]
        insert_fields = fields_in
    ...
    if scalars is not None:
        assert isinstance(scalars, dict), \
            "If provided, scalars must be a simple key->value dictionary."
        try:
            select_fields.extend([column(v).label(k)
                                for k, v in scalars.iteritems()])
            insert_fields.extend([v
                                for k, v in scalars.iteritems()])
        except:
            raise Exception("Unable to tack on select_fields and insert_fields for scalars. Does scalars look like a simple key->value map? (Got %s)" % scalars)
    ...
    for wnn in where_not_null:
        sel = sel.where(source.c[wnn] != None)
...
def dte_load(eng, handle, table_name=None):
    """
    ...
    Returns SQLAlchemy Table object representing the newly created table.
    ...
    if table_name is None:
        ...
        table_name = '%s_%s' % (base_filename, timestamp)
