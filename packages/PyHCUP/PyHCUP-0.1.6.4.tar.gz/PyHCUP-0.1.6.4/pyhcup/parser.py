...
class UtilizationFlagger:
    def __init__(self, uflag_name, revcodes=None, chg_group=None, pr=None, prccs=None):
        """Object holding a definition for a utilization flag. Can
        also apply the definition to a set of charges and procedures
        data in order to determine the appropriate value for the
        utilization flag. Must specify at least one of revcodes, pr,
        and prccs.
        
        Parameters
        revcodes: list (optional)
            Contains a list of UB92/UB04 revenue codes associated
            with the utilitzation flag.
        
        chg_group: list (optional)
            Contains a list of charge group numbers, for state-years
            with charge buckets rather than per-revenue code charges.
            These will correspond to the n in CHGn in the source files.
            
            WARNING: If revcodes is set, chg_group is ignored.
        
        pr: list (optional)
            Contains a list of ICD-9-CM procedure codes associated
            with the utilitzation flag.
        
        prccs: list (optional)
            Contains a list of Clinical Classification Software (CCS)
            procedure codes associated with the utilization flag.
        """
    ...
    def sql(self, tbl_out, tbl_core, tbl_pr, tbl_charges, state='', year='NULL'):
        """
        Generates a SQL statement for generating values for this
        u_flag within a database.
        
        If provided, state should be a string and year should be
        an integer or the string literal 'NULL'.
        """
        sel_cols = ['t_core."KEY"']
        ...
        if self._revcodes is not None:
            ...
            rev_clause = """
            CASE WHEN (
                SUM(CASE WHEN t_revcode."GROUP_NUMBER" ({cs_chg_group}) AND t_revcode."CHARGE" > 0 THEN 1 ELSE 0 END)
                ) > 0
                THEN true
                ELSE false
                END AS has_revcode
        ...
        full_sql = """
            ...
            INSERT INTO {tbl_out} ("KEY", "NAME", "VALUE", "STATE", "YEAR")
            (SELECT "KEY",
    ...
    def apply(self, record_id, chgs_df, pr_df):
        """
        Searches procedures and charges to determine whether the
        indicated record_id should have this particular utilization
        flag. Returns the appropriate value based on HCUP flag
        coding conventions.
        
        Parameters

        pr_df: pandas DataFrame, required
            Procedure data in long format (indexed on KEY, data are
            ICD-9 codes in column PR and CCS codes in column PRCCS)

        chgs_df: pandas DataFrame, required
            Charges data in long format (indexed on KEY, data are
            UB92/UB04 codes in column REVCODE [and dollar amounts
            in CHARGE?])
        ...
        Return values:
            ...
            *N.B. that this does not mean that contradictory
            information is present, only that one category of
            information lacks positive evidence in support of the
            utilization flag.
        """
    ...
    for k, d in definitions.iterrows():
        ...
        if s_up == 'AZ':
            ...
            if year > 2009 and not pd.isnull(d['chgn_fl_gt_2009']):
                chg_group = str(d['chgn_fl_gt_2009']).split(';')
        ...
        f = UtilizationFlagger(d['name'], revcodes=r, chg_group=chg_group,
                               pr=pr, prccs=prccs)
        flaggers.append(f)
