...
def CurveShift(crv, rule, calendar=None):
    ...
    shifted_crv = crv.__new__(crv.__class__)
    return shifted_crv.__init_raw__(shifted_dts, crv.values, crv.dtype)
...
def CurveIntersect(curves):
    ...
    for crv in curves:
        ...
        new_crv = crv.__new__(crv.__class__)
        new_crv.__init_raw__(crv.dates[idxs], crv.values[idxs], crv.dtype)
...
#                mask[idx] = False
#        new_crv = crv.__new__(crv.__class__)
#        new_crv.__init_raw__(dts[mask], crv.values[mask], crv.dtype)
#        intersected_curves.append(new_crv)
...
def CumSum(crv):
    ...
    new_crv = crv.__new__(crv.__class__)
    return new_crv.__init_raw__(crv.dates, np.cumsum(crv.values), crv.dtype)
...
def ApplyToCurves(curves, func=min):
    ...
    new_crv = crv.__new__(crv.__class__)
    return new_crv.__init_raw__(crv.dates, [func(v) for v in vls], crv.dtype)
...
def Daily(crv, method="InterpolateStep", calendar=None):
    ...
    new_crv = crv.__new__(crv.__class__)
    return new_crv.__init_raw__(dates, values, crv.dtype)
