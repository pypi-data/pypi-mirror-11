...
#dyn 7 = -? Float, 8 = -? Int, 9 = /? Float, 0 = /? Int
...
#dyn 1 = Float, 2 = Integer, 3 = ?x Float, 4 = ?x Int, 5 = +? Float, 6 = +? Int, None = Float
#dyn 7 = -? Float, 8 = -? Int, 9 = /? Float, 0 = /? Int
def rand_sleep(start=1, end=10, decimal_places=0, dyn=None, dyn2=None):
    if decimal_places is None:
        if dyn % 2 == 1:
            dyn -= 1
    if dyn == 1 or None:
...
#dyn: None = print, 1 = return, 2 = print + return, 3 = print two terms, 4 = return two terms
#dyn2: 0=item before msg, 1=msg before item (except on return)
def i_call(item, dyn=None, item2=None, msg="", dyn2=0):
    item = str(item)
    msg = str(msg)
    ...
    if dyn is None:
        ...
        if dyn2 == 0:
            print(msg + item + item2)
        else:
            print(item + item2 + msg)
    else:
        if dyn2 == 0:
             var = str(msg + item + item2)
             return var
        elif dyn2 == 1:
            var = str(item + item2 + msg)
            return var
        else:
            var = str(item + item2)
            return var
#Dyn = 0 = is not, 1 = is, 2 = greater than, 3 = less than, 4 = greater than or is, 5 = less than or is
def check(val1, val2=None, dyn=1):
    bool = False
    if dyn is 0:
        if val1 != val2:
            bool = True
