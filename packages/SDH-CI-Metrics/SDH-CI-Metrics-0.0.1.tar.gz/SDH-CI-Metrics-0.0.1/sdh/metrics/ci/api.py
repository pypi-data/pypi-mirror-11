...
@app.orgmetric('/total-passed-executions', 'sum', 'passed-executions', title='Executions')
...
@app.orgmetric('/total-failed-executions', 'sum', 'failed-executions', title='Executions')
...
@app.repometric('/total-passed-repo-executions', 'sum', 'passed-executions', title='Executions')
...
@app.repometric('/total-failed-repo-executions', 'sum', 'failed-executions', title='Executions')
...
@app.repotbd('/repo-build-time', 'build-time', title='Build time')
...
@app.orgtbd('/avg-build-time', 'build-time', title='Build time')
...
@app.orgtbd('/total-build-time', 'build-time', title='Build time')
...
@app.orgtbd('/total-broken-time', 'broken-time', title='Broken time')
def get_total_broken_time(**kwargs):
    return [sum([store.get_broken_time(rid) for rid in store.get_repositories()])]


@app.repometric('/repo-broken-time', 'sum', 'broken-time', title='Broken time')
...
@app.repometric('/repo-time-to-fix', 'avg', 'time-to-fix', title='Time to fix')
def get_repo_time_to_fix(rid, **kwargs):
    ...
    return {'begin': begin, 'end': end}, [store.get_time_to_fix(rid, begin=begin, end=end)]


@app.orgmetric('/time-to-fix', 'avg', 'time-to-fix', title='Time to fix')
def get_time_to_fix(**kwargs):
    ...
    return {'begin': begin, 'end': end}, avg(
        [store.get_time_to_fix(rid, begin=begin, end=end) for rid in store.get_repositories()])
