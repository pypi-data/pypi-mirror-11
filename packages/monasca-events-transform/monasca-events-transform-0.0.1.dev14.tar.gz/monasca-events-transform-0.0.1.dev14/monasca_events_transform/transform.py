...
class NewCondenser(condenser.DictionaryCondenser):

    def __init__(self, **kw):
        self.clear()
        super(condenser.DictionaryCondenser, self).__init__(**kw)
...
class Transform(object):
    def __init__(self):
        ...
        for row in retrieve_transforms():
            transform_id = row[0]
            transform_def = yaml.load(row[1])
            print ""
            print "TRANSFORM FROM MYSQL"
            print transform_def
            print ""
            self._distiller_table[transform_id] = (
                distiller.Distiller(transform_def))

        self._condenser = NewCondenser()
    ...
    def run(self):
        ...
        for event in self._event_consumer:
            ...
            for v in self._distiller_table.values():
                e = v.to_event(event_data[0], self._condenser)
                if e:
    ...
    def _transform_definitions(self):
        for definition in self._definition_consumer:
            ...
            if transform_def == []:
                ...
                print ""
                print "GOT A DEFINITION"
                print definition_payload
                print ""
