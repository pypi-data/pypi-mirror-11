...
class Chores(object):
    ...
    def _target_statuses(self, status=None, exclude=None):
        ...
        if status is None:
            ...
            targets = set(status) if is_sequence(status) else set([status])
        if exclude:
            excluding = set(exclude) if is_sequence(exclude) else set([exclude])
            targets = targets - excluding
        return targets

    def marked(self, status=None, exclude=None):
        """
        ...
        if status is None and exclude is None:
            ...
            return [ key for key, item in self._data.items() if item.status in target ]
