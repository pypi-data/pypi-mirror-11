...
import xml.etree.ElementTree as XML, multiprocessing, ConfigParser, subprocess, threading, platform,\
...
class Storage(object):
	...
	def select(self, name, **kwargs):
		...
		$kwargs is interpreted as key=value conjunction, except for the keywords:
		- 'where' (expression with ? placeholders), 'parameters' (values)
		- 'orderby' (list of strings), 'desc' (boolean)
		"""
		with self.connection:
			...
			if "orderby" in kwargs:
				sql += " order by %s %s" % (",".join(kwargs["orderby"]), "desc" if kwargs.get("desc", True) else "asc") 
			try:
