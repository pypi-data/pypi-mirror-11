...
class RecordCollection(object):
    ...
    def coCiteNetwork(self, dropAnon = True, nodeType = "full", nodeInfo = True, fullInfo = False, weighted = True, dropNonJournals = False, filters = ['anon', 'journal', ]):
...
def isiParser(isifile):
    ...
    try:
        if "VR 1.0" not in f.__next__()[1] and "VR 1.0" not in f.__next__()[1]: #Check first 3 lines
            openfile.close()
