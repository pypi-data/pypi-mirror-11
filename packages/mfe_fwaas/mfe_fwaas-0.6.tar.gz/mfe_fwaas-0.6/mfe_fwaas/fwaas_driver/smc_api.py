...
class SMCAPIConnection(object):
    ...
    def login(self):
        # TODO: join the neutron plugin SMC and this
        self.session = requests.session()
        auth_key_dict = {"authenticationkey": self.auth_key}

        res = self.post("login", json.dumps(auth_key_dict))
        if res.code == 200:
            return True
        else:
            LOG.error("Authentication failed to %s with:%s" %
                      (self.host_api_url, json.dumps(auth_key_dict)))
            return False
    ...
    def session_op(self, attr, path, raw=False, data=None, headers=None):
        ...
        LOG.debug("[SMC-API] [%s]:[%s]:[%s]:[%s]" % (attr, path, headers, data))
        if raw:
