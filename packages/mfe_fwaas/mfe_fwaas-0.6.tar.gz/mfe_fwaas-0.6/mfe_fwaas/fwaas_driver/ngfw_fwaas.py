...
import mfe_fwaas.fwaas_driver.smc_api as smc_api
...
class NgfwFwaasDriver(fwaas_base.FwaasDriverBase):
    ...
    def delete_firewall(self, agent_mode, apply_list, firewall):
        ...
        # TODO: the engine is left with the original policy
    ...
    def update_firewall(self, agent_mode, apply_list, firewall):
        ...
        for router_info in apply_list:
            ...
            if (rt['tenant_id'] == firewall['tenant_id'] and
                ...
                LOG.debug("Firewall %s not active, ignoring update" % (firewall))
    ...
    def _get_ref_from_addr(self, addr):
        if addr == 'None':
            return self._create_network('0.0.0.0/0')
    ...
    def _get_ref_from_service_data(self, service_path, service_data):
        '''Return reference to the corresponding SMC element.

        Checks if element of the same same name already exists and
        returns it. Otherwise, creates a new element.

        TODO: validate that the element network value matches.
        '''
        LOG.debug("service_path:%s, service_data:%s" % (service_path,service_data))
        #pdb.set_trace()
        with self.connection.login_server():
            r = self.connection.get("elements/" + service_path)
            results = r[0]['result']
            #pdb.set_trace()
            for element in results:
                if element['name'] == service_data["name"]:
                    return element['href']
    ...
    # TODO: convert empty IPs to ANY?
    # TODO: unable to crewate 0.0.0.0/0 network 
    def _convert_ipv4_to_ngfw_rule(self, rule):
        # convert the ipv4 rule into ngfw rules

        # create src/dst of hosts or networks
        # TODO: SMC does not allow duplicate element names sow if src and dst are 
        # the same it fails
    ...
    def _clear_policy(self, router, fw):
        ...
        with self.connection.login_server():
            ...
            '''
            #TODO: /elements/search_unused does not work 
    ...
    def _create_network(self, cidr):
        '''Create element network element in the SMC.

        cird must be sring valid CIDR format string with netmask < 32.

        TODO: why can we not create 0.0.0.0/0 network element in SMC?
        '''
        ref = None

        if netaddr.IPNetwork(cidr).ip.__int__() == 0:
            name = "Any network"
        else:
