...
class Group:
    ...
    def get_bluetooth_list(self, after=None):
        ...
        :raises: HTTPError, socket.timeout
        """
        return self._get_list(self.server_url + "/bluetooth.json", after=after)
    ...
    def get_serial_list(self, after=None):
        ...
        :raises: HTTPError, socket.timeout
        """
        return self._get_list(self.server_url + "/serial.json", after=after)

    def _get_list(self, uri, after=None):
        if after:
            uri += '?after=' + str(after)
        return IoTurllib.get(uri, self.timeout)
