...
import argparse
import logging
from graphalviz.log import setup_logging

setup_logging()
logger = logging.getLogger('graphalviz')

from graphalviz.settings import SERVER_HOST, SERVER_PORT
...
def main():
    ...
    if args.server:
        from graphalviz.server import main as server_main
        logger.debug('Running graphalviz as server')
        server_main(host=args.host)
