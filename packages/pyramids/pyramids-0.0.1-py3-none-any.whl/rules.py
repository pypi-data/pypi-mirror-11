...
from pyramids.categories import Category, Property
from pyramids.parsetrees import ParseTreeNode
...
class ParseRule:
    ...
    def set_score(self, measure, score, accuracy):
        if not isinstance(measure, ScoringMeasure):
            measure = ScoringMeasure(measure)
        score = float(score)
    ...
    def __call__(self, parser_state, new_token, index):
        if new_token in self:
            ...
            parser_state.add_node(
                ParseTreeNode(parser_state, self, index, category, index))
            return True
        else:
            return False
...
class SetRule(LeafRule):
    ...
    def __repr__(self):
        return type(self).__name__ + "(" + repr(
            self.category) + ", " + repr(sorted(self.tokens)) + ")"

    def __str__(self):
        return str(self.category) + '.ini'
...
class BranchRule(ParseRule):
    ...
    def iter_scoring_measures(self, parse_node):
        ...
        yield ScoringMeasure(('head', parse_node.head_token))
...
class ConjunctionRule(BranchRule):
    ...
    def get_category(self, parser, subtree_categories, head_index=None):
        if head_index is None:
            ...
            raise NotImplementedError()
        if self.category.is_wildcard():
            category = Category(subtree_categories[-1].name,
                                self.category.positive_properties,
                                self.category.negative_properties)
        else:
            category = self.category
        ...
        conjunction_property = Property("conjunction")
        compound_property = Property("compound")
        simple_property = Property("simple")
        single_property = Property("single")
