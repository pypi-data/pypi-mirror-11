...
# TODO: Switch this over to use parse_semantics.py instead of
#       knowledge_base.py, etc.
import knowledge_base

import language_graphs
from pyramids.benchmarking import Benchmark
from pyramids.categories import Property, Category
from pyramids.exceptions import GrammarSyntaxError
from pyramids.parser import Parser
from pyramids.rules import SequenceRule, AnyTermMatchRule, \
    AllTermsMatchRule, CompoundMatchRule, HeadMatchRule, OneTermMatchRule, \
    LastTermMatchRule, ConjunctionRule, SuffixRule, SetRule, \
    PropertyInheritanceRule
from tokens import StandardTokenizer
import words
...
class ParserConfigInfo:
    ...
    def __init__(self, config_file_path, defaults=None):
        ...
        # Scoring
        self._scoring_measures_file = config_parser.get(
            'Scoring',
            'Scoring Measures File'
        )

        # Benchmarking
        self._benchmark_file = config_parser.get(
            'Benchmarking',
            'Benchmark File'
        )
...
class ParserLoader:
    ...
    def parse_category(definition, offset=1):
        ...
        if '(' in definition:
            ...
            return Category(
                name,
                [Property(name) for name in positive],
                [Property(name) for name in negative]
            )
        else:
            if ')' in definition:
                raise GrammarSyntaxError(
    ...
    def parse_grammar_definition_file(self, path):
        ...
        for raw_line in open(path):
            ...
            try:
                ...
                raise error
                # TODO: Wrap regular errors with filename & lineno
                #       information like a GrammarSyntaxError, without
                #       hiding the original traceback.
        return branch_rules

    def parse_match_rule(self, definition, offset=1):
        if not definition.startswith('['):
            raise GrammarSyntaxError("Expected: '['", offset)
        if not definition.endswith(']'):
            raise GrammarSyntaxError(
                "Expected: ']'",
                offset + len(definition) - 1
        ...
        for raw_line in open(path, 'r'):
            try:
                ...
                try:
                    ...
                    error.set_info(path, line_number, None, line)
                    raise error
                tokens = frozenset(tokens.split())
                leaf_rules.append(SetRule(category, tokens))
            except GrammarSyntaxError as error:
    ...
    def load_word_set(self, file_path):
        ...
        return SetRule(category, tokens)
    ...
    def load_parser(self, config_info):
        ...
        tokenizer = StandardTokenizer(config_info.discard_spaces)
    ...
    def standardize_word_set_file(self, filepath):
        ...
        if str(category) + '.ini' != filename:
            os.rename(
                filepath,
                os.path.join(folder, str(category) + '.ini')
            )
    ...
    def find_word_set_path(self, config_info, category):
        ...
        for folder_path in config_info.word_sets_folders:
            return os.path.join(folder_path, str(category) + '.ini')
...
class ParserCmd(cmd.Cmd):
        ...
        if not line:
            if self._parser and self._parser.config_info:
                ...
                config_info = ParserConfigInfo(
                    os.path.abspath('competitive_parser.ini'))
        else:
    ...
    def do_load(self, line=''):
        ...
        self.do_save()
        if not line:
            line = os.path.abspath('data/competitive_parser.ini')
        config_info = ParserConfigInfo(line)
        self._parser = self._parser_loader.load_parser(config_info)
        self._benchmark = Benchmark.load(
            config_info.benchmark_file) if os.path.isfile(
            config_info.benchmark_file) else Benchmark()
        self._benchmark_dirty = False
    ...
    def do_discard(self, line=''):
        ...
        if os.path.isfile(config_info.benchmark_file):
            self._benchmark = Benchmark.load(config_info.benchmark_file)
        else:
    ...
    def do_links(self, line):
        ...
        if self.parses_available:
            ...
            builder = language_graphs.LanguageGraphBuilder(kb)
            parse.build_language_graph(builder)
            for sentence in builder.build():
                print(sentence.to_str(False))
                print('')
        else:
            print("No parses found.")

    def do_linchpins(self, line):
        """Display the semantic net linchpin links for the current parse.
        """
        if line:
            print("'linchpins' command does not accept arguments.")
            return
        if self.parses_available:
            parse = self._parses[self._parse_index]
            kb = knowledge_base.KnowledgeBase()
            ...
            for sentence in builder.build():
                ...
                print('')
                link_strings = [
                    words.WordUse(link.source).spelling +
                    ' --' + link.type + '--> ' +
                    words.WordUse(link.sink).spelling
                    for link in sentence.iter_linchpins()
                ]
                for link_str in sorted(link_strings):
                    print(link_str)
                print('')
        else:
            print("No parses found.")
    ...
    def do_reverse(self, line):
        ...
        if self.parses_available:
            ...
            start_time = time.time()
            parse.build_language_graph(builder)
            sentences = list(builder.build())
            results = [
                self._parser.generate3(sentence, exact=True)
    ...
    def _get_benchmark_target(self):
        ...
        result = set()
        for sentence in builder.build():
            result.add(sentence.to_str(False))
        return '\n'.join(sorted(result))
    ...
    def _scoring_function(self, target):
        ...
        if (not target or
                ...
                self._parses[self._parse_index].get_weighted_score() < .9):
    ...
    def do_list(self, line):
        ...
        for input_val in sorted(self._benchmark.samples):
            print("  " + input_val)
            tokens = len(list(self._parser.tokenizer.tokenize(input_val)))
            total_tokens += tokens
