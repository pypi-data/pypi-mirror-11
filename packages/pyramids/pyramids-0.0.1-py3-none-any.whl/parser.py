...
from pyramids.utility import iter_combinations
from pyramids.rules import LeafRule, SequenceRule
...
from pyramids.parsetrees import ParseTreeNodeSet, Parse, ParseTree
from pyramids.parsetrees import BuildTreeNode
...
from pyramids.categories import Property
...
class CategoryMap:
    ...
    def add(self, node):
        ...
        if start not in self._map:
            node_set = ParseTreeNodeSet(node)
...
class ParserState:
    ...
    def tokens(self):
        if self._token_sequence is None:
            self._token_sequence = TokenSequence(self._tokens)
    ...
    def get_parse(self):
        ...
        return Parse(
            self.tokens,
            [ParseTree(self.tokens, node) for node in self._roots]
        )
...
class Parser:
    ...
    def extend_properties(self, category):
        ...
        negative -= positive
        return Category(category.name, positive, negative)
    ...
    def _generate3(self, head_node, sentence):
        # Find the subnodes of the head node
        subnodes = set()
        for link in head_node.outgoing.iter_links():
            if link.sink in sentence:
                subnodes.add(link.sink)
        # Build the subtree for each subnode
        subtrees = {}
        for subnode in subnodes:
            subtrees[subnode] = self._generate3(subnode, sentence)
        # Find all leaves for the head node
        subtrees[head_node] = set()
        word_use = words.WordUse(head_node)
        positive_case_properties, negative_case_properties = \
            LeafRule.discover_case_properties(word_use.spelling)
        for rule in self._primary_leaf_rules:
            if word_use.spelling in rule:
                ...
                if category in word_use.category:
                    tree = BuildTreeNode(rule, category, word_use)
        ...
        # serve as results
        if len(subtrees) == 1:
            if head_node is sentence.head:
                for tree in subtrees[head_node]:
                    if tree.category in sentence.category:
                        results.add(tree)
                    else:
        ...
        while insertion_queue:
            ...
            for rule in self._branch_rules:
                ...
                for index in range(len(rule.link_type_sets)):
                    ...
                    for link_type in required_incoming:
                        component_head_candidates &= {
                            l.source
                            for l in head_node.incoming.iter_links(
                                link_type
                            )
                            if l.type in self._rules_by_link_type
                        }
                    ...
                    for link_type in required_outgoing:
                        component_head_candidates &= {
                            ...
                            if link.type in self._rules_by_link_type
                        }
                        if not component_head_candidates:
                            break
                    if not component_head_candidates:
                ...
                for component_combination in \
                        iter_combinations(possible_components):
                    covered = set()
                        ...
                        if rule.is_non_recursive(
                            ...
                            if new_tree not in results:
                                if subnodes <= new_tree.node_coverage:
                                    if ((new_tree.head_node.root_node is
                                            not sentence.head) or
                                            category in sentence.category):
    ...
    def generate3(self, sentence, language_graph_builder=None, exact=True):
        # if language_graph_builder is None:
        #     kb = knowledge_base.KnowledgeBase()
        #     language_graph_builder = \
        #         language_graphs.LanguageGraphBuilder(kb)
        return self._generate3(sentence.head, sentence)
