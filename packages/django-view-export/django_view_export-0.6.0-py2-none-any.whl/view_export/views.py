...
def csv_view_export(request, view_name, prefix='v_'):
    ...
    Args:
        ...
        view_name: The name of the SQL view to export. If the value provided
    ...
    filename = get_report_path(view_name[len(prefix):])
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename={filename}'.format(
            filename=filename)

    if view_name in getattr(settings, 'OFF_PEAK_VIEWS', []):
        try:
            ...
            logger.error(
                ...
                'was not run overnight.'.format(
                    report=view_name, user=request.user))
    ...
    try:
        export_view(view_name, response, prefix)
