...
class MySQLHelper(object):
    def __init__(self, db_config):
        self._host = db_config['db_host']
        self._user = db_config['db_user']
        self._pwd = db_config['db_pwd']
        self._db = db_config['db_database']
        self.last_id = 0
        self.conn = MySQLdb.connect(host=self._host, user=self._user, passwd=self._pwd, db=self._db, charset='utf8')
        if self.conn:
    ...
    def query_count(self, sql_text, args=None):
        """ 查询符合条件的记录数
        :param sql_text: 查询字符串，如：select * from member where account=%s and pwd=%s
        :param args: 查询条件值，如：('waner','123')
        :return: 返回self.fetch_assoc() 或 0
        """
        if not self.__query(sql_text, args):
            return 0
        rs = self.cursor.fetchone()
        return rs['ROWS_COUNT']
...
if __name__ == "__main__":
    # DB_MYSQL_CONN = {'db_host': '127.0.0.1', 'db_user': 'root', 'db_pwd': 'wanersoft', 'db_database': 'wechat_v1'}
    # with MySQLHelper(DB_MYSQL_CONN) as sql:
    #     ls = sql.query_count("select COUNT(*) as ROWS_COUNT from ws_wechat where id=0", None)
    #     print ls
        # for l in ls:
        #     print l
    pass
