...
class Model(dict):
    ...
    def count(cls, where=None, args=None):
        ...
        :return:返回数字
        """
        sql_str = "select COUNT(*) as ROWS_COUNT from %s" % cls.__table__
        if where:
            sql_str += ' where %s' % where
        sql_str = sql_str.replace('?', '%s')

        with MySQLHelper(cls.__conn__) as sql:
            return sql.query_count(sql_str, args)
    ...
    def limit(cls, current=0, page_size=0, field=None, where=None, args=None):
        ...
        :param field: 读取字段，如：id,account,pwd
        :param where: 如：account=? and pwd=?
        :param args: 条件值，如：('waner','123')
        :return:
        """
        current = 0 if current == 1 else current - 1
        # select id,order_no from ws_payrecords where id>=(select id from ws_payrecords limit 2000000,1) limit 10;
        ...
        if current * page_size > 1000:
            from_str = "select `%s` from %s" % (cls.__primary_key__, cls.__table__)
            if where:
                from_str += ' where %s' % where
            from_str += ' limit %s,1' % (current * page_size)
            sql_str += ' where %s>=(%s) limit %s' % (cls.__primary_key__, from_str, page_size)
        else:
            if where:
                sql_str += ' where %s' % where
            sql_str += ' limit %s,%s;' % (current * page_size, page_size)
