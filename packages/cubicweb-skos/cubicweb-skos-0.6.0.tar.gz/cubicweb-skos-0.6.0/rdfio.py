...
def entity_same_as_uris(entity):
...
def add_entities_to_rdf_graph(graph, entities, reg, entity_same_as_uris=entity_same_as_uris):
    """Add information about a list of entities and a RDF registry into the RDF graph.
    """
    type_predicate = graph.uri(TYPE_PREDICATE_URI)
    same_as_predicate = graph.uri(SAME_AS_PREDICATE_URI)
    build_uri = lambda x: graph.uri(x.absolute_url())
    _todo = set(entities)
    _processed = set()

    def _add_related(uri, rtype, predicate_uri, reverse, related_entities):
        """Add entities related through 'predicate_uri' to the graph, and add them to the stack of
        entities to process.
        """
        predicate = graph.uri(predicate_uri)
        for related_entity in related_entities:
            if related_entity.eid not in _processed:
                _todo.add(related_entity)
            if reverse:
                graph.add(build_uri(related_entity), predicate, uri)
            else:
                graph.add(uri, predicate, build_uri(related_entity))

    while _todo:
        entity = _todo.pop()
        _processed.add(entity.eid)
        etype = entity.cw_etype
        if etype not in reg.etype2rdf:
            continue
        uri = build_uri(entity)
        graph.add(uri, type_predicate, reg.etype2rdf[etype])
        for same_as_uri in entity_same_as_uris(entity):
            graph.add(uri, same_as_predicate, graph.uri(same_as_uri))
        for rtype, predicate_uri, reverse in reg.predicates_for_subject_etype(etype):
            value = getattr(entity, rtype)
            if value is None:
                continue
            if isinstance(value, (tuple, list)):  # relation
                _add_related(uri, rtype, predicate_uri, reverse, value)
            else:
        ...
        for rtype, predicate_uri, reverse in reg.predicates_for_object_etype(etype):
            try:
                ...
                continue
            _add_related(uri, rtype, predicate_uri, not reverse, related)
