...
class RDFAdaptersTC(CubicWebTC):
    ...
    def _test_entity_list_repr(self, graph):
        with self.admin_access.client_cnx() as cnx:
        ...
        self.assertEqual(set(graph.triples()), expected)

    @require_module('rdflib')
    def test_rdflib_entity_list_repr(self):
        self._test_entity_list_repr(RDFLibRDFGraph())

    @require_module('RDF')
    def test_librdf_entity_list_repr(self):
        self._test_entity_list_repr(LibRDFRDFGraph())

    def _test_entity_primary_repr(self, graph):
        with self.admin_access.client_cnx() as cnx:
            scheme = cnx.execute('ConceptScheme X').one()
            ...
            rdfgenerator.fill(graph)
        expected = {(scheme_uri, TYPE_PREDICATE_URI, SKOS_SCHEME),
    ...
    @require_module('rdflib')
    def test_rdflib_entity_primary_repr(self):
        self._test_entity_primary_repr(RDFLibRDFGraph())

    @require_module('RDF')
    def test_librdf_entity_primary_repr(self):
        self._test_entity_primary_repr(LibRDFRDFGraph())
    ...
    def test_warm_relation_cache(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            self.assertEqual(set(scheme.cw_attr_cache),
