...
def register_skos_rdf_mapping(reg):
    ...
    reg.register_relation_equivalence('Concept', 'in_scheme', 'ConceptScheme', 'skos:inScheme')
    ...
    reg.register_relation_equivalence('Concept', 'related_concept', 'Concept', 'skos:related',
                                      reverse=True)
    reg.register_relation_equivalence('Label', 'pref_label_of', 'Concept', 'skos:prefLabel',
                                      reverse=True)
    reg.register_relation_equivalence('Label', 'alt_label_of', 'Concept', 'skos:altLabel',
                                      reverse=True)
    reg.register_relation_equivalence('Label', 'hidden_label_of', 'Concept', 'skos:hiddenLabel',
                                      reverse=True)
...
def _skos_entities(graph):
    ...
    register_skos_rdf_mapping(reg)
    # turn it into dumb intermediary representation
    def _build_label(extentity, rtype, uris):
        for label in uris:
            lang = getattr(label, 'lang', None)
            md5hash = md5(str(lang) + label.encode('utf-8'))
            labelid = str(extentity.extid) + '#' + rtype + md5hash.hexdigest()
            yield ExtEntity('Label', labelid,
                            {'label': set([label]),
                             'language_code': set([lang]),
                             rtype: set([extentity.extid])})
    return rdfio.rdf_graph_to_entities(reg, graph, ('ConceptScheme', 'Concept'),
                                       {'pref_label_of': _build_label,
                                        'alt_label_of': _build_label,
                                        'hidden_label_of': _build_label})


def _skos_import(cnx, entities, import_log, source=None, raise_on_error=False, metagenerator=None,
                 use_extid_as_cwuri=True):
    ...
    def externaluri_to_concept(extentity, cnx=cnx, extid2eid=extid2eid,
        ...
        try:
            ...
            if extentity.etype == 'Concept' and cnx.entity_metas(eid)['type'] == 'ExternalUri':
                ...
                # 3. drop its extid from the mapping to trigger insertion of the concept by the importer
    ...
    concept_schemes = []
    def externaluri_to_concept(extentity):
