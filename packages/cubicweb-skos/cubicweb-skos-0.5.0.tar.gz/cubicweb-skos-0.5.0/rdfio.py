...
class AbstractRDFGraph(object):
    ...
    def load(self, source, rdf_format=None):
        ...
        if rdf_format is None:
            ...
            try:
                rdf_format = self._extension_format_map[ext]
            except KeyError:
...
def add_entities_to_rdf_graph(graph, entities, reg, _processed=None):
    ...
    for entity in entities:
        ...
        if urlparse(entity.absolute_url()).netloc != urlparse(entity.cwuri).netloc:
            graph.add(uri, same_as_predicate, graph.uri(entity.cwuri))
        for rtype, predicate_uri, reverse in reg.predicates_for_subject_etype(etype):
            ...
            if isinstance(values, (tuple, list)):  # relation
                add_entities_to_rdf_graph(graph, values, reg, _processed)
                values = (build_uri(e) for e in values)
            else:
                values = (values,)
            for value in values:
                if reverse:
                    graph.add(value, predicate, uri)
                else:
                    graph.add(uri, predicate, value)
        for rtype, predicate_uri, reverse in reg.predicates_for_object_etype(etype):
            predicate = graph.uri(predicate_uri)
            try:
                entities = getattr(entity, 'reverse_' + rtype)
            except AttributeError:
                # symmetric relations have no 'reverse_' attribute
                # XXX what if this is simply a bad mapping?
                continue
            add_entities_to_rdf_graph(graph, entities, reg, _processed)
