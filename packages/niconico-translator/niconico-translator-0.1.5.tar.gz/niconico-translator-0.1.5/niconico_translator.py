...
# We resolve the hostname of {msg,nmsg}.nicovideo.jp using dnspython,
# to workaround the situation that {msg,nmsg}.nicovideo.jp refers to 127.0.0.1
# in /etc/hosts file.
API_HOSTNAMES = frozenset(['msg.nicovideo.jp', 'nmsg.nicovideo.jp'])
API_IP_ADDRESSES = {host: query(host)[0].to_text() for host in API_HOSTNAMES}
...
class App(object):
    ...
    def wsgi_app(self, environ, start_response):
        ...
        original_request.host = API_IP_ADDRESSES[
...
def translate_multiple(text_list, language):
    ...
    while len(result_list) < total:
        ...
        while buffer_limit > 0:
            ...
            split = re.split(r'\s*' + boundary.strip() +
                             '\s*', translated)[:-1]
            if split:
                break
            buffer_limit //= 2
