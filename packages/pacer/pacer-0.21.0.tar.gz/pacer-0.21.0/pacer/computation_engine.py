...
class Engine(object):
    ...
    def __init__(self, n, raise_exceptions=True):
    ...
    def _run_async(self, f, args, callback, run_local=False, debug=False):
        ...
        if debug:
            ...
            if run_local:
                try:
                    get_logger(self).info("start {} with args {}".format(f, printable(args)))
                    result = f(*args)
                    get_logger(self).info("function {} returned {}".format(f, printable(result)))
                except Exception, e:
                    get_logger(self).info("function {} rose {}".format(f, e))
    ...
    def run(self, node, re_raise_exceptions=False):
        self._setup_pool()
        self.exception = None
...
"""
1. wo hängt philipps code / kann ich das in pacer nachvollziehen ?
2. delivery mit neuem pacer
3. test server checken
4. clean cache stuff checken auf test server !
"""
