...
@app.usertbd('/user-longest-streak', 'streak', title='Longest Streak')
def get_user_longest_streak(uid, **kwargs):
    ts_commits = [ts for (_, ts) in
                  store.db.zrangebyscore('metrics:total-user-commits:{}'.format(uid), '-inf', '+inf', withscores=True)]
...
@app.repometric('/avg-repo-commits', 'avg', 'commits', title='Commits')
