...
@app.repousermetric('/avg-repo-user-commits', 'avg', 'commits', title='Commits per day')
def get_avg_repo_user_commits(rid, uid, **kwargs):
    return aggregate(store, 'metrics:total-repo-user-commits:{}:{}'.format(rid, uid), kwargs['begin'], kwargs['end'],
                     kwargs['max'], aggr=avg)


@app.usermetric('/avg-user-commits', 'avg', 'commits', title='Commits per day')
...
@app.orgmetric('/avg-commits', 'avg', 'commits', title='Commits per day')
