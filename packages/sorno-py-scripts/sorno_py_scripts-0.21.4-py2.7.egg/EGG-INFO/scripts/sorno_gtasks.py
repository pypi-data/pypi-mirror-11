...
class GoogleTasksConsoleApp(object):
    ...
    def auth(self, flags, use_credentials_cache=True):
        ...
        if not client_id:
            _log.info(
                "Please set the environment variable GOOGLE_CLOUD_CLIENT_ID"
            )
            sys.exit(1)
            ...
            for tasklist_name in tasklists_names:
                if tasklist_name not in tasklists_map:
                    ...
                    for index, tasklist in enumerate(tasklists, 1):
                        if _plain_logger.isEnabledFor(logging.DEBUG):
...
--with-notes option to print the notes as well. Use the --debug option of the
script to show details for all tasks.

Examples:

    To show the details for all tasks and all task lists.

        $ sorno_gtasks.py --debug get_tasks
    """
