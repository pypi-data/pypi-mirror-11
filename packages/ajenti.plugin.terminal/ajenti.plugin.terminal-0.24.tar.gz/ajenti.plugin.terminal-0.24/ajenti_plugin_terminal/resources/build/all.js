...
// Generated by CoffeeScript 1.8.0
(function() {
  angular.module('ajenti.terminal').controller('ScriptWidgetController', function($scope, $location, notify, terminals) {
    return $scope.run = function() {
      ...
      if ($scope.widget.config.terminal) {
        ...
        return terminals.script({
          ...
          if (data.code === 0) {
            return notify.success('Script has finished', data.output + data.stderr);
          } else {
            return notify.warning('Script has failed', data.stderr + data.output);
          }
        })["catch"](function(err) {
          return notify.error('Could not launch the script', err.message);
        });
      }
    };
  });
