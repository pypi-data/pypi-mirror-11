...
class HTTPRequest(object):
    ...
    def authorize(self, username, password, token):
...
class HttpConnection(object):
    ...
    def __init__(self, host, port,
        ...
        # self._auth_handler = HTTPBasicAuth()
        self.debug = debug
    ...
    def _build_http_request(self, url, base_params, verb):
        ...
        if verb.lower() == 'get':
            ...
            ext_url = ""
        return HTTPRequest(verb, self.protocol, "", self.host, self.port, url + ext_url,
                 params)

    def send(self, url, params, verb = 'GET', user_name="", passwd="", token=""):
        ...
        while retry_time < self.retry_time:
            ...
            try:
                if verb == "POST":
                    request.authorize(username=user_name, password=passwd, token=token)
                ...
                if response.status == 200:
                    ...
                    for header in response.getheaders():
                        if header[0].lower() == 'location':
                            request = self._build_http_request(header[1], {}, 'GET')
                    conn = self._get_conn()
                else:
                    conn = self._get_conn()
            except:
