...
from cubes.wsme.types import PassThroughType, JsonData, Any, Base
from cubes.wsme.predicates import match_ws_etype

from cubes.wsme.loader import to_fetchtree, load_entities
...
class WSCRUDController(WSController):
    ...
    def _get(self, orderby=None, filter=None, limit=0, offset=0, fetch=None,
             keyonly=False):
        ...
        return load_entities(
            self._cw, self.__cwetype__,
            orderby=orderby, query_filter=filter,
            limit=limit, offset=offset,
            fetchtree=to_fetchtree(fetch, keyonly))

    def _post(self, fetch=None, keyonly=False, data=None):
        ...
        return load_entities(
            self._cw, self.__cwetype__,
            query_filter={'eid': entity.eid},
            fetchtree=to_fetchtree(fetch, keyonly))[0]

    def _entity_get(self, entity, fetch=None):
        """ Default implementation of `GET /etype/eid`."""
        return load_entities(
            self._cw, self.__cwetype__,
            query_filter={'eid': entity.eid},
            fetchtree=to_fetchtree(fetch, False))[0]

    def _entity_put(self, entity, fetch=None, data=None):
