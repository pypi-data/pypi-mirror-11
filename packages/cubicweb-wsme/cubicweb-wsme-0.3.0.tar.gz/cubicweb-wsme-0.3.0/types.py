...
class wsattr(wsme.types.wsattr):
    ...
    def reginit(self, vreg, class_):
        ...
        try:
            ...
            log.exception("Cannot reginit %s", self.name)
            return
    ...
    def finalize_init(self, class_):
        ...
        try:
            ...
            log.exception("Cannot finalize_init %s", self.name)
            return
...
class Base(six.with_metaclass(BaseMeta, wsme.types.Base)):
    ...
    @classmethod
    def attr_by_name(cls, name):
        l = filter(lambda a: a.name == name, cls._wsme_attributes)
        if len(l) == 0:
            raise KeyError("No attr named {}".format(name))
        return l[0]
    ...
    def reginit(cls, vreg):
        ...
        if getattr(cls, '__autoattr__', False):
            try:
                ...
                log.exception("Cannot reginit %s", cls.__name__)
                return
        ...
        vreg.wsme_registry.reregister(cls)
    ...
    def finalize_init(cls):
        ...
        """
        if not hasattr(cls, '_wsme_attributes'):
            return
        for attr in cls._wsme_attributes:
            if isinstance(attr, wsattr):
    ...
    def load_from_row(self, cnx, rset, row_i, row, mapping, fetchtree):
        for attrname, index_or_relmap in mapping.items():
            if isinstance(index_or_relmap, int):
                value = row[index_or_relmap]
            else:
                value = getattr(self.__class__, attrname).datatype.from_row(
                    cnx, rset, row_i, row, index_or_relmap, fetchtree)
            setattr(self, attrname, value)

    @classmethod
    def from_row(cls, cnx, rset, row_i, row, mapping, fetchtree):
        if row[mapping['eid']] is None:
            return None
        instance = cls()
        instance.load_from_row(cnx, rset, row_i, row, mapping, fetchtree)
        return instance
    ...
    def to_entity(self, cnx, extra_relations=()):
        ...
        for attr in self._wsme_attributes:
            ...
            if attr.isfinal:
                ...
                values[attr.rtype] = value.to_entity(cnx) if value else None
            else:
...
class Any(Base):
    ...
    def load_from_row(self, cnx, rset, row_i, row, mapping, fetchtree):
        i = mapping['eid']
        self.cw_etype = str(rset.description[row_i][i])
        super(Any, self).load_from_row(
            cnx, rset, row_i, row, mapping, fetchtree)
