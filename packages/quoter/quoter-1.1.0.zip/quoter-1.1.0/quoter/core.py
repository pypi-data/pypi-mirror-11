...
class XMLQuoter(Quoter):
    ...
    def __getattr__(self, name):
        try:
            ...
            #try:
            #    return Quoter.__dict__[name]
            #except KeyError:
            #    return XMLQuoter(tag=name, name=name)
            return XMLQuoter(tag=name, name=name)

    # Having an auto-instantiate capability through __getattr__ is
    # great from an ease point of view, but makes the implementation
    # very tricky to get right, especially when inheritance is
    # involved. Have tried several different mechanisms such as
    # defining a clone method in XMLQuoter and having a multi-layer
    # __dict__ examination (which could grow to cover the entire
    # __mro__, possibly). For now, taking out instance auto-derivation
    # (e.g. ``xml.b(atts='.big')``) as that is the worst case, and the
    # only one that doesn't work well. It screws everything else up.
...
_ml_quoter = Quoter(prefix='<!--', suffix='-->', padding=1)
...
for t in _SELFCLOSING:
    setattr(HTMLQuoter, t, HTMLQuoter(t, void=True))
