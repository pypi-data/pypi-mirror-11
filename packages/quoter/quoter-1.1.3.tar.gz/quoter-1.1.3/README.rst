...
content with element class provided with the CSS selector syntax::
...
these look pretty ugly. They're low-level, dense invocations. Writing them
out, it's easy to mistype a character here or there, or to forget some of
the gotchas. Say you're formatting values, some of which are strings, but
others are integers or other primitive types. Instant ``TypeError``, because
only strings can be concatenated with strings in Python directly.

The repetitive, *ad hoc* nature of textual quoting wrapping is tiresome and
error-prone. It's never more so than when constructing multi-level quoted
strings, such as Unix command line arguments, SQL commands, or HTML
attributes.
