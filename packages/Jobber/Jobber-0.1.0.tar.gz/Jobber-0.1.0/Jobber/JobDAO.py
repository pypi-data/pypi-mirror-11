...
class JobDAO():
    ...
    def activateJob(self, jobId):
        ...
        if self.getJobStatus(jobId) == DEACTIVATED:
            self.updateJobStatus(jobId, IDLE)
    ...
    def getGroupJobAction(self, jobId):
        ...
        if self.isGroupJobFailed(jobId):
            return NOTHING if jobStatus == FAILED else FAIL
        if self.isGroupJobCancelled(jobId):
            return CANCEL if jobStatus != CANCELLED else NOTHING
        if self.isGroupJobFinished(jobId):
            return FINISH_GROUP if jobStatus != FINISHED else NOTHING
        if self.isGroupJobRunning(jobId):
            return EXECUTE if jobStatus != RUNNING else NOTHING
        else:
            return IDLE_GROUP if jobStatus != IDLE else NOTHING
    ...
    def getJobAction(self, jobId):
        jobStatus =  self.getJobStatus(jobId)
        if jobStatus != IDLE and jobStatus != RUNNING:
            return NOTHING
