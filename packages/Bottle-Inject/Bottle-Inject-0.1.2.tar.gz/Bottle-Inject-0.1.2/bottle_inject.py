...
class _InjectionPoint(object):
    ...
    def __init__(self, name, parameters=None, config=None, implicit=False):
        self.name = name
...
class Injector(object):
    ...
    def _prime(self, ip):
        ...
        try:
            ...
            err = InjectError("Could not resolve provider for injection point %r" % name)
            if not ip.implicit:
                raise err
        ...
        provider = self.call_inject(provider_resolver, *ip.parameters, **ip.config)
