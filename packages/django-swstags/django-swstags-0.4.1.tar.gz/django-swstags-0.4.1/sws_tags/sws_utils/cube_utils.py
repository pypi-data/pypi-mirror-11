...
class Salida_Grid:
    ...
    def filter(self, query, cube, dimension, database_cube_dict=[], exclude_rows=[], dimension_extra='Id'):
        ...
        select = mdx_class.__select_for_filter(cube, on_rows=on_rows, medida=measure, exclude_rows=exclude_rows)
...
class MDX:
    ...
    def __where_dict(self, cube, part_where):
        ...
        if time_from and time_to:
            ...
            if part_where[self.dim_date][0].date() == part_where[self.dim_date][1].date():
                ...
                if not(datetime.combine(part_where[self.dim_date][0] + timedelta(days=1), time.min) > datetime.combine(part_where[self.dim_date][1] - timedelta(days=1), time.max)):
    ...
    def __select_for_filter(self, cube, on_rows=[], medida='', exclude_rows={}):
