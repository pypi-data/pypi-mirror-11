...
class Importer(BaseAction):
    ...
    def machine_exists(self, id):
        ...
        return int(id) in map(int,
                              self.env.status().get('machines', {}).keys())
        ...
        if machines:
            ...
            for machine_name, spec in machines.items():
                if self.machine_exists(machine_name):
                    ...
                    self.machines[machine_name] = self.deployment.get_machine(
                        machine_name)
                else:
                ...
                for container_host in placement.get_new_machines_for_containers():
                    if self.machine_exists(machine_name):
                        self.log.info("Machine %s already exists,"
                                      "skipping creation" % machine_name)
                        self.machines[container_host] = self.deployment.get_machine(
                            container_host)
                    else:
                        self.log.info("A new container will be created"
                                      "on machine: %s" % container_host)
                        self.machines[container_host] = self.env.add_machine()
            self.deployment.set_machines(machines)
    ...
    def deploy_services(self, add_units=True):
        ...
        for svc in self.deployment.get_services():
            ...
            # Unset num_units if we are not adding units.  This should be done
            # after the above work to ensure that the status is still retrieved
            # as necessary.
            if not add_units:
                num_units = None
