...
class ServiceUnitPlacementV4(ServiceUnitPlacement):
    ...
    def validate(self):
        ...
        for i, placement in enumerate(unit_placement):
            container, placement, unit_number = self._parse_placement(placement)

            if container and container not in ('lxc', 'kvm'):
                feedback.error(
                    "Invalid container type: %s service: %s placement: %s" \
                        % (container, self.service.name, unit_placement[i]))
            # XXX Nesting containers not supported yet.
            # Makyo - 2015-03-01
            if container is not None and not (placement.isdigit()
                    or placement == 'new'):
                feedback.error(
                    "Invalid target for container: %s" % (
                        unit_placement[i]))
            # Specify an existing machine (or, if the number is in the
            # list of machine specs, one of those).
            if placement.isdigit():
                if placement in machines:
                    continue
                else:
                    feedback.error(
                        ("Service placement to machine "
                         "not supported %s to %s") % (
                             self.service.name, unit_placement[i]))
            # Specify a machine from the machine spec.
            elif placement in self.deployment.get_machines():
                continue
            # Specify a service for colocation.
            elif placement in services:
                # Specify a particular unit for colocation.
                if unit_number is not None and \
                        unit_number > services[placement].num_units:
                    feedback.error(
                        "Service unit does not exist, %s to %s/%s" % (
                            self.service.name, placement, unit_number))
                elif self.deployment.get_charm_for(placement).is_subordinate():
                    feedback.error(
                        "Cannot place to a subordinate service: %s -> %s" % (
                            self.service.name, placement))
    ...
    def get(self, unit_number):
        ...
        container, placement, unit_number = self._parse_placement(unit_placement)

        if placement in self.machines_map:
            return self._format_placement(self.machines_map[placement], container)

        # Handle <container_type>:new
        if placement == 'new':
            return self._format_placement(
                self.machines_map['%s/%d' % (self.service.name, u_idx)], container)
