...
class Sh(Command):
    ...
    def sh(self, image):
        ...
        if not img and len(image.split('/')) <= 2:
            ...
            if not img:
                print("Can't found  image {}.".format(image))
                return
        elif not img:
            img = image
        ...
        if repo and tag:
            self.ask_push(username, repo, tag)
    ...
    def ask_push(self, username, repo, tag):
        while True:
    ...
    def resolve_image(self, image):
        images = [s for s in self.get_all_images() if image == s]
        if len(images) > 1:
