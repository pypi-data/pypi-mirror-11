...
def manage(p, *args, **kwargs):
    ...
    # OpenOpt label
    Frame(root).pack(ipady=4, expand=True)
    Label(root, text=' OpenOpt ' + ooversion + ' ').pack(expand=True, fill='x')
    Label(root, text=' Solver: ' + (p.solver if isinstance(p.solver, str) else p.solver.__name__) + ' ').pack(expand=True, fill='x')
    Label(root, text=' Problem: ' + p.name + ' ').pack(expand=True, fill='x')
    #TODO: use Menubutton 
...
#    b.pack(side='right')
    ...
    Frame(root).pack(ipady=8, expand=True, fill='x')
    ...
    def invokeExit():
        ...
        root.destroy()
    Frame(root).pack(ipady=8, expand=True, fill='x')
    Button(root, text="      Exit      ", command = invokeExit).pack(ipady=15, expand=True, fill='x')
    
    '''                             Time                         '''
    Frame(root).pack(ipady=0, expand=True, fill='x')
    F = Frame(root)
    _time = StringVar()
    p._manager_time = 0
