...
class Project:
    ...
    def projects(self):
        ...
        content = self.connection.send_request("projectset_manager/get_projectset_list")
    ...
    def set_active(self, project_id):
        ...
        content = self.connection.send_request(
        ...
        if content.get("success", False):
            for project in self.projects:
                if project["name"] == project_name:
                    ...
                    logging.info("Project was created and activated.")
                    return True
            logging.error("Project was created but we were unable to activate it.")
            return False
        else:
    ...
    def metadata_parameters(self):
        ...
        content = self.connection.send_request("patient_manager/module_config")
    ...
    def add_metadata_parameter(self, param_id, param_title=None, param_type="string", visible=False):
        """
        Add a metadata parameer to the project to which this subject belongs.

        :param param_id: identificator of this new parameter
        :param param_title: title of this new parameter
        ...
        :rtype: Bool.
        """
        # use param_title equal to param_id if param_title is not provided
        param_title = param_title or param_id

        param_names = list(self.metadata_parameters.items())
        param_names.sort(key=lambda x: x[-1]["order"])
        param_names = [p[0] for p in param_names]
        param_names.append(param_id)

        param_properties = [param_id, param_title, param_type, str(int(visible))]

        post_data = {"add": "|".join(param_properties),
                     "edit": "",
                     "delete": "",
                     "order": "|".join(param_names)
                    }

        answer = self.connection.send_request(
                                        "patient_manager/save_metadata_changes",
                                        req_parameters=post_data)
        if not answer.get("success", False) or not param_id in answer.get("data", {}):
            logging.error("Could not add new parameter: {}".format(param_id))
            return False
        else:
            logging.info("New parameter added:", param_id, param_properties)
            return True

    def list_analysis(self, limit=100):
        req_headers = {"X-Range":"items=0-" + str(limit - 1)}
        return self.connection.send_request("project_manager/get_project_list",
    ...
    def list_input_containers(self, search_condition="*", limit=1000):
        ...
        req_headers = {"X-Range": "items=0-" + str(limit - 1)}
        content = self.connection.send_request("file_manager/get_container_list",
    ...
    def list_container_files(self, container_id):
        ...
        content = self.connection.send_request("file_manager/get_container_files",
    ...
    def download_file(self, container_id, file_name, local_filename=False,
        ...
        content = self.connection.send_request("file_manager/download_file",
                                               params, stream=True,
                                               return_raw_response=True)

        with open(local_filename, 'wb') as f:
            for chunk in tqdm(list(content.iter_content(chunk_size=1024))):
    ...
    def add_subject(self, subject):
        ...
        content = self.connection.send_request("patient_manager/upsert_patient",
    ...
    def upload_file(self, file_path, subject_name, date_of_scan="", description="",
        ...
        with open(file_path, 'rb') as file_object:
            ...
            while chunk_num < 10000:
                ...
                response = self.connection.send_request("upload",
                                         req_parameters=data,
        ...
        if len(error_message) == 0:
            ...
            if not result:
                ...
                if len(mappings_to_send) > 0:
                    ...
                    response_2 = _send_request(self.account,
                                                    _create_url(self.account, "file_manager/confirm_uploaded_files"),
                                                    req_parameters={"container_id":container_id, "mappings":txt_mappings})
