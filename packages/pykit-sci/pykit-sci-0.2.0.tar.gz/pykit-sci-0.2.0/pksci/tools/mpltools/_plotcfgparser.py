...
class PlotConfigParser(object):
    ...
    def _apply_global_options(self, dataset=None):
        if dataset is not None:
            for option, value in self._configobj['datasets'].items():
    ...
    def _parse_plotcfg(self):
        ...
        for axis in default_plotconfig.sections:
            ...
            for k, v in default_plotconfig[axis].items():
        ...
        for dataset_id, dataset in self._datasets.items():
            ...
            if isinstance(dataset, DataSet):
                ...
                for subset_id in dataset_config.sections:
                    ...
                    subset_config = dataset_config[subset_id]
                    for k, v in subset_config.items():
                        if v is not None:
                            try:
    ...
    def _flatten_datasets(self):
        ...
        for dataset_id, dataset in self._datasets.items():
            if isinstance(dataset, DataSet):
                ...
                for subset_id, subset_dataset in dataset.items():
                    self._update_flattened_datasets(
                        key=subset_id,
    ...
    def _parse_xydata(self):
        ...
        for dataset_id, dataset in self._flattened_datasets.items():
