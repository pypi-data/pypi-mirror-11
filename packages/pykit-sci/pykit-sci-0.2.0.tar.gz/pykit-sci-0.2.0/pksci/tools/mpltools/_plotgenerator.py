...
class XYPlot(MPLPlot):
    ...
    def plot(self):
        ...
        for dataset_id, dataset in self._datasets.items():
            ...
            for i, (yvar, ydata) in enumerate(dataset.ydata.items()):
                xdata = dataset.xdata[yvar]
                linewidth = dataset_plotconfig.linewidths[i]
                markercolor = dataset_plotconfig.markercolors[i]
                label = dataset_plotconfig.labels[i]
                alpha = dataset_plotconfig.alphas[i]

                if dataset.axhline:
                    ymax = max(ymax, ydata)
                    ymin = min(ymin, ydata)
                    axes.axhline(y=ydata, lw=linewidth, c=markercolor,
                                 label=label, alpha=alpha)
                elif dataset.axvline:
                    xmax = max(xmax, xdata)
                    xmin = min(xmin, xdata)
                    axes.axvline(x=xdata, lw=linewidth, c=markercolor,
                                 label=label, alpha=alpha)
                else:

                    xdata = xdata * self._plotconfig.datasets.xscale_factor
                    ydata = ydata * self._plotconfig.datasets.yscale_factors[i]
                    ...
                    p1, = axes.plot(
                        xdata, ydata,
                        marker=dataset_plotconfig.markers[i],
                        color=dataset_plotconfig.markercolors[i],
                        markersize=dataset_plotconfig.markersizes[i],
                        mec=dataset_plotconfig.markeredgecolors[i],
                        mew=dataset_plotconfig.markeredgewidths[i],
                        mfc=dataset_plotconfig.markerfacecolors[i],
                        fillstyle=dataset_plotconfig.fillstyles[i],
                        ls=dataset_plotconfig.linestyles[i],
                        lw=dataset_plotconfig.linewidths[i],
                        label=dataset_plotconfig.labels[i],
                        alpha=dataset_plotconfig.alphas[i],
                    ...
                    plots.append(p1)
                    offsets.append(
                        (self._figkwargs['dropshadow.offset.x'] / 2,
    ...
    def set_axes_limits(cls, axes=None, plotconfig=None, datalimits=None):
        ...
        if plotconfig.xaxis.left_limit is not None:
            ...
            left_limit = axes.get_xlim()[0]

        if plotconfig.xaxis.scale_left_limit:
            left_limit = plotconfig.xaxis.left_limit_scale_factor * left_limit

        left_limit += plotconfig.xaxis.left_limit_shift

        if plotconfig.xaxis.round_left_limit_down_to_nearest is not None:
            left_limit = \
                rounddown2nearestN(
                    left_limit,
        ...
        right_limit += plotconfig.xaxis.right_limit_shift

        if plotconfig.xaxis.round_right_limit_down_to_nearest is not None:
            right_limit = \
                rounddown2nearestN(
                    right_limit,
        ...
        if plotconfig.yaxis.bottom_limit is not None:
            ...
            bottom_limit = axes.get_ylim()[0]

        if plotconfig.yaxis.scale_bottom_limit:
            bottom_limit = \
                plotconfig.yaxis.bottom_limit_scale_factor * bottom_limit

        bottom_limit += plotconfig.yaxis.bottom_limit_shift

        if plotconfig.yaxis.round_bottom_limit_down_to_nearest is not None:
            bottom_limit = \
                rounddown2nearestN(
                    bottom_limit,
        ...
        if plotconfig.yaxis.top_limit is not None:
            ...
            top_limit = axes.get_ylim()[-1]

        if plotconfig.yaxis.scale_top_limit:
            top_limit = \
                plotconfig.yaxis.top_limit_scale_factor * top_limit

        top_limit += plotconfig.yaxis.top_limit_shift

        if plotconfig.yaxis.round_top_limit_down_to_nearest is not None:
            top_limit = \
                rounddown2nearestN(
                    top_limit,
            ...
            for r in range(self._rows):
                for dataset_id, dataset in self._datasets.items():
                        ...
                        for i, yvar in enumerate(
                            ...
                            if dataset.dataformat in ('textlog', 'txtlog'):
                                ...
                                ydata[yvar] = \
                                    dataset.data[:, dataset.headers.index(
                                        yvar)]
                            ydata[yvar] = ydata[yvar] * \
                                self._plotconfig.datasets.yscale_factors[i]
                    ...
                    if dataset.xdata is None:
                        ...
                        for yvar in dataset.ydata.keys():
                            if xvar is not None:
                                if dataset.dataformat in ('textlog', 'txtlog'):
                                    xdata[yvar] = \
                            ...
                            xdata[yvar] = xdata[yvar] * \
                                self._plotconfig.datasets.xscale_factor
                        dataset.xdata = xdata

                    for i, (yvar, ydata) in \
                            enumerate(dataset.ydata.items()):
                        xdata = dataset.xdata[yvar]
                        ...
                        self._axes[r, c].plot(
                            ...
                            marker=dataset_plotconfig.markers[i],
                            color=dataset_plotconfig.markercolors[i],
                            ms=dataset_plotconfig.markersizes[i],
                            mec=dataset_plotconfig.markeredgecolors[i],
                            mew=dataset_plotconfig.markeredgewidths[i],
                            mfc=dataset_plotconfig.markerfacecolors[i],
                            fillstyle=dataset_plotconfig.fillstyles[i],
