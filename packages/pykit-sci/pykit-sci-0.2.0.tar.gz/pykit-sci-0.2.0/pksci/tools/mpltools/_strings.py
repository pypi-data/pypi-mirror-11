...
def generate_axis_label_dict(var_dict=None, units_dict=None,
    ...
    for strtype in ('tex', 'txt'):
        if strtype in list(var_dict.keys()):
            axis_label_dict[strtype] = {}
            for var in var_dict[strtype].keys():
                if var in list(units_dict[strtype].keys()):
...
var_txtunits.update(dict.fromkeys(nanotube_spatial_vars, str(u'\u00c5')))
...
var_texstrings = {}
for varkey, varstr in var_txtstrings.items():
...
axis_texlabels = {}
axis_txtlabels = {}
for varkey in var_txtstrings.keys():
...
for varkey, var in var_txtstrings.items():
    if varkey in dimensionless_vars:
        label = var
    else:
