...
from pksci.version import short_version as version
    ...
    for findex, f in enumerate(txtlogs, start=1):
        if os.path.isfile(f):
            ...
            if xlsxfpath is not None:
                ...
                # write field names to first row columns
                for col_idx, field in enumerate(fields, start=1):
                    col = get_column_letter(col_idx)
                    ws.cell('%s%s' % (col, 1)).value = field

                # skip the field name column delimiter in second line of txtlog
                rows = lines[2:]

                # write field values for each column, row by row
                for row_idx, row in enumerate(rows, start=2):
                    values = re.split('\s{2,}', row.strip())
