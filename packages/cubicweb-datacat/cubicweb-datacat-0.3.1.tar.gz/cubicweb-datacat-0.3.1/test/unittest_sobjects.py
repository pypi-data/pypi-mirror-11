...
class RDFDCATFeedParserTC(CubicWebTC):
    """Functional test case for RDF DCAT data import."""

    def setup_database(self):
        """Set-up the database before testing.

        That is: create a new data source for the RDF DCAT feed parser.
        """
        self.dcat_uri = u'file://' + self.datapath('dcat.n3')
        with self.admin_access.repo_cnx() as cnx:
            cnx.create_entity('CWSource', name=u'test_rdf_catalog', type=u'datafeed',
                              parser=u'rdf.dcat', url=self.dcat_uri)
            cnx.commit()

    def pull_data(self, source_name):
        """Fetch data from the source identified by the given name."""
        dfsource = self.repo.sources_by_uri[source_name]
        with self.repo.internal_cnx() as cnx:
            stats = dfsource.pull_data(cnx, force=True, raise_on_error=True)
        return stats

    def test_rdf_dcat_import(self):
        """Check that import of RDF DCAT data works correctly."""
        dset_cwuri = u'file://' + self.datapath('dset1.n3')
        stats = self.pull_data('test_rdf_catalog')
        self.assertEqual(len(stats['created']), 12)
        self.assertEqual(len(stats['updated']), 0)
        with self.admin_access.client_cnx() as cnx:
            dset = cnx.find('Dataset', cwuri=dset_cwuri).one()
            self.assertEqual(dset.title, "Special Eurobarometer 376: Women in decision-making "
                             "positions")
            self.assertEqual(len(dset.in_catalog), 1)
            self.assertEqual(dset.in_catalog[0].cw_etype, 'DataCatalog')
            self.assertEqual(dset.in_catalog[0].cwuri, self.dcat_uri)
        # Check that importing same data again won't create new entities
        stats = self.pull_data('test_rdf_catalog')
        self.assertEqual(len(stats['created']), 0)
        self.assertEqual(len(stats['updated']), 12)
