...
class Imagefy(object):
    ...
    def upload(self, source_image, tags=None):
        ...
        files = {
            'image': (source_image.name, source_image),
        }
        data = {}
        if tags:
        ...
        try:
            ...
            if response.status_code == 201:
                data = response.json()
                return Image(data['id'], data['image'], data['created'], data['user'], data['tags'])
        except RequestException as e:
            raise ImagefyOperationException("Network error.", 'upload', original_exc=e)

    def modify(self, image, actions):
        """
        ...
        :return:
        """
        url = '%s%s/modified/' % (self.host, image.uuid)
        headers = {
            'Authorization': 'Token %s' % self.api_key,
        ...
        try:
            ...
            if response.status_code == 201:
                data = response.json()
                return ModifiedImage(data['id'], data['image'], data['created'], data['source'], data['settings'])
        except RequestException as e:
