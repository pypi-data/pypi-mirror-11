...
def processDataSection(data_line, lines, i, localcontext, globalcontext):
    ...
    if isinstance(res.element.inline_content, OrderedDict):
...
class Template(object):
    ...
    def getResourcePath(self, s, l, t):
        host = self.globalcontext['_resources'][t.resourcename]['host']
        port = self.globalcontext['_resources'][t.resourcename]['port']
        username = self.globalcontext['_resources'][t.resourcename]['username']
        password = self.globalcontext['_resources'][t.resourcename]['password']
        hostport = "%s:%s" % (host, port)
        r = zml.rest.Resource(hostport, username, password)
        restdata = r.get('/blog/_design/app/_view/posts')
        return restdata
