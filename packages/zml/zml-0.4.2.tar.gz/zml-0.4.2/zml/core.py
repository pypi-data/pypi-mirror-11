...
attributevalue = qs | Word(printables, excludeChars=" ")
...
class Node(OrderedDict):
    ...
    def __setattr__(self, key, value):
        if key in self.__dict__['__oditems']:
            ...
            internal = ['_root', '_inited', '_db',
                        '_silent', '__oditems', '_parent', '_name']
            if key in internal:
                self.__dict__[key] = value
            else:
...
def processDataSection(lines, i, context):
    datamode = True
    data_line = indentobj(
        ...
        for i, line in enumerate(lines):
            ...
            if code_block_active:
                ...
                if line.strip().startswith('#'):
                    ...
                    self.jump_to = processDataSection(lines, i, self.context)
                    c = Node(self.context)
                    globals().update(c)
                elif line.strip().startswith('%'):
