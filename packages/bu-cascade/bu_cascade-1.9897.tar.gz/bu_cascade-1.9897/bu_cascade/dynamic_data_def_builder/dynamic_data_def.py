...
%(3)s</li>\n""" % {'0': identifier, '1': i, '2': value, '3': indent, '4': indent + "    ", '5': parameters}
    ...
    def recursively_build_metadata_dict(self, typed_dict):
        to_return = {}
        types_not_recurred_on = [sudsText, dateTime, bool, tuple, list]
        for key, val in typed_dict:
            if val is not None and key != "structuredDataNodes" and key != "lastModifiedDate":
...
""" % {'0': styling, '1': scripts, '2': ckeditor_path}
        ...
        self.my_form += "    "*3 + "<input type=\"hidden\" id=\"prebuilt_dict\" value=\"%(0)s\"/>" %{'0': str(self.storage)}
        self.my_form += "        </div>"
