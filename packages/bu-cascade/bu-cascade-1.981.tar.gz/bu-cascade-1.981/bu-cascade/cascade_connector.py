...
# from bu-cascade.dynamic_data_def_builder.fetcher import fetch_array
...
class Cascade(object):
    ...
    def get_form_from_data_def(self, data_def_id, user_name):
        if not self.dynamo:
            self.dynamo = DataDefinitionBuilder(self, user_name)
        return self.dynamo.build_data_def(data_def_id)

    def edit_data_def(self, page_id, page_type, user_name):
        if not self.dynamo:
            self.dynamo = DataDefinitionBuilder(self, user_name)
        return self.dynamo.edit_page_built_from_data_def(page_id, page_type)

    # AJAX methods to be called from Tinker; the commented routes should be added above filler methods that in turn call
    # these ones. This makes Cascade remain Flask-free while still having AJAX capabilities.

    # @app.route("/fetch/<username>", methods=['POST'])
    # @app.route("/fetch/<username>/<path:called>", methods=['POST'])
    def get_cascade_folder(self, username, called=""):
        return self.dynamo.alias_fetch_array(username, called)

    # @app.route("/submit", methods=['POST'])
    def submit_part_2(self, form_contents):
        self.dynamo.submit_thineself(form_contents)
