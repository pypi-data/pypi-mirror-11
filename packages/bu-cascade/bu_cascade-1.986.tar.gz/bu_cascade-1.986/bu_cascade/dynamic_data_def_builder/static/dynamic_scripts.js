...
function fetchArrayFromCascade(path, typeOfFile, idOfButtonThatCalledMe) {
...
function constructInternalDisplay(typeOfFileToChoose, idOfButtonThatCalledFetch) {
    ...
    for (var i = 1; i < arrayOfContents.length; i++) {
        ...
        if (thisType == 'folder') {
            ...
            subfolder.setAttribute("onclick", "fetchArrayFromCascade(\'" + filepath + "/" + thisName + "\',\'"
                + typeOfFileToChoose + "\', \'" + idOfButtonThatCalledFetch + "\');");
...
function submitForm(){
    ...
    for(var i = 0; i < formHead.elements.length; i++){
        ...
        if(id.contains("addTo") || id.contains("removeFrom") || id == "") {}
            ...
            array_of_results.append(id + "=" + val);
        }
    }
