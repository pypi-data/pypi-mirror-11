 ...
 */
//todo: use indexOf as a contains method here http://stackoverflow.com/questions/1789945/how-can-i-check-if-one-string-contains-another-substring
...
function fetchArrayFromCascade(path, typeOfFile, idOfButtonThatCalledMe, username) {
    //Display loading icon now
    //alert("Fetch clicked with args: path=\""+path+"\", typeOfFile=\""+typeOfFile
    //    +"\", idOfButtonThatCalledMe=\""+idOfButtonThatCalledMe+"\", username=\""+username+"\"");
    var urlToSend = "fetch/"+username;
    if (path != "") {
        urlToSend += path;
    }
    alert("Sending URL: "+urlToSend);
    $.ajax({
        type: "POST",
        url: urlToSend,
        data: { uname: username, path: path }
    }).done(function (response) {
        arrayOfContents = JSON.parse(response);
        alert(arrayOfContents);

        if (arrayOfContents.length == 0) {
            alert("Empty folder");
        }
        else {
            constructInternalDisplay(typeOfFile, idOfButtonThatCalledMe);
        }
        //Stop the loading icon now?
    });
}
...
function constructInternalDisplay(typeOfFileToChoose, idOfButtonThatCalledFetch) {
    ...
    goBack.setAttribute("onclick", "fetchArrayFromCascade(\'" + parent_path + "\',\'" + typeOfFileToChoose + "\',\'" + idOfButtonThatCalledFetch + "\');");
    ...
    for (var i = 1; i < arrayOfContents.length; i++) {
        ...
        //alert("Have: "+thisType+"; want: "+typeOfFileToChoose);
        if (thisType == 'folder') {
            //It's a subfolder
            var subfolder = document.createElement("button");
            subfolder.innerHTML = "+";
            subfolder.style = "float:right";
            subfolder.setAttribute("onclick", "fetchArrayFromCascade(\'" + filepath + "/" + thisName + "\',\'" + typeOfFileToChoose + "\',\'" + idOfButtonThatCalledFetch + "\');");
...
function submitForm(){
    ...
    alert(formHead.elements.length);
    var string_version = "";
    var id, val;
    for(var i = 0; i < formHead.elements.length; i++){
        ...
        if(id.contains("addTo") || id.contains("removeFrom") || id == "") {}
            ...
            //string_version += id + "=" + val + "`\n";
            string_version += "(\"" + id + "\", \"" + val + "\"),\n";
        }
    }
    alert(string_version);
}
