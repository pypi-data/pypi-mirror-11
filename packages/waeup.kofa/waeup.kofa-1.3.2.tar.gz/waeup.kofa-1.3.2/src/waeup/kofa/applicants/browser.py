## $Id: browser.py 13254 2015-09-09 13:06:31Z henrik $
##
...
class ApplicantCheckStatusPage(KofaPage):
    ...
    def label(self):
        if self.result:
            return _('Application status of ${a}',
                     mapping = {'a':self.applicant.applicant_id})
        return _('Check your application status')
    ...
    def update(self, SUBMIT=None):
        ...
        if SUBMIT:
            if not self.captcha_result.is_valid:
                ...
                # No need to flash something.
                return
            applicant_id = form.get('applicant_id', None)
            lastname = form.get('lastname', None)
            if not applicant_id or not lastname:
                self.flash(
            ...
            if results:
                applicant = results[0]
                if applicant.lastname.lower() != lastname.lower():
                    # Don't tell the truth here. Anonymous must not
                    # know that a record was found and only the lastname
                    # verification failed.
                    self.flash(
