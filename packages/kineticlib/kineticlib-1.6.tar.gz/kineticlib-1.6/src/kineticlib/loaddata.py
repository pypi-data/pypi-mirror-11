...
def load_dissociation_parameters(molecule: Molecule, partner: Particle) -> np.ndarray:
    ...
    **Molecule + Partner -> Atom1 + Atom2 + Partner**.
    ...
    np.ndarray
        An array, which contains the following quantities:
        result[0] - Arrhenius model n parameter (values given by Chul Park et al.)
        result[1] - Arrhenius model A parameter (values given by Chul Park et al.)
        result[3] - Arrhenius model n parameter (values given by Thomas J. Scanlon et al.)
        result[4] - Arrhenius model A parameter (values given by Thomas J. Scanlon et al.)
