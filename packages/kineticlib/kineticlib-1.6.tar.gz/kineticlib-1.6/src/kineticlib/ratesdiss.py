...
def k_diss_eq(T: float, model_data: np.ndarray, molecule: Molecule, diss_data: str='Scanlon') -> float:
    ...
    diss_data : str
        the values of the Arrhenius parameters which to use, refers to the first author of the paper with the
        given values, possible values:

            * ``'Park'`` - the Arrhenius parameters given in **Review of Chemical-Kinetic Problems of Future
            NASA Missions, II: Mars Entires** by Chul Park et al. will be used
            * ``'Scanlon'`` - the Arrhenius parameters given in **Open Source DSMC Chemistry Modelling
            for Hypersonic Flows** by Thomas J. Scanlon et al. will be used
    ...
    """
    if diss_data == 'Scanlon':
        return raw_k_diss_eq(T, model_data[2], model_data[3], molecule.diss)
    else:
...
def diss_rate_treanor_marrone_sts(T: float, model_data: np.ndarray, molecule: Molecule, i: int,
                                  model: str='D6k', diss_data: str='Scanlon') -> float:
    ...
    """
    if diss_data == 'Scanlon':
        return raw_diss_rate_treanor_marrone_sts(T, model_data[2], model_data[3], molecule.Z_diss(T, i, model),
                                                 molecule.diss)
    else:
...
def rec_rate_treanor_marrone_sts(T: float, model_data: np.ndarray, molecule: Molecule, atom1: Atom, atom2: Atom,
                                 i: int, model: str='D6k', diss_data: str='Scanlon') -> float:
    ...
    if isinstance(molecule, MoleculeMultiT):
        return raw_rec_rate_treanor_marrone_sts(T, c1, c2, molecule.Z_diss(T, i, model),
    ...
    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
            res += molecule.vibr_exp(T, T1, i) \
                   * raw_diss_rate_treanor_marrone_sts(T, c1, c2,
...
def rec_rate_treanor_marrone(T: float, T1: float, model_data: np.ndarray, molecule: MoleculeQuasi,
                             atom1: Atom, atom2: Atom, model: str='D6k', diss_data: str='Scanlon') -> float:
    ...
    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
            res += raw_rec_rate_treanor_marrone_sts(T, c1, c2,
