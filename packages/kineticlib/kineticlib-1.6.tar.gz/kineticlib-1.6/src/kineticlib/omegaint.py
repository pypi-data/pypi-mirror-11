    ...
    model : str
        the interaction model to be used, possible values:
            ...
            * 'Switch' (returns the result for the Lennard-Jones model when :math:`T / \\varepsilon_{cd} < 10`
              and for the exponential repulsive potential otherwise)
...
def omega_dimensionless_BM(T: float, l: int, sigma: float, eps: float, phizero: float, beta: float) -> float:
    """Returns the dimensionless :math:`\\Omega^{(l,l)}`-integral for the exponential-repuslive
    potential (for ``l=1`` and ``l=2``)
    ...
    eps : float
        the Lennard-Jones :math:`\\varepsilon_{cd}` parameter, divided by Boltzmann's constant
    phizero : float
        the exponential repulsive potential :math:`\\phi_{0}` parameter, divided by Boltzmann's constant
    beta : float
    ...
    -------
    float
        The dimensionless calculated exponential repulsive omega integral
    ...
    if l == 1:
        ...
        raise KineticlibError('Cannot calculate the Omega^{(' + str(l) + ',' + str(l)
                              + ')} integral using the exponential repulsive potential potential')
    Abig[:] = a[:, 0] + (a[:, 1] + a[:, 2] / log(vstar / 10)
                         + a[:, 3] / ((log(vstar / 10)) ** 2)) / ((rstar * log(vstar / 10)) ** 2)
    return (coeff + Abig[0] / (TT ** pows[0]) + Abig[1] / (TT ** pows[1]) + Abig[2] / (TT ** pows[2])) *\
           ((rstar * log(vstar / Tstar)) ** 2)
