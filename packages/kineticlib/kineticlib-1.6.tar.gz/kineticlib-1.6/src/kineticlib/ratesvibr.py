...
def vv_rate_ssh(T: float, idata: np.ndarray, molecule: Molecule,
    ...
    if i_delta == -1:
        ...
        return raw_vv_rate_ssh_harmonic_up(T, idata[1], molecule.mass, molecule.vibr[i],
...
def raw_vv_rate_ssh_harmonic_down(T: float, sigma: float, mass: float, hvc: float, i: int, k: int) -> float:
    ...
    """
    return i * (k + 1) * 2. * sigma**2 * ((4 * constants.pi * constants.k * T / mass) ** 0.5)\
...
def raw_vv_rate_ssh_harmonic_up(T: float, sigma: float, mass: float,
    ...
    mass : float
        the mass of the molecule
    ve_before1 : float
        the vibrational energy of the first molecule before the transition
    ve_after1 : float
        the vibrational energy of the first molecule after the transition
    ve_before2 : float
        the vibrational energy of the second molecule before the transition
    ve_after2 : float
        the vibrational energy of the second molecule after the transition
    hvc : float
        the harmonic coefficient in the formula for vibrational level energies
    i : int
        the vibrational level of the first molecule
