...
class DB(object):
    ...
    def get_backend_list(self, balancer_name):
        ...
        for b_id in b_ids:
            ...
            base_key = '%s:%s' % (BackendModel.__prefix_key__, b_id)
            for k in self._get_class_members(BackendModel):
                setattr(backend, k, self.redis.get('%s:%s' % (base_key, k)))
    ...
    def get_entrypoint_list(self, balancer_name):
        ...
        for e_id in e_ids:
            ...
            base_key = '%s:%s' % (EntrypointModel.__prefix_key__, e_id)
    ...
    def save_balancer(self, balancer):
        if isinstance(balancer, BalancerModel):
            ...
            for backend in balancer.backends:
                self.save_backend(backend, balancer.name)
            for entrypoint in balancer.entrypoints:
                self.save_entrypoint(entrypoint, balancer.name)
    ...
    def save_backend(self, backend, balancer_name):
        if isinstance(backend, BackendModel) and self.redis.sismember(
            ...
            for k, v in backend.as_dict().items():
                self.redis.set(('%s:%s:%s:%s' % (
                    ...
                    backend.port, k)), v)
                backend_key = '%s:%s' % (balancer_name, backend.__prefix_key__)
                self.redis.sadd(backend_key, ('%s:%s' % (
                    balancer_name, backend.port)))
            latest_version = str(datetime.datetime.now)
            self.redis.publish(Config.LISTEN_REDIS_CHANNEL,  latest_version)
            self.redis.set(Config.LATEST_VERSION, latest_version)
            return backend

    def save_entrypoint(self, entrypoint, balancer_name):
        if isinstance(entrypoint, EntrypointModel) and self.redis.sismember(
                BalancerModel.__prefix_key__, balancer_name):
            for k, v in entrypoint.as_dict().items():
                self.redis.set(('%s:%s:%s:%s:%s' % (
                    entrypoint.__prefix_key__, balancer_name,
                    entrypoint.domain, entrypoint.port, k)), v)
                entrypoint_key = '%s:%s' % (balancer_name,
                                            entrypoint.__prefix_key__)
                self.redis.sadd(entrypoint_key, ('%s:%s:%s' % (
                    balancer_name, entrypoint.domain, entrypoint.port)))
            latest_version = str(datetime.datetime.now)
            self.redis.publish(Config.LISTEN_REDIS_CHANNEL,  latest_version)
            self.redis.set(Config.LATEST_VERSION, latest_version)
            return entrypoint
    ...
    def delete_balancer(self, balancer_name):
        if self.redis.sismember(BalancerModel.__prefix_key__, balancer_name):
            ...
            self.delete_backend(balancer_name)
    ...
    def delete_backend(self, balancer_name):
        if self.redis.sismember(BalancerModel.__prefix_key__, balancer_name):
            backends_key = '%s:%s' % (
                balancer_name, BackendModel.__prefix_key__)
            base_keys = self.redis.smembers(backends_key)
            for base_key in base_keys:
                self._delete_end(BackendModel, '%s:%s' % (
                    BackendModel.__prefix_key__, base_key))
    ...
    def delete_backend_by_port(self, balancer_name, port):
        backends_key = '%s:%s' % (balancer_name, BackendModel.__prefix_key__)
        key = '%s:%s' % (balancer_name, port)
        if self.redis.srem(backends_key, key):
            self._delete_end(BackendModel, '%s:%s' % (
                BalancerModel.__prefix_key__, key))
            latest_version = str(datetime.datetime.now)
            self.redis.publish(Config.LISTEN_REDIS_CHANNEL,  latest_version)
            self.redis.set(Config.LATEST_VERSION, latest_version)
            return True
    ...
    def delete_entrypoint(self, balancer_name):
        if self.redis.sismember(BalancerModel.__prefix_key__, balancer_name):
            entrypoints_key = '%s:%s' % (
                balancer_name, EntrypointModel.__prefix_key__)
            base_keys = self.redis.smembers(entrypoints_key)
            for base_key in base_keys:
                self._delete_end(EntrypointModel, '%s:%s' % (
                    EntrypointModel.__prefix_key__, base_key))
