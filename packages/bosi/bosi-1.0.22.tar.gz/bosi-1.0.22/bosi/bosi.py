...
def deploy_bcf(config, fuel_cluster_id, rhosp, tag, cleanup):
    ...
    sorted_time_dict = sorted(time_dict.items(), key=operator.itemgetter(1))
    for hostname in sorted_time_dict:
        Helper.safe_print("node: %s, time:%.2f, last_log:%s", %
                          (hostname, sorted_time_dict[hostname], node_dict[hostname].last_log))
