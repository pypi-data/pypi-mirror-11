...
def wrapper(args):
    ...
    df.to_string(outloc, index=False,col_space=10,float_format=utils.format_string)

# Connects argparse to wrapper
def add_subparser(subparsers):
    p = subparsers.add_parser('simulate_evaluate')
    p.add_argument('-m', '--modeltype', type=str,choices=['RandomLinear','LinearEmat',
        'Custom'],help ='Type of Model to use')
    p.add_argument('-mp', '--modelparam', default=None, \
        help='''Parameters should be entered as a list, with RandomLinear=[LengthofSeq],LinearEmat=[FileName]
        . If you are using a custom model enter a list of parameters with the first parameter
