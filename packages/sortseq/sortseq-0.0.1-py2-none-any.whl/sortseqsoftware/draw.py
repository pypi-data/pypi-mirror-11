...
'''This script plots tables as a pdf or png. The type of plot is determined
...
def draw_from_to(df,seq_dict,inv_dict):
    '''Plot probabilities of mutation of 1 base to another.'''
    L = len(seq_dict)
...
def draw_enrichment_profile(df,seq_dict,inv_dict):
    '''For proteins, make a plot of enrichment of each residue at each position.'''
    fig,ax = plt.subplots()
    column_headers = ['le_' + str(inv_dict[i]) for i in range(len(seq_dict))]
    #set special color for NaN values
    masked_array = np.ma.array (df[column_headers], mask=np.isnan(df[column_headers]))
...
def draw_logo_from_matrix(df,seq_dict,inv_dict):
    ...
    '''We need to supply a scalar to determine pssm total info, we want total info
        content of approx 1 bit per basepair, so we will minimize info - L'''
    optimal_x = newton(PSSM_info,x0=19,args=(matrix,))
    pssm_df = pd.DataFrame(np.transpose(utils.get_PSSM_from_weight_matrix(matrix,optimal_x)),columns=rename_headers)
...
def draw_pairwise_info(df,seq_dict):
    '''Display mutual information between a base being mutated at one position
        and a base being mutated at another.'''
...
def draw_compare_predictive_info(df,title=None):
    try:
    ...
    L = len(expnames)
    fig, ax = plt.subplots()
    heatmap = ax.pcolor(normalized_df, cmap=plt.cm.coolwarm)
    fig.set_figheight(10)
    fig.set_figwidth(10)
    ...
    try: 
        ax.set_xticklabels([expnames[z] for z in range(L)], minor=False)
        ax.set_yticklabels([expnames[z] for z in range(L)], minor=False)
    except:
        pass
    if title:
...
def main(df,dicttype,logo=False,title=None):
    debug()
    seq_dict,inv_dict = utils.choose_dict(dicttype)
    matrix_headers = ['val_' + inv_dict[i] for i in range(len(seq_dict))]
    columns = df.columns
    '''some functions can be output through plt.savefig, others must be written'''
    ...
    if {'ct','seq'}.issubset(columns):
        ...
        myimage = draw_compare_predictive_info(df,title=title)
...
def wrapper(args):
    ...
    myimage,output_via_write = main(df,dicttype,logo=args.logo,title=args.title)
