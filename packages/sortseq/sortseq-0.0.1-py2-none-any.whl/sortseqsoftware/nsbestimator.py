...
This script is a python version of Mathematica functions by Christian Mendl, with a python implementation by Sungho Hong, and altered by Bill Ireland, implementing the Nemenman-Shafee-Bialek (NSB) estimator of entropy.
...
def _Si(w, nxkx, N, K): #This caluclates p(B|c)*<H_q|c,beta> This is the integrand for the nsb mean entropy estimation
  sbeta = w/(1-w)
  beta = sbeta*sbeta
...
def S(x, N, K): 
  """
  Return the estimated entropy. x is a vector of counts, nxkx is the histogram of the input histogram constructed by make_nxkx. N is the total number of elements, and K is the degree of freedom.
...
def nsb_integrand_variance_term1(beta,nxkx,N,m): 
    '''computer variance of the nsb esimator, with term 1,2  names refering to eq 3.34 from kinney thesis, the third term, which is simply the square of the expectation value of the
