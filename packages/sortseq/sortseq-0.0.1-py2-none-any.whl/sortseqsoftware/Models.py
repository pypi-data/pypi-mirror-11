...
class LinearModel(ExpModel):
    ''' This model generates energies of binding through a linear energy matrix model'''
    def __init__(self,param,dicttype):
        ...
        if isinstance(param,str):
            ...
            self.matrix = np.transpose(np.array(param[headers]))
        else:
            raise ValueError('Linear Model Input is not of correct type. Enter File name of matrix (with header) or a matrix.')
    ...
    def genexp(self,seqs):
        ...
        if self.matrix.shape[0] == len(self.seq_dict)-1:
            for i,s in enumerate(seqs):
                 energies[i] = np.sum(self.matrix*utils.parameterize_seq(s,self.seq_dict))
...
class RaveledModel(ExpModel):
    '''This model type generates an energy matrix model, but accepts flattened matrices (for sklearn based fitting)'''
    def __init__(self,param):
          ...
          self.matrix = param
          '''
          if type(param) == sp.sparse.csr.csr_matrix:
               self.matrix = param
          else:
               raise ValueError('Linear Model Input is not of correct type. Enter File name of matrix (with header) or a matrix.')
          '''
...
class CustomModel(ExpModel):
    def __init__(self,mp=''):
        ...
        try:
            self.model = importlib.import_module(mp[0]) #import the modeltype function
        except:
            raise ValueError('Input Custom Model does not exist. Make sure it is a python function of the name you listed')
    def genexp(self,seqs):
...
class LogNormalNoise(NoiseModel):
    '''Noise model that adds autoflourescence and then draws the noisy measurement from a log normal distribution with this mean'''
...
class NormalNoise(NoiseModel):
    ...
    def gennoisyexp(self,logexp):
        nexp = logexp + np.random.normal(scale=(np.abs(logexp).mean())*self.scale,size=len(logexp))
        return nexp
