...
def main(df,T_Counts,timepoints,beta):
...
def wrapper(args):
    ...
    header = df.columns
    output_df = main(df,T_Counts,timepoints,beta)
    if args.out:
...
def add_subparser(subparsers):
    ...
    p.add_argument('-tp','--timepoints',type=int,default=2,help='''Number of timepoints''')
    p.add_argument('-beta','--beta',default=None,help='''Selection Constant''')
    p.add_argument('-C', '--totalcounts', type=int,default=1000000,help ='Number of Sequencing Counts')
