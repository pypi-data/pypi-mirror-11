...
def main(df,dicttype):
    ...
    for z in range(len(strings[i])):
        wtarr[z] = np.argmax(np.histogram(seq_arr[:,z],bins=range(len(seq_dict)+1),weights=df['ct'])[0])
    seqL = len(strings[0])
    ...
    for i in range(len(strings[0])):
        ...
        for z in range(i,len(strings[0])):
            ...
            for q in range(2):
                ...
                partialent = partialent + len(partialmut)/len(strings)*nsb.S(np.array(mutcounts),np.sum(mutcounts),len(mutcounts))
...
def wrapper(args):    
    # Run funciton
    seqs_df = pd.io.parsers.read_csv(sys.stdin,delimiter='\t',usecols=['seq','ct'])
