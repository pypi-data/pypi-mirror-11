...
def main(df,numsub=200,numcopies=1000,decay_constant = 100,disttype='constant'):
    ...
    output_df = pd.DataFrame(np.random.choice(df['seqs'],size=numsub,p=parr),columns=['seqs'])
    
    if disttype == 'constant':
        output_df['ct'] = [numcopies for z in range(len(output_df['seqs']))]
    return output_df
...
def wrapper(args):
    ...
    # Run funciton
    output_df = main(df,numsub=200,numcopies=1000,decay_constant = 100,disttype='constant')
    ...
    output_df.to_string(outloc, index=False, columns=['seqs','ct'],col_space=10,float_format=utils.format_string)

# Connects argparse to wrapper
def add_subparser(subparsers):
    p = subparsers.add_parser('simulate_sublibrary')
    
    p.add_argument('-d','--disttype',default='constant',choices = ['constant', 'exponential'], 
            help ='''how should the number of times each sequence appears be distributed?''')
    p.add_argument('-ns','--numsub',type=int,default=200,help='''Number of 
            Different SubLib Sequences''')
    p.add_argument('-nc','--numcopies',type=int,default=1e3,help='''Number of 
            times to measure a given sequence, if an exponential distrubtion 
            type is selected. This will be the maximum number of seqs''')
    p.add_argument('-dc','--decayconstant',type=float,default=100,help='''If 
            exponential distribution is selected, the decay constant of the distribution''')
