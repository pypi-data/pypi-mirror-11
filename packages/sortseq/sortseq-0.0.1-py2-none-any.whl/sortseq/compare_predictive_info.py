...
def main(data_dfs,model_dfs,dataname,dicttype,exptype=None,start=0,end=None):
    ...
    lengths = lengths + [len(data_dfs[z]['seq'][0]) for z in range(nummodels)]
    ...
    for z, model_df in enumerate(model_dfs):
        for q, data_df in enumerate(data_dfs):            
            MI[z,q],V = predictiveinfo.main(data_df,model_df,dicttype=dicttype,exptype=exptype,modeltype='LinearEmat',start=start,end=end)
...
def wrapper(args):
    ...
    output_df = main(dfs,mymodels,dataname,dicttype,exptype=args.exptype,start=args.start,end=args.end)
