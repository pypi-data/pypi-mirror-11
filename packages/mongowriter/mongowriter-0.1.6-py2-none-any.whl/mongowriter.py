...
@retry_on_exception(retries=14, exc=AutoReconnect, pause=5)
def init_client(**client_kwargs):
    return MongoReplicaSetClient(**client_kwargs)


@retry_on_exception(retries=14, exc=OperationFailure, pause=5)
...
class MongoWriter(object):
    ...
    def run(self):
        while not (self.stopping and self.queue.empty()):
            try:
