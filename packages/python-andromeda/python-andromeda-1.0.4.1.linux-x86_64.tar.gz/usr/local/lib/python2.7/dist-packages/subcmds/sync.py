...
"""
    ...
    def _Options(self, p, show_smart=True):
        ...
        p.add_option('--no-clone-bundle',
                     ...
                     help='isable use of /clone.bundle on HTTP/HTTPS')
        ...
        #TODO verify these can be refactored.
        g = p.add_option_group('andromeda Version options')
        g.add_option('--no-repo-verify',
                     dest='no_repo_verify', action='store_true',
                     help='Do not verify andromeda source code')
...
def _PostRepoFetch(rp, no_repo_verify=False, verbose=False):
    if rp.HasChanges:
        print('info: A new version of andromeda is available', file=sys.stderr)
        print(file=sys.stderr)
        if no_repo_verify or _VerifyTag(rp):
            ...
            if not syncbuf.Finish():
                sys.exit(1)
            print('info: Restarting andromeda with latest version', file=sys.stderr)
            raise RepoChangedException(['--repo-upgraded'])
        else:
            print('warning: Skipped upgrade to unverified version', file=sys.stderr)
    else:
        if verbose:
            print('andromeda version %s is current' % rp.work_git.describe(HEAD),
                  file=sys.stderr)

#TODO yank out GPG.
def _VerifyTag(project):
    gpg_dir = os.path.expanduser('~/.andromedaconfig/gnupg')
    if not os.path.exists(gpg_dir):
        print('warning: GnuPG was not available during last "andromeda init"\n'
              'warning: Cannot automatically authenticate andromeda."""',
              file=sys.stderr)
        return True
...
class _FetchTimes(object):
    ...
    def __init__(self, manifest):
        self._path = os.path.join(manifest.repodir, '.andromeda_fetchtimes.json')
        self._times = None
        self._seen = set()
