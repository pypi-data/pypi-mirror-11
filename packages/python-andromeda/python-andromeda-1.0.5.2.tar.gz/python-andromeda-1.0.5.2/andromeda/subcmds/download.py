...
"""
    ...
    def Execute(self, opt, args):
        for project, change_id, ps_id in self._ParseChangeIds(args):
            ...
            if len(dl.commits) > 1:
                print('[%s] %d/%d depends on %d unmerged changes:' % (project.name, change_id, ps_id, len(dl.commits)),
                      file=sys.stderr)
                for c in dl.commits:
            ...
            if opt.cherrypick:
                try:
                    ...
                    print('[%s] Could not complete the cherry-pick of %s' % (project.name, dl.commit), file=sys.stderr)
                    sys.exit(1)
