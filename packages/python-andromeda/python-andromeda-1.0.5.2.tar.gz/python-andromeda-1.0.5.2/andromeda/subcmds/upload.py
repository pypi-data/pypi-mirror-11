...
%prog [--re --cc] [<project>]...
"""
    # TODO, need to update this to reflect the OpenStack documentation.
    helpDescription = """
The '%prog' command is used to send changes to the OpenStack
...
branches are found, '%prog' opens an editor to allow you to
...
with the OpenStack Review system, or the upload will fail.
...
These are read from each '.git/config' file the OpenStack repositories by default,
but can be changed globally. To change this globally,
...
and will not prompt you further.  If it is set to "false" then andromeda
...
Override the username used to connect to OpenStack Review.
By default the local part of the email address is used.

The URL must match the review URL listed in the manifest XML file,
or in the .git/config within this project.  For example:
...
"""
    ...
    def _Options(self, p):
        p.add_option('-t',
                     ...
                     help='Send local branch name to OpenStack Review.')
            ...
            for branch in todo:
                if not branch.uploaded:
                    ...
                    print(('[FAILED] %-15s %-15s' + fmt) % (
                        branch.project.relpath + '/', branch.name, str(branch.error)),
                        file=sys.stderr)
            print()
