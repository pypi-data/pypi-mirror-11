...
def _ProjectHooks():
    ...
    'andromeda/hooks' directory) on the first call.
...
class RepoHook(object):
    ...
    def _CheckForHookApproval(self):
        ...
        git_approval_key = 'andromeda.hooks.%s.approvedhash' % self._hook_type
        ...
        if sys.stdout.isatty():
            prompt += ('Andromeda %s run the script:\n'
                       '  %s\n'
                       '\n'
    ...
    def Run(self, user_allows_all_hooks, **kwargs):
        ...
        if not os.path.isfile(self._script_fullpath):
            raise HookError('Couldn\'t find andromeda hook: "%s"' % self._script_fullpath)
...
class Project(object):
    ...
    def ReviewName(self):
        '''
        Obtain the user's Gerrit login because it may not be their email.
        '''
        if self._reviewident_name is None:
            # TODO test this.
            self._LoadReviewIdentity()
        return self._reviewident_name

    def _LoadReviewIdentity(self):
        '''
        This will parse the ~/.gitconfig file for
        [review]
        ...
        if self._reviewident_name is None:
                self._reviewident_name = config.get('gitreview', 'username')
        return self._reviewident_name
    ...
    def PrintWorkTreeStatus(self, output_redir=None):
        ...
        if not os.path.isdir(self.worktree):
            ...
            print('  missing (run "andromeda sync")', file=output_redir)
            return
    ...
    def UploadForReview(self, branch=None,
        ...
        url = branch.remote.ReviewUrl(self.ReviewName)
        if url is None:
    ...
    def _RemoteFetch(self, name=None,
        ...
        if not self.manifest.IsMirror:
            if self.clone_depth:
                ...
                depth = self.manifest.manifestProject.config.GetString('andromeda.depth')
            # The repo project should never be synced with partial depth
            if self.relpath == '.andromeda/repo':
                depth = None
    ...
    def _InitGitDir(self, mirror_git=None):
        if not os.path.exists(self.gitdir):
            ...
            ref_dir = mp.config.GetString('andromeda.reference') or ''

            if ref_dir or mirror_git:
                if not mirror_git:
                    mirror_git = os.path.join(ref_dir, self.name + '.git')
                repo_git = os.path.join(ref_dir, '.andromeda', 'projects',
                                        self.relpath + '.git')
...
# TODO update comment strings.
class MetaProject(Project):
    """A special project housed under .andromeda.
    """
