...
class LogPlayer:
    ...
    def start(self):
        ...
        print "%s queries ran faster, %s queries ran slower" % (self.faster, self.slower)
...
def main():
    ...
    p = Pool(len(inputs))
    try:
        # pair every input file with a copy of args
        params = zip([args,]*len(inputs), inputs)
        results = p.map(make_player, params, chunksize=1)
        print results
