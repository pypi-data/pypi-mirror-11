...
def save_document_to_file(document, filepath=''):
    """
    ...
    >>> import os
    >>> simple_list = [1,2,3]
    >>> save_document_to_file(simple_list, './test.pickle')
    './test.pickle'
    >>> read_simple_list = read_document_from_file('./test.pickle')
    ...
    # If no filepath was provided, use a random filename in ./tmp
    if actual_filepath == '':
        ...
        # Create filepath
        actual_filepath = os.path.join(os.path.dirname(__file__), 'tmp', filename)
