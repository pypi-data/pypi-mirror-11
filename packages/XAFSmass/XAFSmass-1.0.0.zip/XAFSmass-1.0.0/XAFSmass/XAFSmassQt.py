...
examples = (
    "Fe2(SO4)3  <i>or</i>  Fe%2((Al2O3)%10SiO2)",
...
#        fig.set_edgecolor(cl)
    ...
    def update_formula(self, formula=None):
        self.fig.clf()
        self.fig.suptitle(formula, x=0.5, y=0.5, ha='center', va='center',
    ...
    def plot(self, compound, E, table):
        ...
        if compound is None:
            ...
            if isinstance(ced, str):
                QtGui.QMessageBox.critical(self, "Error", ced)
                return
            elementsDict = ced[0]
...
class MainDlg(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.whatLabel = QtGui.QLabel(r"&sample:")
        self.whatCB = QtGui.QComboBox()
        self.whatCB.addItems(whats)
        self.connect(self.whatCB,
        ...
        self.compoundLabel = QtGui.QLabel(r"&compound:")
        self.compoundExLabel = QtGui.QLabel("")
        self.compoundEdit = QtGui.QLineEdit()
        self.compoundLabel.setBuddy(self.compoundEdit)

        self.compoundMassLabel = QtGui.QLabel("M (g/mol) = ")
        self.compoundMass = QtGui.QLabel("")

        self.muTdLabel = QtGui.QLabel("")
        self.muTdEdit = QtGui.QLineEdit()
#        font = QtGui.QFont(self.font())
#        font.setPointSize(font.pointSize()+2)
#        fm = QtGui.QFontMetrics(font)
#        self.muTdEdit.setMinimumSize(fm.width("8.88"), fm.height())
        self.areaLabel = QtGui.QLabel("")
        self.areaEdit = QtGui.QLineEdit()
#        self.areaEdit.setMinimumSize(fm.width("8.88"), fm.height())
        self.dmudLabel = QtGui.QLabel(u"δµd = ")
        self.dmudEdit = QtGui.QLineEdit()
#        self.dmudEdit.setMinimumSize(fm.width("8.888"), fm.height())

        self.energyLabel = QtGui.QLabel(r"&E (eV) =")
        self.energyCB = QtGui.QComboBox()
        self.read_energies()
        ...
        self.tableLabel = QtGui.QLabel(r"&data table:")
        self.tableCB = QtGui.QComboBox()
        self.tableCB.addItems(tables)
        self.tableCB.currentIndexChanged.connect(self.calculate)
        self.tableLabel.setBuddy(self.tableCB)
        self.tablePlotButton = QtGui.QPushButton('&Plot f"')
        if not MAC:
            self.tablePlotButton.setFocusPolicy(QtCore.Qt.NoFocus)

        self.connect(
            self.tablePlotButton, QtCore.SIGNAL("clicked()"), self.plotf)

        self.resNuLabel = QtGui.QLabel("")
        self.resNu = QtGui.QLabel("")
        self.resMassLabel = QtGui.QLabel("")
        self.resMass = QtGui.QLabel("")

        self.stepLabel = QtGui.QLabel("absorptance step = ")
        self.stepCB = QtGui.QComboBox()
        self.stepCB.setSizePolicy(
            QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Fixed)
#        self.stepCB.setMinimumWidth(200)

        self.extraRhoLabel = QtGui.QLabel(u"ρ (g/cm<sup>3</sup>) = ")
        self.extraRhoEdit = QtGui.QLineEdit()
#        self.extraRhoEdit.setMinimumSize(fm.width("8.88"), fm.height())
        self.extraRhoEdit.setSizePolicy(
            QtGui.QSizePolicy.MinimumExpanding, QtGui.QSizePolicy.Fixed)
        self.extraDLabel = QtGui.QLabel(u"d (µm) = ")
        self.extraD = QtGui.QLabel(u"")

        self.buttonCalculate = QtGui.QPushButton("Calculate")
        self.connect(
            self.buttonCalculate, QtCore.SIGNAL("clicked()"), self.calculate)
        self.buttonAbout = QtGui.QPushButton(r"&About...")
        self.connect(
            self.buttonAbout, QtCore.SIGNAL("clicked()"), self.about)
        self.buttonHelp = QtGui.QPushButton(r"&Help...")
        self.connect(
            self.buttonHelp, QtCore.SIGNAL("clicked()"), self.myhelp)

        whatLayout = QtGui.QHBoxLayout()
        whatLayout.addWidget(self.whatLabel)
        whatLayout.addWidget(self.whatCB)
        whatLayout.addStretch()

        compoundLayout = QtGui.QHBoxLayout()
        compoundLayout.addWidget(self.compoundLabel)
        compoundLayout.addWidget(self.compoundExLabel)
        compoundLayout.addStretch()

        compoundMassLayout = QtGui.QHBoxLayout()
        ...
        energyLayout = QtGui.QHBoxLayout()
...
#        stepLayout.addStretch()
        ...
        hline1 = QtGui.QFrame(self)
        hline1.setFrameStyle(QtGui.QFrame.HLine)
    ...
    def parse_compound(self):
        ...
        try:
            ...
            if withX:
                ...
                self.compoundMass.setText(
                    '{0}'.format(xc.round_to_n(cMass, 4)))
    ...
    def calculate(self):
        ...
        if self.what == POWDER:
            ...
            if isinstance(res, str):
                self.clear_results()
                QtGui.QMessageBox.critical(self, "Error", res)
                return
            nu, m, th, eDict = res
            ...
            for i, (k, v) in enumerate(eDict.items()):
                self.stepCB.addItem("{0}(m={1}mg={2}wt%): {3}".format(
