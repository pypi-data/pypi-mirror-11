...
class XenRT(object):
    ...
    def release_machine_resource(self, name, resource):
        """
        Release a resource locked by a machine  

        Parameters:  
        `name`: string - Machine to release resource from  
        `resource`: string - Resource to release  
        """
        path = "%s/machine/{name}/resources/{resource}" % (self.base)
        path = path.replace("{name}", self.__serializeForPath(name))
        path = path.replace("{resource}", self.__serializeForPath(resource))
        paramdict = {}
        files = {}
        payload = {}
        myHeaders = {'content-type': 'application/json'}
