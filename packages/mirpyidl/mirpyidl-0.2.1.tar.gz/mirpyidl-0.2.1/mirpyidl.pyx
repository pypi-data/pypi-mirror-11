...
def _IDL_SetVariableArray(varname_bytes, vartype, ndarray):
    ...
    Note:
      There is another way to do this in which I could use 
      PyArray_DISABLEFLAGS(ndarray, np.NPY_OWNDATA) to tell numpy not to
      deallocate the memory used by the ndarray during garbage collection
      (At least I think that is what it does.)  I am honestly not sure if
      that is a good solution or not however.
    """

    cdef IDL_VPTR vptr
    cdef UCHAR *data_copy

    # Copy the array data in the ndarray.
    data_copy = <UCHAR *> malloc(np.PyArray_NBYTES(ndarray))
    memcpy(data_copy, <UCHAR *> np.PyArray_DATA(ndarray), np.PyArray_NBYTES(ndarray))
