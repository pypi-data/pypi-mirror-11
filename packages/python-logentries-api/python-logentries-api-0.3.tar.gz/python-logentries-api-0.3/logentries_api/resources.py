...
class Alerts(Resource):
    ...
    def create_inactivity_alert(self,
                                name,
                                logs,
                                regexes,
                                alert_configs,
        ...
        :param logs: A list of the logset ID's (the 'key' field)
        :type logs: list of str

        :param regexes: A list of the regexes to match
        :type regexes: list of str

        :param alert_configs: A list of AlertConfig classes (Ex:
            [``EmailAlert('me@mydomain.com')``])
        :type alert_configs: list of
        ...
        # TODO
        # Create an inactivity-specific label (with an 'sn' key
        # Create an inactivity-specific tag (with type 'tagit')
        # Create an inactivity hook
        #   with {
        #       'trigger': ['/No logs coming through/'],
        #       'actions': [ list of actions + inactivity-specific tag]
        #       'aux': {
        #           'inactivityAlertId': some uuid,
        #           'sn': label-sn},

        # inactivity_label = Labels(self.account_key).create(
        #     name=name,
        # )

        labels = Labels(self.account_key)
        label = labels.create(
            name='_HOOK_IDENTIFIER_',
            color='ffffff',
            description=''
        )
        tags = Tags(self.account_key)
        tag = tags.create(label.get('sn'))

        alert_ids = [
            self.create(
                alert_config,
                alert_frequency_count=alert_frequency_count,
                alert_frequency_unit=alert_frequency_unit,
            ).get('id')
            for alert_config in
            alert_configs
        ]
        alert_ids.append(tag.get('id'))

        hook_name = 'inactivity_{}'.format(
            str(fleming.unix_time(datetime.utcnow())) + '000'
        )

        hook_regexes = ['/No logs coming through/']
        hook_regexes.extend(regexes)

        hooks = Hooks(self.account_key)
        hook = hooks.create(
            name=hook_name,
        ...
        # add
        # 'aux': {
        #   'inactivityAlertId': '03294556-cab9-42d1-854e-7df9db81304a',
        #   'sn': '1030'
        # },
