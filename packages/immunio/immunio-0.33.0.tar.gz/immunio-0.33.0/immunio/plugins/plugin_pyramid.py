...
def add_hooks(run_hook, get_agent_func=None):
    ...
    add_remember_hook(run_hook)
    add_forget_hook(run_hook)
    ...
    add_parse_ticket_hook(run_hook)

    add_signed_deserialize_hook(run_hook)
    add_webob_signed_serializer_hook(run_hook)
    add_check_csrf_hook(run_hook)
    ...
    add_handle_request_hook(run_hook, get_agent_func)

    return meta


def add_handle_request_hook(run_hook, get_agent_func):
    """
    ...
    # the wsgi `app` object.
    @monkeypatch(pyramid.config.Configurator, "make_wsgi_app")
...
def add_signed_deserialize_hook(run_hook):
    """
...
def add_webob_signed_serializer_hook(run_hook):
    """
    ...
    @monkeypatch(pyramid.session.SignedSerializer, "loads")
...
def add_check_csrf_hook(run_hook):
    """
...
def add_parse_ticket_hook(run_hook):
    """
...
def add_remember_hook(run_hook):
    """
    ...
    @monkeypatch(pyramid.security, "remember")
...
def add_forget_hook(run_hook):
    ...
    @monkeypatch(pyramid.security, "forget")
