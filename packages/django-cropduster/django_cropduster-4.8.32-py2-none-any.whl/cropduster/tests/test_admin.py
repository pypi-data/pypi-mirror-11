...
class TestAdmin(CropdusterTestCaseMediaMixin, AdminSeleniumWebDriverTestCase):
    ...
    user_app, _, _ = User.__module__.rpartition('.')

    if user_app not in available_apps:
        available_apps.append(user_app)
    ...
    def tearDown(self):
        ...
        if hasattr(self, 'client') and hasattr(self.client, 'logout'):
            self.client.logout()
