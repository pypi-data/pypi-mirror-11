...
def equalize_histogram(img_path, out_path, color_space='YCR_CB'):
    if not cv2:
        warnings.warn("OpenCV not installed; histogram equalization not applied")
        return img_path
    ...
    # color_space = 'LAB'
    color_transform = getattr(cv2, 'COLOR_BGR2%s' % color_space)
    color_transform_rev = getattr(cv2, 'COLOR_%s2BGR' % color_space)

    if color_space in ('HSV', 'HSV_FULL'):
        channels = [2]
    elif color_space in ('HLS', 'HLS_FULL'):
        channels = [1]
    ...
    mode_img = cv2.cvtColor(img, color_transform)
    mode_channels = cv2.split(mode_img)

    for i in channels:
        # mode_channels[i] = cv2.equalizeHist(mode_channels[i])
        mode_channels[i] = clahe.apply(mode_channels[i])

    mode_img_eq = cv2.merge(mode_channels)
    new_image = cv2.cvtColor(mode_img_eq, color_transform_rev)

    cv2.imwrite(out_path, new_image)
    # lab_image_eq = cv2.merge([l_channel_eq, a_channel, b_channel])
    # new_image = cv2.cvtColor(lab_image_eq, cv2.COLOR_LAB2BGR)
    # cv2.imwrite(out_path, new_image)
