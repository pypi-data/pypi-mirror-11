    ...
    def autenticacao(self):
        ...
        if self.AUTENTICA_APLICACAO:
            if not self.chave_aplicacao:
                raise ValueError(u"O valor de CHAVE_APLICACAO não foi definido no entanto AUTENTICA_APLICACAO está como True.")
            autorizacao = "chave_aplicacao {} ".format(self.chave_aplicacao)
        if self.AUTENTICA_LOJA:
            if not self.chave_loja:
                raise ValueError(u"O valor de CHAVE_LOJA não foi definido no entanto AUTENTICACAO_LOJA está como True.")
            autorizacao = "{}chave_loja {} ".format(autorizacao, self.chave_loja)
        if self.AUTENTICA_USUARIO:
            if not self.chave_usuario:
                raise ValueError(u"O valor de CHAVE_USUARIO não foi definido no entanto AUTENTICACAO_USUARIO está como True.")
