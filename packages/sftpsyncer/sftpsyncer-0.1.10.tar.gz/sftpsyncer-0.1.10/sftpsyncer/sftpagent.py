...
def retry_on_fail(function, retry_num=3):
    """
    Based on paramiko.utils.retry_on_signal, but slightly broader, and retries
    on any failure
    """
    attempt_num = 1

    while attempt_num <= retry_num:
        try:
            return function()
        except (EnvironmentError, IOError):
            if attempt_num < retry_num:
                logger.info("SFTP operation failed: retry {} of {}".format(
                    attempt_num, retry_num))
            else:
                logger.exception(
                    "Exceeded SFTP operation retry attempts for {}".format(
                        function))
                raise
        attempt_num += 1
...
class SyncAgent(object):
    ...
    def disconnect(self):
        if self.conn:
            try:
                retry_on_fail(lambda: self.conn.close())
            except Exception as e:
                logger.debug(
                    "Couldn't terminate SFTP connection cleanly: %s" % e)
        if self.transport:
            try:
                retry_on_fail(lambda: self.transport.close())
            except Exception as e:
                logger.debug(
    ...
    def pull_dir(self, rel_path):
        ...
        files = retry_on_fail(lambda: self.conn.listdir(remote_abs_dir_path))
