...
class Agent:
    def __init__(self):
        self.fb_obj      = FeedBack()
        self.par         = {}         # Parameter dict
        self.ana         = {}         # Analytical dict
        self.catalog     = {}         # Catalog
        self.definitions = {}         # Definitions

    def fb(self, debug_level, data):
        self.fb_obj.add(debug_level, data)
...
def get_domain(cp, targetfile):
    ...
    try:
        ...
        if   (cp == 'cpanel'):
            ...
            cmd = subprocess.Popen(["/usr/bin/sitelookup", "-s", user[2]], stdout=subprocess.PIPE)
            domain, _ = cmd.communicate()
            domain = domain.split(' ')
            domain = domain[0]
        elif (cp == 'plesk'):
