...
def massflow_avg(filter,weight,var,cell=True):
  ...
  for var in var_list:
    avg_dict[var] = extract_data(integrate,var,fetch,cell=cell)
...
class file_reader:
  def  __init__(self,filename,comment='#'):
     ...
     for line in file:
         ...
         # for most headers
         if comment in  line and contains_digits(line) and contains_digits(line.split(' ')[1]):
             self.header_dict[line.split()[-1].strip()] = idx
             idx = idx + 1
     file.close()
...
def get_widget(vari,vmin,vmax):
     ...
     try:
        vdiff = vmax-vmin 
        variable_table = pa.GetLookupTableForArray(vari,1,RGBPoints=[vmin, 0.278431, 0.278431, 0.858824, vmin+(0.143*vdiff), 0.0, 0.0, 0.360784,vmin+(0.2857*vdiff), 0.0, 1.0, 1.0, 
        vmin+(0.4285*vdiff), 0.0, 0.501961, 0.0,  vmin+(0.5714*vdiff), 1.0, 1.0, 0.0,  vmin+(0.714257*vdiff), 1.0, 0.380392, 0.0,  vmin+(0.8571*vdiff), 0.419608, 0.0, 0.0, vmax,0.49, 0.0, 0.0]
        ,VectorMode='Magnitude',NanColor=[0.25,0.0,0.0],ColorSpace='RGB')
...
def get_slice_3D(filter,x,y,z,normal="z"):
