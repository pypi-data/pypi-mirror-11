...
def open_editor(editor_arg, filename):
    if editor_arg:
...
def run():
    ...
    parser.add_argument(
        '-e', '--editor', help='Opens the default editor', action='store_true'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '-w', '--watch', help='Watch for changes to articles and re-generate', action='store_true'
    )
    ...
    if args.generate:
        ...
        filename = new_item_task(Page, args.page)
        open_editor(args.editor, filename)
    elif args.article:
        filename = new_item_task(Article, args.article)
        open_editor(args.editor, filename)
    elif args.clear:
        ...
        print('\n=== WATCHING ===\n')
        watcher = Watcher()
        watcher.register(os.path.join(os.getcwd(), 'content'))
        watcher.start()
