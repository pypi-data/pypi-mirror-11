...
"""
    statico.contents
    ----------------
    Provides classes for handling contents
...
from .utils import parse_metadata, sorted_list_dir
...
class Page(object):
    ...
    def __init__(self, title, author, date=None, extension='md'):
        ...
        >>> page = Page('Test', 'Fred Smith')

        :param title:     str   The page's title
        :param author:    str   The page's author
        :param date:      str   The page's publication date (ISO format)
        :param extension: str   The extension to use
        """
        if date is None:
    ...
    def get_url_from(title_slug):
        """
        Gets the url from a slug

        :param title_slug: str  The title slug
        :return:           str  The url
        """
        return '/' + title_slug
    ...
    def get_default_metadata(self):
        """
        Returns the default metadata for the page item
        :return:
        """
        dm = self.__dict__
        ls = ['---\n']
    ...
    def get_file_pointer(self, mode='w'):
        """
        Opens the page file return a file pointer
    ...
    def create(self):
        """
        Creates the page item in the output directory
        :return:
        """
        try:
            ...
            fp = self.get_file_pointer()
            try:
                fp.writelines([unicode(x) for x in self.get_default_metadata()])
            except NameError:
                fp.writelines(self.get_default_metadata())
            fp.close()
...
class FileManager(object):
    """Defines utility methods for dealing with files"""

    @staticmethod
    def get_settings():
        """
        Returns the site settings in JSON format
        :return:
        """
        try:
    ...
    def process_categories(data, env):
        ...
        if categories:
            ...
            articles = Article.get_articles(article_files, content=False)
    ...
    def create_project():
        ...
        error = copy_directory(static_path, 'static')
        if error is not None:
            print(action(error, success=False))
        else:
            print(action('Static assets'))

        # Templates
        templates_path = os.path.join(dir_path, 'templates')
        error = copy_directory(templates_path, 'templates')
        if error is not None:
            print(action(error, success=False))
        else:
