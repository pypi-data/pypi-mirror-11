...
Version 2.0.2: The `repr()`for EasySettings was modified slightly to better
represent python syntax.

Version 2.0.1: Added unit tests and `JSONSettings`/`JSONMixin`.
...
as a dict. There is also a class method, ``JSONSettings.from_file()`` for
...
``__setitem__()`` method is preferred (``mysettings[option] = value``) over
``set()``.
