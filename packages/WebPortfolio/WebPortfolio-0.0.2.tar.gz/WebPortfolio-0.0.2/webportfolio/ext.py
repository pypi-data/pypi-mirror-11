...
def _with_user_roles(timeout=600, *roles):
    name = "decorator_with_user_roles_%s" % flask_login.current_user.id

    @cache.memoize(timeout, make_name=name)
    def cached(user_id, *roles):
        return flask_login.current_user.has_any_roles(*roles)
    return cached(flask_login.current_user.id, *roles)
    ...
    def wrapper(f):
        ...
        def wrapped(*args, **kwargs):
            if user_authenticated():
                if not _with_user_roles(timeout=300, *roles):
                    return abort(403)
            else:
                return abort(401)
...
def with_user_permission(*permissions):
    pass
