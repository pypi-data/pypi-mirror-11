...
**Note** Whereas ``intspan``
attempts to faithfully implement the attributes of a Python ``set``,
``intspanlist`` is a thin, experimental shim. It works fine as an immutable
type, but changes are more problematic. It does not
...
in the container). ``insert`` and other ``list`` update
methods, however, provide no such promises. Indeed, it's not entirely sure
what the behavior for updates should be, given the use case. Raise an exception?
Silently refuse to add items already seen (the current default)? Or something
else.

Also, unlike with the core
``intspan`` objects, ``intspanlist`` tests are not yet complete. Swim at
your own risk.
