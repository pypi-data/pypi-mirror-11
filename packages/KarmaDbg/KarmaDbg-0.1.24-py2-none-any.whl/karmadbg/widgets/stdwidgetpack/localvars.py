...
from karmadbg.dbgcore.varprint import getLocals, getFields
...
class LocalVarsWidget(NativeDataViewWidget):
    ...
    def dataUpdate(self):
        ...
        try:
            ...
            vars = yield( self.uimanager.debugClient.callServerAsync(getLocals ) )
            for varName, varValue, varType, varLocation in vars:
                ...
                fields = yield( self.uimanager.debugClient.callServerAsync(getFields, rootItem.type, rootItem.location ) )
    ...
    def onExpandItem(self, modelIndex):
        ...
        for r in range(item.rowCount()):
            ...
            if not childItem.hasChildren():
                fields = yield( self.uimanager.debugClient.callServerAsync(getFields, childItem.type, childItem.location ) )
                for fieldName, fieldValue, fieldType, fieldLocation in fields:
                    fieldItem = VarItem(self.uimanager, fieldName, fieldType, fieldLocation)
    ...
    def dataUpdate(self):
        ...
        try:
            ...
            for r in range(self.treeModel.rowCount() - 1):
                ...
                varName, varValue, varType, varLocation = yield( self.uimanager.debugClient.callServerAsync(getTypedVar, varName ) )
                rootItem = VarItem(self.uimanager, varName, varType, varLocation)
