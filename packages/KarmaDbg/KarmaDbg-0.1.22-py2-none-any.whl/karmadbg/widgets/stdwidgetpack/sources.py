...
class SourceView(QTextEdit):
    ...
    def setContent(self, content):
        ...
        self.highlight()
        self.setHtml(self.highlightedContent)

    def highlight(self):
        content = self.rawContent.splitlines()
        body = ""
        for i in xrange(len(content)):
            lineno = i + 1
            line = self.codec.toUnicode(content[i])
            if line == "":
                line = " "
            if lineno == self.currentLine:
                body += self.highlightCurrentLine(line)
            elif lineno in self.breakpointLines:
                body += self.highlightBreakpointLine(line)
            else:
                body += self.highlightSourceLine(line)

        self.highlightedContent = self.htmlTemplate % (self.css, body)

    def highlightCurrentLine(self, str):
        return "<div class=\"current\"><pre>%s</pre></div>\n" % cgi.escape(str)

    def highlightSourceLine(self, str):
        return "<div class=\"normal\"><pre>%s</pre></div>" % cgi.escape(str)

    def highlightBreakpointLine(self, str):
        return "<div class=\"breakpoint\"><pre>%s</pre></div>" % cgi.escape(str)
    ...
    def contextMenuEvent(self, event):
        ...
        bpLineNo = reduce(lambda x,y: x +1 if y=="\n" else x, self.rawContent[:textPosition], 0) + 1
