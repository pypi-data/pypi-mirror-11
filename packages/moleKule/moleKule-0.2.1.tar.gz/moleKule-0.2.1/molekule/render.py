...
def take_snapshot(cwd=None, xyzFile=None, cmdFile=None, pngOut=None,
    ...
    if not os.path.isfile(fXyzPath):
        raise IOError('No file named %s found' % (fXyzPath,))
    if cmdFile is None:
        cmdFile = 'vmd_commands'
    fCmdPath = os.path.join(cwd, cmdFile)
    gen_vmdCmds(cwd=cwd, cmdFile=cmdFile, pngOut=pngOut)

    if not simulate:
        fCmdPath = os.path.join(cwd, 'temp'+cmdFile)
...
def gen_vmdCmds(cwd=None, cmdFile='vmd_commands', pngOut='vmdscene.png'):
    if not os.path.isfile(os.path.join(cwd, cmdFile)):
        with open(os.path.join(cwd, 'temp'+cmdFile), 'w') as f:
            vmd_cmds = vmd_commands % pngOut
            for line in vmd_cmds.split('\n'):
                f.write(line + '\n')
    else:
        with open(os.path.join(cwd, cmdFile), 'r') as f:
            s = ''.join(f.readlines())
        with open(os.path.join(cwd, 'temp'+cmdFile), 'w') as f:
            vmd_cmds = s % pngOut
            for line in vmd_cmds.split('\n'):
...
def gen_movie(files=None, cwd=None):
    # HERE
    pass
...
def FitBlock(data=None, x=None, y=None, sysName=None, poli='Polynomial',
             pngOut=None, left=None, right=None, **kwargs):
    """
    ...
    ax0.set_ylabel('y')
    ax0.set_xlabel('x')
    ax0.scatter(x, y, s=50, facecolors='none', edgecolors='red', **kwargs)
    ...
    if left:
        ax1.set_xlim(left=left)
        ax2.set_xlim(left=left)
    if right:
        ax1.set_xlim(right=right)
        ax2.set_xlim(right=right)
    ...
    f2.suptitle(sysName)
    ax1.scatter(x, y, s=30, facecolors='none', edgecolors='blue', **kwargs)
    ...
    while True:
        ...
        if selist[0] == 'r':
            ...
            for i in range(len(left_x)):
                if re.match(patterns['real'], str(left_x[i])):
