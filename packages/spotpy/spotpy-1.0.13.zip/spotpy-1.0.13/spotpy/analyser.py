...
def calc_like(results,evaluation):
    """
    Calculate another likelihood of your results
...
def compare_different_likelihoods(like1,like2):
	...
	:like1: Likelihood values
	:type: list
	
	:like2: Other Likelihood values
	:type: list
...
def get_best_parameterset(results,maximize=True):
    """
	Get the best parameter set of your result array, depending on your first likelihood 
    
    :results: Expects an numpy array which should have as first axis an index "like" or "like1". 
    :type: array  
	
	:maximize: Optional, default=True meaning the highest likelihood is taken as best, if False the lowest likelihood is taken as best.
	:type: boolean
...
def get_sensitivity_of_fast(results,like_index=None,M=4, print_to_console=True):
	...
	:like_index: Optional, index of Likelihood to base the sensitivity on, default=None first likelihood is taken 
    :type: int  
...
def plot_fast_sensitivity(results,likes=['mean'],like_indices=None,number_of_sensitiv_pars=10):
	...
	:likes: Optional, header of your likelihood
	:type: list
	
	:like_indices: Optional, index of Likelihood to base the sensitivity on, default=None first likelihood is taken 
    :type: int  
...
def plot_likelihoodtraces(results,evaluation,algorithms,filename='Like_trace'):
    font = {'family' : 'calibri',
        'weight' : 'normal',
        'size'   : 20}
    plt.rc('font', **font)   
