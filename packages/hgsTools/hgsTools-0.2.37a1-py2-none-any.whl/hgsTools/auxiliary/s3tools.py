...
def s3cmdGET(config, s3_path, local_dir, shellOutput=True):
    s3cfg_loc = auxiliary.read_option(config, 'general', 's3cfg_loc')
    cmd = "s3cmd -c %s get %s %s" %(s3cfg_loc, s3_path, os.path.join(local_dir, os.path.basename(s3_path)))
