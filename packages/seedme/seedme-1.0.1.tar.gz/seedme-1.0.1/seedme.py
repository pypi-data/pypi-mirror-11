...
try:
    import json
except ImportError: # use backport for python 2.5, thirparty module
    try: # look for installed version
...
class SeedMe(object):
    ...
    ###########################################################################
    # Sets timeout out for HTTP POST and GET
    ###########################################################################
    def set_timeout(self, duration=60):
        ...
        if not isinstance(duration, int) or duration < 1:
            raise TypeError('Connection timeout duration must be an integer > 0')
        ...
        for i in range(retry):
            ...
            if old_service in self.base_url:
                ...
                for d in result_list:
                    if 'files' in d: # has nested list, so walk and add child dict
                        for item in d['files']:
            ...
            if keyvalues:
                ...
                if isinstance(keyvalues, str):
                    if len(keyvalues.split(':')) >= 2:
                        ...
                        log.warning('Skipping keyvalue: string should be' +\
                                    'colon delimited as key:value.' +\
                                    ' You specified:' + keyvalues)
                else:
    ...
    def create_collection(self,
        ...
        if not title and not self.url[-6:] == 'update':
            log.error('Collection title is required')
            sys.exit(1) #bail
    ...
    def _do_query_(self, q_data):
        ...
        if my_method == 'curl':
            ...
            if self.timeout:
                my_buffer += ' --connect-timeout ' + str(self.timeout)
...
|                   |   collection is  | -V version info  | -to timeout        |
|                   |   shared         |                  | -url alt REST url  |
|                   |                  |                  | -v verbosity: DEBUG|
|                   |                  |                  |     INFO, WARNING, |
|                   |                  |                  |     ERROR, CRITICAL|
.-------------------.------------------.-----------------.---------------------.
...
"""
    ...
    if sequence_dict and not args.sequence_title:
        print("ERROR: Argument -st or -sequence_title is required\n")
        sys.exit(1)
    ...
    # Set timeout
    if args.timeout:
        seedme.set_timeout(args.timeout)
    ...
    if num_args and not args.update_id and not args.query and not args.download:
        ...
        if isinstance(args.query, bool): #query all collections
            ...
            result = seedme.query(qid=args.query,
