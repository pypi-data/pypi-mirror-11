...
class CompareTable(object):
	...
	def alignCore(self,align):
		if self.useSql():
			...
			if (self.comp.p is None):
				...
				tab2 = self.comp.b
				tab3 = self.comp.a
			sc = SqlCompare(tab1.getDatabase(), tab1, tab2, tab3, align)
			sc.apply()
	...
	def useSql(self):
		if (self.comp.compare_flags is None):
			return False
		return (self.comp.compare_flags.diff_strategy == "sql")
...
class Coopy(object):
	...
	def loadTable(self,name):
		...
		if (ext == "sqlite"):
			...
			helper = SqliteHelper()
			name1 = ""
			if (((self.flags is None) or ((self.flags.tables is None))) or ((python_lib_Builtin.len(self.flags.tables) == 0))):
				...
				if (python_lib_Builtin.len(names) == 0):
					...
					return None
				name1 = (names[0] if 0 < python_lib_Builtin.len(names) else None)
			else:
				name1 = python_internal_ArrayImpl._get(self.flags.tables, 0)
				if (python_lib_Builtin.len(self.flags.tables) > 1):
					self.io.writeStderr("! Cannot compare more than one table yet\n")
			tab = SqlTable(sql, SqlTableName(name1), helper)
			self.strategy = "sql"
			return tab
	...
	def coopyhx(self,io):
		...
		if (cmd1 == "git"):
			...
			if (pos23 < 0):
				pos23 = 0
			res23 = args[pos23:(pos23 + len)]
			del args[pos23:(pos23 + len)]
			types = res23
		...
		if git:
			...
			if (pos24 < 0):
				pos24 = 0
			res24 = args[pos24:(pos24 + ct)]
			del args[pos24:(pos24 + ct)]
			git_args = res24
			...
			if (pos25 < 0):
				pos25 = 0
			res25 = args[pos25:(pos25 + len1)]
			del args[pos25:(pos25 + len1)]
			res25
			offset = 0
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 937, 'className': "Coopy", 'methodName': "show"}))
	...
	def examineCell(x,y,view,raw,vcol,vrow,vcorner,cell,offset = 0):
		...
		if ((x == 0) and ((offset > 0))):
			def _hx_local_8():
				cell.category = "index"
				return cell.category
			cell.category_given_tr = _hx_local_8()
...
Index = _hx_classes.registerClass("Index", fields=["items","keys","top_freq","height","cols","v","indexed_table","hdr","ignore_whitespace"], methods=["addColumn","indexTable","toKey","toKeyByContent","getTable"])(Index)
...
class Meta(object):
	pass
Meta = _hx_classes.registerClass("Meta", methods=["alterColumns","changeRow","applyFlags","asTable","cloneMeta","useForColumnChanges","useForRowChanges","getRowStream"])(Meta)
...
class SqlCompare(object):
	...
	def validateSchema(self):
		...
		if (((python_lib_Builtin.len(all_cols1) == 0) or ((python_lib_Builtin.len(all_cols2) == 0))) or ((python_lib_Builtin.len(all_cols3) == 0))):
			raise _HxException("Error accessing SQL table")
		if (not ((self.equalArray(key_cols1,key_cols2) and self.equalArray(key_cols1,key_cols3)))):
			haxe_Log.trace("sql diff not possible when primary key changes",_hx_AnonObject({'fileName': "SqlCompare.hx", 'lineNumber': 71, 'className': "SqlCompare", 'methodName': "validateSchema"}))
			return False
		if (python_lib_Builtin.len(key_cols1) == 0):
			haxe_Log.trace("sql diff not possible when primary key not available",_hx_AnonObject({'fileName': "SqlCompare.hx", 'lineNumber': 75, 'className': "SqlCompare", 'methodName': "validateSchema"}))
			return False
		return True
	...
	def link(self):
		...
		if (i2 == -3):
			i2 = self.at2
			_hx_local_4 = self
			_hx_local_5 = _hx_local_4.at2
			_hx_local_4.at2 = (_hx_local_5 + 1)
			_hx_local_5
		offset = 4
		if (i0 >= 0):
			_g1 = 0
	...
	def apply(self):
		...
		if (self.alt is not None):
			all_cols3 = self.alt.getColumnNames()
		data_cols1 = self.local.getAllButPrimaryKey()
		data_cols2 = self.remote.getAllButPrimaryKey()
		...
		while ((_g16 < _g7)):
			...
			sql_key_cols = (HxOverrides.stringOrNull(sql_key_cols) + HxOverrides.stringOrNull(self.local.getQuotedColumnName((key_cols[i6] if i6 >= 0 and i6 < python_lib_Builtin.len(key_cols) else None))))
		sql_all_cols = ""
		_g17 = 0
		...
		while ((_g17 < _g8)):
			...
			sql_all_cols = (HxOverrides.stringOrNull(sql_all_cols) + HxOverrides.stringOrNull(self.local.getQuotedColumnName((all_common_cols[i7] if i7 >= 0 and i7 < python_lib_Builtin.len(all_common_cols) else None))))
		sql_all_cols1 = ""
		_g18 = 0
		...
		while ((_g19 < _g10)):
			...
			sql_all_cols2 = (HxOverrides.stringOrNull(sql_all_cols2) + HxOverrides.stringOrNull(self.local.getQuotedColumnName((all_cols2[i9] if i9 >= 0 and i9 < python_lib_Builtin.len(all_cols2) else None))))
		sql_all_cols3 = ""
		if (self.alt is not None):
			_g110 = 0
			...
			while ((_g110 < _g20)):
				...
				sql_all_cols3 = (HxOverrides.stringOrNull(sql_all_cols3) + HxOverrides.stringOrNull(self.local.getQuotedColumnName((all_cols3[i10] if i10 >= 0 and i10 < python_lib_Builtin.len(all_cols3) else None))))
		sql_key_match2 = ""
		_g111 = 0
		...
		while ((_g111 < _g21)):
			...
			n = self.local.getQuotedColumnName((key_cols[i11] if i11 >= 0 and i11 < python_lib_Builtin.len(key_cols) else None))
		...
		if (self.alt is not None):
			...
			while ((_g112 < _g22)):
				...
				n1 = self.local.getQuotedColumnName((key_cols[i12] if i12 >= 0 and i12 < python_lib_Builtin.len(key_cols) else None))
		...
		while ((_g113 < _g23)):
			...
			n2 = self.local.getQuotedColumnName((data_common_cols[i13] if i13 >= 0 and i13 < python_lib_Builtin.len(data_common_cols) else None))
		...
		while ((_g114 < _g24)):
			...
			if (not key5 in present1.h):
				if (sql_data_mismatch != ""):
					sql_data_mismatch = (HxOverrides.stringOrNull(sql_data_mismatch) + " OR ")
				n3 = self.remote.getQuotedColumnName(key5)
		...
		if (self.alt is not None):
			...
			while ((_g115 < _g25)):
				...
				if (python_lib_Builtin.len(sql_data_mismatch) > 0):
					sql_data_mismatch = (HxOverrides.stringOrNull(sql_data_mismatch) + " OR ")
				n4 = self.local.getQuotedColumnName((data_common_cols[i15] if i15 >= 0 and i15 < python_lib_Builtin.len(data_common_cols) else None))
			...
			while ((_g116 < _g26)):
				...
				if (not key6 in present1.h):
					if (sql_data_mismatch != ""):
						sql_data_mismatch = (HxOverrides.stringOrNull(sql_data_mismatch) + " OR ")
					n5 = self.alt.getQuotedColumnName(key6)
		...
		while ((_g117 < _g27)):
			...
			n6 = self.local.getQuotedColumnName((all_cols1[i17] if i17 >= 0 and i17 < python_lib_Builtin.len(all_cols1) else None))
		...
		while ((_g118 < _g28)):
			...
			n7 = self.local.getQuotedColumnName((all_cols2[i18] if i18 >= 0 and i18 < python_lib_Builtin.len(all_cols2) else None))
		...
		if (self.alt is not None):
			...
			while ((_g119 < _g29)):
				...
				n8 = self.local.getQuotedColumnName((all_cols3[i19] if i19 >= 0 and i19 < python_lib_Builtin.len(all_cols3) else None))
		...
		while ((_g120 < _g30)):
			...
			n9 = self.local.getQuotedColumnName((key_cols[i20] if i20 >= 0 and i20 < python_lib_Builtin.len(key_cols) else None))
		...
		self.at1 = 1
		self.at2 = 1
		sql_inserts = ((((((((("SELECT DISTINCT 0 AS __coopy_code, NULL, " + HxOverrides.stringOrNull(rowid)) + " AS rowid, NULL, ") + HxOverrides.stringOrNull(sql_all_cols2)) + " FROM ") + HxOverrides.stringOrNull(sql_table2)) + " WHERE NOT EXISTS (SELECT 1 FROM ") + HxOverrides.stringOrNull(sql_table1)) + HxOverrides.stringOrNull(self.where(sql_key_match2))) + ")")
		sql_inserts_order = (["__coopy_code", "NULL", "rowid", "NULL"] + all_cols2)
		self.linkQuery(sql_inserts,sql_inserts_order)
		if (self.alt is not None):
			sql_inserts1 = ((((((((("SELECT DISTINCT 1 AS __coopy_code, NULL, NULL, " + HxOverrides.stringOrNull(rowid)) + " AS rowid, ") + HxOverrides.stringOrNull(sql_all_cols3)) + " FROM ") + HxOverrides.stringOrNull(sql_table3)) + " WHERE NOT EXISTS (SELECT 1 FROM ") + HxOverrides.stringOrNull(sql_table1)) + HxOverrides.stringOrNull(self.where(sql_key_match3))) + ")")
		...
		if (self.alt is None):
			sql_deletes = ((((((((("SELECT DISTINCT 0 AS __coopy_code, " + HxOverrides.stringOrNull(rowid)) + " AS rowid, NULL, NULL, ") + HxOverrides.stringOrNull(sql_all_cols1)) + " FROM ") + HxOverrides.stringOrNull(sql_table1)) + " WHERE NOT EXISTS (SELECT 1 FROM ") + HxOverrides.stringOrNull(sql_table2)) + HxOverrides.stringOrNull(self.where(sql_key_match2))) + ")")
...
SqlCompare = _hx_classes.registerClass("SqlCompare", fields=["db","local","remote","alt","at0","at1","at2","align","peered","alt_peered","needed"], methods=["equalArray","validateSchema","denull","link","linkQuery","where","scanColumns","apply"])(SqlCompare)
...
class SqlTable(object):
	...
	def changeRow(self,rc):
		if (self.helper is None):
			haxe_Log.trace("No sql helper",_hx_AnonObject({'fileName': "SqlTable.hx", 'lineNumber': 179, 'className': "SqlTable", 'methodName': "changeRow"}))
			return False
		if (rc.action == "+++"):
	...
	def asTable(self):
		...
		while ((_g < w)):
			...
			mt.setCell(i,2,(1 if ((self.columns[x] if x >= 0 and x < python_lib_Builtin.len(self.columns) else None).primary) else 0))
		return mt
...
SqlTable = _hx_classes.registerClass("SqlTable", fields=["db","columns","name","quotedTableName","cache","columnNames","h","helper","id2rid"], methods=["getColumns","getPrimaryKey","getAllButPrimaryKey","getColumnNames","getQuotedTableName","getQuotedColumnName","getCell","setCellCache","setCell","getCellView","isResizable","resize","clear","insertOrDeleteRows","insertOrDeleteColumns","trimBlank","get_width","get_height","getData","clone","getMeta","alterColumns","changeRow","asTable","useForColumnChanges","useForRowChanges","cloneMeta","applyFlags","getDatabase","getRowStream","fetchRow","fetchColumns"], interfaces=[RowStream,Meta,Table])(SqlTable)
...
class SqliteHelper(object):
	...
	def attach(self,db,tag,resource_name):
		if (not db.begin((("ATTACH ? AS `" + HxOverrides.stringOrNull(tag)) + "`"),[resource_name],[])):
			haxe_Log.trace(((("Failed to attach " + HxOverrides.stringOrNull(resource_name)) + " as ") + HxOverrides.stringOrNull(tag)),_hx_AnonObject({'fileName': "SqliteHelper.hx", 'lineNumber': 128, 'className': "SqliteHelper", 'methodName': "attach"}))
			return False
		db.end()
		return True
	...
	def fetchSchema(self,db,name):
		...
		if (not db.begin(query,None,["sql"])):
			haxe_Log.trace(("Cannot find schema for table " + HxOverrides.stringOrNull(tname)),_hx_AnonObject({'fileName': "SqliteHelper.hx", 'lineNumber': 143, 'className': "SqliteHelper", 'methodName': "fetchSchema"}))
			return None
		sql = ""
		if db.read():
			sql = db.get(0)
	...
	def _hx_exec(self,db,query):
		if (not db.begin(query)):
			haxe_Log.trace("database problem",_hx_AnonObject({'fileName': "SqliteHelper.hx", 'lineNumber': 224, 'className': "SqliteHelper", 'methodName': "exec"}))
			return False
		db.end()
		return True
