...
@leip.arg('-t', '--task', help='regarding a certain task')
...
def create_indici(app, args):
    ...
    db.ensure_index('task')
    db.ensure_index('event')
    db.ensure_index('cwd')
...
def tail(app, args):
    ...
    def _recprint(rec):
        ...
        if rec['channel'] == 'default':
            ...
            chan = " %s(%s)%s" % (col.fg(62), rec['channel'], col.attr(0))

        loctime = pytz.utc.localize(rec['time']).astimezone(tzlocal())
        print("%s%s%02d%s %s%s%s%s %s %s(%s)%s" % (
