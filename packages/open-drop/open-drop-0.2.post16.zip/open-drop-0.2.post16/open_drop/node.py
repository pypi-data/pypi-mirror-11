...
class Proxy(ProxyBase):
    ...
    _CMD_GET_BUFFER = 0xe0;
    _CMD_BEGIN = 0xe1;
    _CMD_SET_I2C_ADDRESS = 0xe2;
    _CMD_STATE_OF_CHANNELS = 0xe3;
    _CMD_SET_STATE_OF_CHANNELS = 0xe4;
    _CMD_CHANNEL_COUNT = 0xe5;
    _CMD_ON_STATE_VOLTAGE_CHANGED = 0xe6;
    _CMD_ON_STATE_FREQUENCY_CHANGED = 0xe7;
    ...
    def eeprom_e2end(self):
        command = np.dtype('uint8').type(self._CMD_EEPROM_E2END)
        payload_size = 0
        payload_data = ''
    ...
    def i2c_packet_reset(self):
        command = np.dtype('uint8').type(self._CMD_I2C_PACKET_RESET)
        payload_size = 0
        payload_data = ''
    ...
    def state_of_channels(self):
        command = np.dtype('uint8').type(self._CMD_STATE_OF_CHANNELS)
        payload_size = 0
        payload_data = ''
    ...
    def set_state_of_channels(self, channel_states):
        command = np.dtype('uint8').type(self._CMD_SET_STATE_OF_CHANNELS)
        ARG_STRUCT_SIZE = 4
        ...
        struct_data = np.array([(array_info.length['channel_states'], ARG_STRUCT_SIZE + array_info.start['channel_states'], )],
                               dtype=[('channel_states_length', 'uint16'), ('channel_states_data', 'uint16'), ])
        payload_data = struct_data.tostring() + array_data
    ...
    def channel_count(self):
        command = np.dtype('uint8').type(self._CMD_CHANNEL_COUNT)
        payload_size = 0
        payload_data = ''
    ...
    def on_state_voltage_changed(self, original_value, new_value):
        command = np.dtype('uint8').type(self._CMD_ON_STATE_VOLTAGE_CHANGED)
        ARG_STRUCT_SIZE = 8
        array_data = ''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(original_value, new_value, )],
                               dtype=[('original_value', 'float32'), ('new_value', 'float32'), ])
        payload_data = struct_data.tostring() + array_data
    ...
    def on_state_frequency_changed(self, new_value):
        command = np.dtype('uint8').type(self._CMD_ON_STATE_FREQUENCY_CHANGED)
        ARG_STRUCT_SIZE = 4
        array_data = ''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(new_value, )],
