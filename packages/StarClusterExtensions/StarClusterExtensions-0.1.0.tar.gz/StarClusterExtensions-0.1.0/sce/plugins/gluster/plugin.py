...
class GlusterSetup(PluginSetup):
    _pool = None
    ...
    def run2(self):
        ...
        # commented out because gluster is expected to be available via apt-get already
        # log.info('Installing gluster packages.')
        # master.ssh.execute('add-apt-repository ppa:gluster/glusterfs-3.5 -y')
        # apt_update(master, checkfirst=False)
        # master.apt_install('openssh-server glusterfs-server glusterfs-client xfsprogs attr')
        master.apt_install('glusterfs-server')
    ...
    def on_add_node2(self, node):
        ...
        node.ssh.execute('add-apt-repository ppa:gluster/glusterfs-3.5 -y')
