        ...
        for attr in [a for a in dir(self.dataobject) if not a.startswith('_')]:
            ...
            if humanpath or humanfile:
                ...
                if isinstance(get_attr, str) or isinstance(get_attr, unicode):
                    ...
                    if attr in tokens:
                        setattr(self.dataobject, attr + '_human', urllib.unquote_plus(get_attr.encode('utf-8')))
...
def is_json(blob):
    ...
    try:
        ...
        raise Exception("Tried loading a json blob from function \"%s\", but failed. Blob: %s" % (mod.__name__, blob))
