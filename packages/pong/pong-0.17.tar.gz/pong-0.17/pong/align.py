...
def get_best_perm(pong, run1, run2, worst_choice=2, sim_thresh=.97):
	...
	if runs[run2].K == runs[run1].K+1:
		run2_tuples = match.get_best_2tuples()[:(runs[run1].K+1)**2]
		average = 0
		tmp_tuple = (0,0)
		for t0,t1 in run2_tuples:
			labels = [x for x in match.to_nodes if (x == t1) or ((len(x)<2) and (x[0] != t1[0] and x[0] != t1[1]))]
			mat = [[sys.maxsize if t1==x or (t0-1)==y else 1-match.edges[(y+1,x)] for x in labels] for y in range(len(labels))]
			mat[t0-1][labels.index(t1)] = 1-match.edges[(t0,t1)]
		...
		# p1,p2,average = find_permutation(mat,labels)
