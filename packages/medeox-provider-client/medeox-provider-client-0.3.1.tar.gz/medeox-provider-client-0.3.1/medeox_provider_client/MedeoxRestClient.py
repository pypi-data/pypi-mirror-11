...
import requests
from utils import Connection, toListOfDicts, handleResponseErrors, toDateTimeString
...
class MedeoxRestClient:
	...
	def createSchedule(self, schedule):
		response = requests.post(self._getSchedulesBaseUrl(), data = schedule.__dict__,
	...
	def rescheduleAppointment(self, providerAppointmentId, newSlot):
		response = requests.patch(self._getAppointmentUrl(providerAppointmentId), 
			data = { 'newStart':toDateTimeString(newSlot.datetime), 'newDuration': newSlot.duration},
            headers = self._connection.headers)
		handleResponseErrors(response)
		#print response