...
class DeltaTest(testcase.ZDBTestCase):
    ...
    def test_metadata_tags_merge_many(self):
        ...
        stop_ip = start_ip + 5
        ra, rb = self.makeTwoSetOfRecordsWithDifferentTagsAndMetdata(
            start_ip, stop_ip, 443, 20, 6, 80, 8, 5
        )
        rc, rd = self.makeTwoSetOfRecordsWithDifferentTagsAndMetdata(
            start_ip, stop_ip, 443, 20, 4, 995, 10, 4
        )
        for a, b, c, d in itertools.izip(ra, rb, rc, rd):
            delta = self.service.PutHostIPv4Record(a, self.TIMEOUT)
            self.assertEqual(delta.delta_scope, hoststore_pb2.Delta.SCOPE_HOST)
            self.assertDeltaMatchesRecord(delta, a)
            self.assertDeltaHasMergedTagsAndMetadata(delta, [a])
            delta = self.service.PutHostIPv4Record(b, self.TIMEOUT)
            self.assertEqual(delta.delta_scope, hoststore_pb2.Delta.SCOPE_PORT)
            self.assertDeltaMatchesRecord(delta, b)
            delta = self.service.PutHostIPv4Record(c, self.TIMEOUT)
            self.assertEqual(delta.delta_scope, hoststore_pb2.Delta.SCOPE_SUBPROTOCOL)
            self.assertDeltaMatchesRecord(delta, c)
            delta = self.service.PutHostIPv4Record(d, self.TIMEOUT)
            self.assertEqual(delta.delta_scope, hoststore_pb2.Delta.SCOPE_PORT)
            self.assertDeltaMatchesRecord(delta, d)
            self.assertDeltaHasMergedTagsAndMetadata(delta, [a, b, c, d])
