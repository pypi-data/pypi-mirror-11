...
class AdminService(object):
    ...
    def dump_certificates(self, path, incremental=False, max_records=0):
        return self._service.DumpCertificatesToJSON(rpc_pb2.Command(filepath=path, 
                incremental_dump=incremental, max_records=max_records), self.TIMEOUT)

    def dump_ipv4(self, path, max_records=0):
        return self._service.DumpIPv4ToJSON(rpc_pb2.Command(filepath=path, max_records=max_records), self.TIMEOUT)

    def dump_domain(self, path, max_records=0):
        return self._service.DumpDomainToJSON(rpc_pb2.Command(filepath=path, max_records=max_records), self.TIMEOUT)
