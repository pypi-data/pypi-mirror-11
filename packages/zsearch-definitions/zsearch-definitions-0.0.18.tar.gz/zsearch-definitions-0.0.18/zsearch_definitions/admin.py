...
class AdminService(object):
    ...
    def dump_ipv4(self, path, max_records=0):
        retv = self._service.DumpIPv4ToJSON(rpc_pb2.Command(filepath=path,
                max_records=max_records), 8*60*60) # eight hours
        if retv.status != rpc_pb2.CommandReply.SUCCESS:
            raise Exception("ZDB failure: %s" % retv.error)
        return retv
    ...
    def dump_domain(self, path, max_records=0):
        retv = self._service.DumpDomainToJSON(rpc_pb2.Command(filepath=path,
                max_records=max_records), 60*60) # one hour
        if retv.status != rpc_pb2.CommandReply.SUCCESS:
            raise Exception("ZDB failure: %s" % retv.error)
        return retv
