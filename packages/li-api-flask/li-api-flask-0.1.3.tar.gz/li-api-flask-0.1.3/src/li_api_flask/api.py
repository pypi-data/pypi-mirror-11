...
class LIApi(object):
    ...
    def __init__(self, settings, nome=None, prefixo_url_api=None, usa_metadados=False):
        ...
        if prefixo_url_api:
            self.prefixo_url = self._monta_url(prefixo_url_api, self.prefixo_url)
    ...
    def registrar_recurso(self, url, recurso, max_age=0, sem_cache=None, usa_prefixo=True):
        ...
        if not isinstance(url, tuple):
            url = (url, )
        url = tuple([self._monta_url(self.prefixo_url, _url) if usa_prefixo else _url for _url in url])
