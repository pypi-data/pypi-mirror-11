...
from flask import Flask, make_response, logging
...
class LIApi(object):
    ...
    def __init__(self, settings, nome=None):
        """
        ...
        self.registrar_recurso('/', Home)
        self.registrar_recurso('/healthcheck', Healthcheck)

        if settings.SENTRY_DSN_API:
            sentry = Sentry(dsn=settings.SENTRY_DSN_API)
            sentry.init_app(self.app)

    def registrar_recurso(self, url, recurso, max_age=0, sem_cache=None):
        """
        ...
        if isinstance(url, tuple):
            for _url in url:
                self.http_cache.define_cache(_url, max_age=max_age, sem_cache=sem_cache)
    ...
    def registra_excecoes(self):
        ...
        def recurso_nao_encontrado(erro):
            conteudo, status = serializacao.ResultadoDeApi.resposta({}, self.nome, self.versao, 404)
    ...
    def trata_erros_api(self, erro):
        ...
        if nome == 'Method Not Allowed':
            ...
            conteudo, status = serializacao.ResultadoDeApi.resposta({}, self.nome, self.versao, 405)
            return make_response(conteudo, status, headers)
        if nome == 'Bad Request':
            conteudo, status = serializacao.ResultadoDeApi.resposta({'mensagem': u'A requisição é inválida.'}, self.nome, self.versao, 400)
            return make_response(conteudo, status, headers)
        if type(erro) is cadastro.DadosInvalidos:
            conteudo, status = serializacao.ResultadoDeApi.resposta({'mensagem': u'Ocorreram erros de validação.', 'erros': erro.erros}, self.nome, self.versao, 400)
            return make_response(conteudo, status, headers)

        execao = erro
        execao.tipo = exc_type
        execao.tb = tb
        conteudo, status = serializacao.ResultadoDeApi.resposta({'mensagem': 'Ocorreu um erro inesperado no servidor'}, self.nome, self.versao, status=500, excecao=execao)
