...
class LIApi(object):
    ...
    def __init__(self, settings, nome=None, prefixo_url_api=None, usa_metadados=False):
        ...
        self.registrar_recurso('/', Home, usa_prefixo=False)
        self.registrar_recurso('/healthcheck', Healthcheck, usa_prefixo=False)

        if settings.SENTRY_DSN_API:
            sentry = Sentry(dsn=settings.SENTRY_DSN_API)
            sentry.init_app(self.app)

    def _monta_url(self, *partes):
        total = []
        partes = list(partes)
        for parte in partes:
            if parte:
        ...
        prefixo_api = ''
        if self.prefixo_url_api:
            prefixo_api = '/{}'.format(self.prefixo_url_api)
        return '{}/{}'.format(prefixo_api, '/'.join(total))

    def registrar_recurso(self, url, recurso, max_age=0, sem_cache=None, usa_prefixo=True):
        """
        ...
        if not isinstance(url, tuple):
            url = (url, )
        url = tuple([self._monta_url(self.prefixo_url, _url) if usa_prefixo else self._monta_url(_url) for _url in url])
