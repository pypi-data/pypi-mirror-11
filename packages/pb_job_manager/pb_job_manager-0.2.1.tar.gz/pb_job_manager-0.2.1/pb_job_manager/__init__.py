...
class PBJobManager(object):
    ...
    def add_job(self, job, dep_job_id=None):
        ...
        print("added job", job_id, type(job))
        print("cur jobs", sorted(self._jobs))
        return job_id
    ...
    def _get_next_leaf_job(self):
        for job_id, job in iteritems(self._jobs):
            ...
            print("cheeck", job_id, job, isinstance(job, self.pb.commands.base.BaseCommand))
    ...
    def _update_branch_jobs(self):
        for job_id, job in iteritems(self._jobs):
            ...
            if not isinstance(job, self.pb.commands.base.BaseCommand):
                print("updating branch for", job_id)
                del self._jobs[job_id]
                self.add_job(job())
                return True

    def _get_next_job(self):
        print("Pending jobs {} with dependencies {}".format(
            sorted(self._jobs), self._deps
        ))
